 ...
 *
 * Letta configures Aera Router as its "lmstudio" provider (localModelDiscovery:
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-letta-settings-"));
...
// ── Test 6: POST refuses to overwrite an existing non-Aera-Router lmstudio config ──
...
// ── Test 7: DELETE → removes the Aera Router lmstudio config ──────────────────
