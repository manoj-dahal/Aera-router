...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-batch-e2e-rl-"));
...
function createServerProcess() {
  ...
  const child = spawn(process.execPath, ["scripts/dev/run-next-playwright.mjs", "dev"], {
    ...
    env: {
      ...
      AERA_ROUTER_DISABLE_BACKGROUND_SERVICES: "false",
