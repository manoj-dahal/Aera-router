 ...
 * rebuilt on top of Aera Router's existing "custom" configType settings pattern
 ...
 * the `[model.aera-router]` section and preserves the rest of the file.
 */
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-grok-build-settings-"));
...
// ── Test 4: POST with valid body → surgically upserts [model.aera-router] ─────

test("grok-build-settings POST: writes [model.aera-router] section and preserves existing content", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      assert.ok(content.includes("[model.aera-router]"), "Config should have [model.aera-router]");
      assert.ok(content.includes("http://localhost:20128/v1"), "Config should contain base URL");
      assert.ok(content.includes('default = "aera-router"'), "Default should point at our slot");
      ...
      assert.ok(
        content.includes('aera-router-prev-default = "grok-build"'),
...
test("grok-build-settings DELETE: removes our section, preserves the rest, restores default", async () => {
  ...
  try {
    ...
    const preConfigured = [
      ...
      'default = "aera-router"',
      "",
      '# aera-router-prev-default = "grok-build"',
      "[model.aera-router]",
      'model = "grok-4.5"',
    ...
    if (res.status === 200) {
      ...
      const configPath = path.join(tmpHome, ".grok", "config.toml");
      const content = fs.readFileSync(configPath, "utf-8");
      assert.ok(!content.includes("[model.aera-router]"), "Our section should be removed");
