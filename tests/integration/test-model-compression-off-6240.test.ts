...
// completely independent of the per-request `x-aera-router-compression` header/mode. The internal
...
// a request carrying `x-aera-router-compression: off` must NOT get the styles system message
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-testmodel-compression-"));
...
test("chatCore: x-aera-router-compression: off suppresses Output Styles injection even with the operator's global style enabled (#6240)", async () => {
  ...
  assert.match(plainFirstMessage?.content ?? "", /Aera Router Output Styles/);

  // The "Test model" connection test sends x-aera-router-compression: off — must be clean.
  const testModelBody = await runChatCore({
    provider,
    model,
    connectionId: connection.id,
    headers: new Headers({ "x-aera-router-compression": "off" }),
  });
  ...
  const anyMessageHasMarker = (testModelBody?.messages ?? []).some((m) =>
    (m?.content ?? "").includes("Aera Router Output Styles")
  );
  assert.equal(
    anyMessageHasMarker,
    false,
