...
test("generateMitmCa produces a CA certificate (basicConstraints CA, key+cert PEM)", async () => {
  const ca = await generateMitmCa("Aera Router MITM CA (test)");
  ...
  assert.match(x.subject, /Aera Router MITM CA \(test\)/);
});
...
test("DynamicCertStore caches one SecureContext per hostname", async () => {
  const store = new DynamicCertStore("Aera Router MITM CA (test)");
