...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-codex-stream-false-"));
...
test("non-stream chat success carries cost-telemetry meta headers (cost/version/tokens)", async () => {
  ...
  // through attachAeraRouterMetaHeaders, which must always emit the cost-telemetry
  ...
  const cost = headers.get(AERA_ROUTER_RESPONSE_HEADERS.responseCost);
  ...
  const version = headers.get(AERA_ROUTER_RESPONSE_HEADERS.version);
  assert.equal(typeof version, "string");
  assert.ok(String(version).length > 0);

  // Real usage from the upstream body must surface as token-count headers.
  assert.equal(headers.get(AERA_ROUTER_RESPONSE_HEADERS.tokensIn), "10");
  assert.equal(headers.get(AERA_ROUTER_RESPONSE_HEADERS.tokensOut), "5");
});
