...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-api-auth-"));
...
test("getApiKeyMetadata recognizes AERA_ROUTER_API_KEY environment variable", async () => {
  const envKey = "sk-test-env-key-" + Date.now();
  process.env.AERA_ROUTER_API_KEY = envKey;
...
test("requireManagementAuth returns null for AERA_ROUTER_API_KEY env passthrough", async () => {
  await setupAuth();
