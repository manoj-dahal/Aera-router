...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "aera-router-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@aera-router\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@aera-router\/open-sse\/services\/model\.ts/);
