...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-emergency-fallback-"));
...
function resetTestState() {
  ...
  delete process.env.AERA_ROUTER_EMERGENCY_FALLBACK;
  resetEmergencyFallbackEnvCache();
...
function withEnv(value: string | undefined, fn: () => void) {
  const previous = process.env.AERA_ROUTER_EMERGENCY_FALLBACK;
  if (value === undefined) {
    delete process.env.AERA_ROUTER_EMERGENCY_FALLBACK;
  } else {
    process.env.AERA_ROUTER_EMERGENCY_FALLBACK = value;
  }
  resetEmergencyFallbackEnvCache();
  try {
    fn();
  } finally {
    restoreEnv("AERA_ROUTER_EMERGENCY_FALLBACK", previous);
    resetEmergencyFallbackEnvCache();
  }
}
...
function withFeatureFlagOverride(value: string, fn: () => void) {
  try {
    setFeatureFlagOverride("AERA_ROUTER_EMERGENCY_FALLBACK", value);
    resetEmergencyFallbackEnvCache();
    fn();
  } finally {
    removeFeatureFlagOverride("AERA_ROUTER_EMERGENCY_FALLBACK");
    resetEmergencyFallbackEnvCache();
  }
}
...
test("AERA_ROUTER_EMERGENCY_FALLBACK=false disables the 402 redirect", () => {
  withEnv("false", () => {
    assert.equal(isEmergencyFallbackEnvEnabled(), false);
    const decision = shouldUseFallback(402, "", false);
    assert.equal(decision.shouldFallback, false);
    assert.match(decision.reason, /AERA_ROUTER_EMERGENCY_FALLBACK/);
  });
});

test("AERA_ROUTER_EMERGENCY_FALLBACK=0 disables the budget-keyword redirect", () => {
  withEnv("0", () => {
