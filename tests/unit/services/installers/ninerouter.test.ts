...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-installer-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-fake-bin-"));
...
test("install creates package.json structure", async () => {
  ...
  assert.equal(parsedHost.name, "aera-router-9router-host");
