...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-suggested-models-route-"));
...
test("route source: imports and uses buildErrorBody (Hard Rule #12)", async () => {
  ...
  assert.match(
    ...
    "must import buildErrorBody from @aera-router/open-sse/utils/error"
  );
