...
// ─── #3476: discoverability of `aera-router runtime repair` ──────────────────────
//
// When better-sqlite3's native binding mismatches the Node ABI, Aera Router prints
...
// A self-heal command already exists (`aera-router runtime repair`); these tests
...
test("serve.mjs ABI-mismatch hint source mentions `runtime repair`", () => {
  ...
  assert.match(
    src,
    /runtime repair/,
    "serve.mjs should surface `aera-router runtime repair` as a recovery hint"
  );
});
