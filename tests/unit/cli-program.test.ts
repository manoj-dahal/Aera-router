...
test("program name is 'aera-router'", () => {
  const program = createProgram();
  assert.equal(program.name(), "aera-router");
});
...
test("program has --api-key option bound to env", () => {
  ...
  assert.equal(opt.envVar, "AERA_ROUTER_API_KEY");
});
...
test("program has --base-url option bound to env", () => {
  ...
  assert.equal(opt.envVar, "AERA_ROUTER_BASE_URL");
});
...
test("--help throws CommanderError with exit code 0", async () => {
  const program = createProgram();
  try {
    await program.parseAsync(["node", "aera-router", "--help"]);
...
test("--version throws CommanderError with exit code 0", async () => {
  const program = createProgram();
  try {
    await program.parseAsync(["node", "aera-router", "--version"]);
    assert.fail("expected error to be thrown");
  } catch (err: any) {
    assert.equal(err.exitCode, 0, `expected exitCode 0, got: ${err.exitCode}`);
  }
...
test("unknown global flag throws CommanderError with exit code 1", async () => {
  const program = createProgram();
  try {
    await program.parseAsync(["node", "aera-router", "--definitely-not-a-flag"]);
    assert.fail("expected error to be thrown");
  } catch (err: any) {
