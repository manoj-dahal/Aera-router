...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-crush-settings-"));
...
test("crush-settings POST: writes crush.json with an openai-compat providers.aera-router block", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        const provider = written.providers?.["aera-router"];
        assert.ok(provider, "providers.aera-router must be written");
...
test("crush-settings DELETE: removes only the aera-router provider entry", async () => {
  ...
  try {
      ...
      JSON.stringify({
        providers: {
          "aera-router": {
            ...
            api_key: "sk-test",
            models: [
              { id: "openai/gpt-5", name: "Aera Router: openai/gpt-5", context_window: 128000 },
            ],
          },
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        const written = JSON.parse(fs.readFileSync(configPath, "utf-8"));
        assert.equal(written.providers?.["aera-router"], undefined);
