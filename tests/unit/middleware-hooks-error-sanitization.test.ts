 ...
 * "SQLITE_ERROR: near /var/lib/aera-router/..."). Guard against
...
for (const path of ROUTES) {
  test(`${path} imports sanitizeErrorMessage from open-sse/utils/error`, () => {
    ...
    assert.ok(
      /import\s*\{[^}]*\bsanitizeErrorMessage\b[^}]*\}\s*from\s*["']@aera-router\/open-sse\/utils\/error["']/.test(
        source
      ),
      `expected ${path} to import sanitizeErrorMessage from "@aera-router/open-sse/utils/error"`
    );
  });
