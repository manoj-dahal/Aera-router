...
test("renameProcessTitle renames a bare 'next-server' title to 'aera-router'", () => {
  assert.equal(renameProcessTitle("next-server"), "aera-router");
});

test("renameProcessTitle preserves a version suffix after 'next-server'", () => {
  assert.equal(renameProcessTitle("next-server (v16.2.9)"), "aera-router (v16.2.9)");
});
...
test("renameProcessTitle is idempotent when called again on an already-renamed title", () => {
  ...
  assert.equal(once, "aera-router (v16.2.9)");
  assert.equal(twice, "aera-router (v16.2.9)");
});
