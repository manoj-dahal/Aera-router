/**
 * #6422 — X-Aera-Router-Compression response header echo.
 *
 * Docs promise that when a request supplies `x-aera-router-compression`, the response
 * echoes `X-Aera-Router-Compression: <mode>; source=<source>`. Internal early-returns
...
function makeRequest(headerValue: string | null): {
  ...
  return {
    headers: {
      get(name: string) {
        if (name.toLowerCase() === "x-aera-router-compression") return headerValue;
        return null;
      },
    },
  };
...
describe("compressionHeaderEcho (#6422)", () => {
  ...
  it("echoes the request header value onto the response when missing", () => {
    ...
    assert.equal(
      wrapped.headers.get("X-Aera-Router-Compression"),
  ...
  it("normalizes off / default / engine:* to lowercase", () => {
    assert.equal(
      withCompressionHeaderEcho(new Response(""), "OFF").headers.get("X-Aera-Router-Compression"),
      "off; source=request-header"
    );
    assert.equal(
      withCompressionHeaderEcho(new Response(""), "Default").headers.get(
  ...
  it("preserves operator casing on named-combo values", () => {
    ...
    assert.equal(
      wrapped.headers.get("X-Aera-Router-Compression"),
      "MyCombo; source=request-header"
    );
  });

  it("never overwrites an existing X-Aera-Router-Compression header set by the inner pipeline", () => {
    const inner = new Response("", {
      headers: {
        "X-Aera-Router-Compression": "stacked; source=routing; tokens=100->42; rules: rtk-nl x2",
      },
    });
    const wrapped = withCompressionHeaderEcho(inner, "engine:rtk");
    assert.equal(
  ...
  it("is a no-op when the request did not supply the header", () => {
    ...
    assert.equal(wrapped.headers.get("X-Aera-Router-Compression"), null);
