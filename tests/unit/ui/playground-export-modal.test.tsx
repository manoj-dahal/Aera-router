...
describe("ExportCodeModal", () => {
  ...
  it("shows curl code with $AERA_ROUTER_API_KEY placeholder", () => {
    const el = renderModal();
    const pre = el.querySelector("pre");
    expect(pre?.textContent).toContain(API_KEY_PLACEHOLDER);
    expect(pre?.textContent).toContain("$AERA_ROUTER_API_KEY");
  });
  ...
  it("switching tab to TypeScript shows TS-specific code", async () => {
    ...
    const pre = el.querySelector("pre");
    const code = pre?.textContent ?? "";
    // TypeScript code should contain AERA_ROUTER_API_KEY
