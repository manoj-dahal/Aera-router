...
const argv = (...args: string[]) => ["node", "aera-router", ...args];
...
test("aera-router CLI --version fast-path prints ONLY the version, skipping bootstrap output", async () => {
  const pkg = JSON.parse(readFileSync(join(process.cwd(), "package.json"), "utf8")) as {
    version: string;
  };

  const { stdout } = await execFileAsync(process.execPath, ["bin/aera-router.mjs", "--version"], {
    cwd: process.cwd(),
