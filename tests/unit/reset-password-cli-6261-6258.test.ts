...
const AERA_ROUTER_BIN = path.join(ROOT, "bin", "aera-router.mjs");
...
// ~/.aera-router so DATA_DIR is the only data directory in play.
function baseEnv(dataDir: string, isolatedHome: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    DATA_DIR: dataDir,
    HOME: isolatedHome,
    // Give the CLI a key so bin/aera-router.mjs never warns/provisions.
...
function mkHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-reset-home-"));
}
function mkDataDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-reset-data-"));
}

// #6261: `aera-router reset-password` must be a real subcommand (not "unknown
...
test("aera-router reset-password subcommand applies the reset over piped stdin (#6261, #6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
    const res = spawnSync("node", [AERA_ROUTER_BIN, "reset-password"], {
...
test("aera-router-reset-password applies the reset over piped two-line stdin (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
...
test("aera-router-reset-password --password-stdin reads the whole stdin as the password (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
