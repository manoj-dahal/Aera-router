...
function assertSecurityInvariants(generated: string, label: string) {
  assert.ok(generated.includes(API_KEY_PLACEHOLDER), `${label}: must include $AERA_ROUTER_API_KEY`);
...
test("API_KEY_PLACEHOLDER is $AERA_ROUTER_API_KEY", () => {
  assert.equal(API_KEY_PLACEHOLDER, "$AERA_ROUTER_API_KEY");
});
...
test("chat.completions × python: contains required elements", () => {
  ...
  assert.ok(generated.includes('os.environ["AERA_ROUTER_API_KEY"]'), "uses os.environ");
  assert.ok(generated.includes("gpt-4o-mini"), "model present");
});
...
test("chat.completions × typescript: contains required elements", () => {
  ...
  assert.ok(generated.includes("process.env.AERA_ROUTER_API_KEY"), "uses process.env");
  assert.ok(generated.includes("gpt-4o-mini"), "model present");
});
