...
// Regression test: null apiKey + unresolvable keyId must yield the sentinel 'sk_aera_router',
...
test("sentinel guard condition catches sk_aera_router and null", () => {
  const SENTINEL = "sk_aera_router";
  // Simulate what the route guard checks: (!apiKey || apiKey === 'sk_aera_router')
  ...
  assert.equal(shouldReject("sk_aera_router"), true, "sentinel must be rejected");
