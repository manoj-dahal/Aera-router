...
// Regression test for GH #6677: `aera-router health` calls GET /api/health, but the
...
test.before(async () => {
  ...
  process.env.AERA_ROUTER_BASE_URL = baseUrl;
});
...
test("GH #6677: aera-router health should succeed against a server that only implements /api/monitoring/health", async () => {
