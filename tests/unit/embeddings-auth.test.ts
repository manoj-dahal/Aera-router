...
test("isValidApiKey", async (t) => {
  const originalEnv = process.env.AERA_ROUTER_API_KEY;

  await t.test("should return true if key matches AERA_ROUTER_API_KEY", async () => {
    process.env.AERA_ROUTER_API_KEY = "test-key";
...
test("POST /v1/embeddings authentication", async (t) => {
  ...
  process.env.AERA_ROUTER_API_KEY = originalOmniKey;
});
