 ...
 * (`serve --tray`) or the Electron Appearance tab; a plain `aera-router serve`
 * user had no way to enable it. The `autostart` command now exposes the
 * shorthand the reporter asked for (`aera-router autostart on` / `... true`) via
...
test("status is the default action (bare `aera-router autostart` is a safe read-only)", () => {
