...
process.env.DATA_DIR = path.join(os.tmpdir(), "aera-router-check-tool-test");
...
async function writeTempFile(filename: string, content: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aera-router-clicheck-"));
...
async function writeCodexConfig(opts: {
  hasAeraRouter: boolean;
  authApiKey?: string;
}): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aera-router-codex-"));
  const configPath = path.join(tmpDir, "config.toml");

  const tomlContent = opts.hasAeraRouter
...
test("codex: returns 'configured' when TOML has Aera Router URL + valid auth key", async () => {
  const configPath = await writeCodexConfig({
    hasAeraRouter: true,
    authApiKey: "sk_aera_router_testkey_1234567890abcdef",
  });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "configured");
});

test("codex: returns 'not_configured' when TOML has Aera Router URL but auth key is masked", async () => {
  const configPath = await writeCodexConfig({
    hasAeraRouter: true,
    authApiKey: "sk_****",
  });
...
test("codex: returns 'not_configured' when TOML does not mention Aera Router", async () => {
  const configPath = await writeCodexConfig({ hasAeraRouter: false });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "not_configured");
});
...
test("hermes: returns 'configured' when config contains Aera Router", async () => {
  const configPath = await writeTempFile(
    "hermes.toml",
    `[openai]\nbase_url = "http://localhost:20128/v1"\n`
  );
...
test("droid: returns 'configured' when JSON config contains sk_aera_router marker", async () => {
  const configPath = await writeTempFile(
    "droid.json",
    JSON.stringify({ apiKey: "sk_aera_router_somekey", baseUrl: "http://localhost:20128/v1" })
  );
  const result = await checkToolConfigStatus("droid", configPath);
  assert.equal(result, "configured");
});

test("openclaw: returns 'configured' when JSON config contains aera-router text", async () => {
  const configPath = await writeTempFile(
    "openclaw.json",
    JSON.stringify({ openAiBaseUrl: "http://aera-router.local/v1", openAiApiKey: "sk-test" })
  );
...
test("kilo: returns 'not_configured' when no Aera Router markers present", async () => {
  const configPath = await writeTempFile(
    "kilo.json",
...
test("qwen: returns 'configured' only for an Aera-Router-managed model entry", async () => {
  const configPath = await writeTempFile(
    "settings.json",
    JSON.stringify({
      modelProviders: {
