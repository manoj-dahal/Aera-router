...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".aera-router") });
  } finally {
...
test("bootstrapEnv prefers ~/.aera-router/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
