...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-settings-ext-"));
...
function cleanupGlobalDb() {
  try {
    if ((globalThis as any).__aeraRouterDb?.open) {
      (globalThis as any).__aeraRouterDb.close();
    }
  } catch {}
  delete (globalThis as any).__aeraRouterDb;
}
