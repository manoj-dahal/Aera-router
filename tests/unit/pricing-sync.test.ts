...
// ─── transformToAeraRouter ────────────────────────────────

describe("transformToAeraRouter", () => {
  test("converts LiteLLM per-token pricing to Aera Router per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToAeraRouter(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToAeraRouter(raw);
