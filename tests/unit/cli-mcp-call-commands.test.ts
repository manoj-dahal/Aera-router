...
test("mcp call envia name e arguments no body", async () => {
  ...
  await (globalThis.fetch as any)("/api/mcp/tools/call", {
    method: "POST",
    body: JSON.stringify({ name: "aera_router_get_health", arguments: {} }),
  });

  globalThis.fetch = origFetch;
  assert.ok(capturedUrl.includes("/api/mcp/tools/call"));
  assert.equal(capturedBody.name, "aera_router_get_health");
...
test("mcp call com --args passa argumentos como JSON", async () => {
  ...
  await (globalThis.fetch as any)("/api/mcp/tools/call", {
    method: "POST",
    body: JSON.stringify({ name: "aera_router_check_quota", arguments: { provider: "openai" } }),
  });
...
test("mcp tools list busca /api/mcp/tools", async () => {
  const TOOLS = [
    { name: "aera_router_get_health", scopes: ["read:health"], auditLevel: "low", phase: 1 },
    { name: "aera_router_list_combos", scopes: ["read:combos"], auditLevel: "low", phase: 1 },
  ];
  const origFetch = globalThis.fetch;
