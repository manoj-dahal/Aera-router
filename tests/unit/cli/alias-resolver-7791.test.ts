 ...
 * `Cannot find package '@/shared'` when Aera Router is installed globally
 * (issue #7791).
 *
...
describe("aliasResolver.resolveAlias — @aera-router/open-sse aliases", () => {
  it("exposes ALIAS_MAP with three entries matching tsconfig paths", () => {
    ...
    // @aera-router/open-sse/ (subpath)
    assert.equal(ALIAS_MAP[1].prefix, "@aera-router/open-sse/");
    assert.equal(ALIAS_MAP[1].target, "open-sse");
    assert.equal(ALIAS_MAP[1].exact, false);
    // @aera-router/open-sse (exact package name)
    assert.equal(ALIAS_MAP[2].prefix, "@aera-router/open-sse");
    assert.equal(ALIAS_MAP[2].target, "open-sse");
    assert.equal(ALIAS_MAP[2].exact, true);
  });

  it("resolves @aera-router/open-sse (bare) to open-sse/index.ts", () => {
    const got = resolveAlias("@aera-router/open-sse", REPO_ROOT);
    assert.ok(got, "expected non-null URL for @aera-router/open-sse");
    assert.ok(got.startsWith("file://"), "must be a file URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("resolves @aera-router/open-sse/services/usage to open-sse/services/usage.ts", () => {
    const got = resolveAlias("@aera-router/open-sse/services/usage", REPO_ROOT);
  ...
  it("resolves @aera-router/open-sse/utils/proxyFetch to open-sse/utils/proxyFetch.ts", () => {
    const got = resolveAlias("@aera-router/open-sse/utils/proxyFetch", REPO_ROOT);
    assert.ok(got, "expected non-null URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("returns null for non-existent @aera-router/open-sse/* paths", () => {
    assert.equal(resolveAlias("@aera-router/open-sse/does/not/exist", REPO_ROOT), null);
  });

  it("returns null for @aera-router/other (unmatched scope)", () => {
    assert.equal(resolveAlias("@aera-router/other", REPO_ROOT), null);
    assert.equal(resolveAlias("@aera-router/other/pkg", REPO_ROOT), null);
  });

  it("rejects path-traversal via @aera-router/open-sse/../../etc/passwd", () => {
    assert.equal(resolveAlias("@aera-router/open-sse/../../etc/passwd", REPO_ROOT), null);
  });
});
...
describe("aliasResolver end-to-end (#7791 regression)", () => {
  ...
  it("resolves @/shared/network/outboundUrlGuard from a child process", () => {
    ...
    // bin/aera-router.mjs does (await import("tsx/esm") first, then register).
    const script = `
