...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "aera-router-autostart-win-"));
...
test("Windows enable/disable writes and removes VBS in Startup folder", async () => {
  ...
  const vbsPath = join(startupDir, "Aera Router.vbs");
  ...
  assert.match(vbs, /serve --no-open --tray/, "launches Aera Router tray server");
