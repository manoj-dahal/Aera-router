...
test("buildRooImport produces an openai-compatible provider profile (baseUrl /v1, model)", () => {
  ...
  const cfg = d.providerProfiles.apiConfigs["Aera Router"];
  ...
  assert.equal(d.providerProfiles.currentApiConfigName, "Aera Router");
});
...
test("buildRooVscodeAutoImport sets the pointer, preserving other settings", () => {
  const s = buildRooVscodeAutoImport(
    { "editor.tabSize": 2 },
    "/home/u/.aera-router/roo-settings.json"
  );
  assert.equal(s["editor.tabSize"], 2);
  assert.equal(s["roo-cline.autoImportSettingsPath"], "/home/u/.aera-router/roo-settings.json");
});
