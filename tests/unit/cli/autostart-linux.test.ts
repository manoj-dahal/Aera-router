...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "aera-router-autostart-linux-"));
...
test("resolveCliPath finds aera-router.mjs from argv", async () => {
  ...
  const unitPath = join(tmpDir, ".config", "systemd", "user", "aera-router.service");
  const desktopPath = join(tmpDir, ".config", "autostart", "aera-router.desktop");
  ...
  if (existsSync(unitPath)) {
    ...
    assert.match(unit, /ExecStart=.*aera-router\.mjs.*serve --no-open/m);
