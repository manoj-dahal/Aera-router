...
test("buildCodexEnv strips stale OpenAI/Codex creds from the child env (defense-in-depth)", () => {
  ...
  assert.equal(env.AERA_ROUTER_API_KEY, "oma_live_x");
...
test("buildCodexEnv uses a no-auth sentinel when no token is given", () => {
  ...
  assert.equal(env.AERA_ROUTER_API_KEY, "aera-router-no-auth");
});
...
test("buildCodexProviderArgs defines the aera-router provider inline (works without config.toml)", () => {
  const args = buildCodexProviderArgs("http://vps:20128");
  const joined = args.join(" ");
  assert.ok(joined.includes('model_provider="aera-router"'));
  assert.ok(joined.includes('model_providers.aera-router.base_url="http://vps:20128/v1"'));
  assert.ok(joined.includes('model_providers.aera-router.env_key="AERA_ROUTER_API_KEY"'));
  assert.ok(joined.includes('model_providers.aera-router.wire_api="responses"'));
  assert.ok(joined.includes("model_providers.aera-router.requires_openai_auth=false"));
