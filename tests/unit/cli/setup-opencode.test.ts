...
test("postProcessOpencodeConfig replaces the literal API key with an env ref (no secret on disk)", () => {
  ...
  assert.equal(cfg.provider["aera-router"].options.apiKey, "{env:AERA_ROUTER_API_KEY}");
  assert.equal(cfg.provider["aera-router"].options.baseURL, "http://vps:20128/v1");
});
...
test("postProcessOpencodeConfig --only filters the model map by substring", () => {
  ...
  assert.ok(cfg.provider["aera-router"].models["glm/glm-5.2"]);
  assert.ok(cfg.provider["aera-router"].models["kmc/kimi-k2.7"]);
  assert.equal("openai/gpt-4o" in cfg.provider["aera-router"].models, false);
});
...
test("postProcessOpencodeConfig preserves $schema, provider name and npm", () => {
  ...
  assert.equal(cfg.provider["aera-router"].npm, "@ai-sdk/openai-compatible");
});
