...
test("wantsProgress detects X-Aera-Router-Progress header correctly", () => {
  const headersObj = { "x-aera-router-progress": "true" };
  assert.equal(wantsProgress(new Headers(headersObj)), true);

  const headersMap = new Map([["x-aera-router-progress", "true"]]);
  assert.equal(wantsProgress(headersMap), true);

  const headersPlain = { "x-aera-router-progress": "true" };
  assert.equal(wantsProgress(headersPlain), true);

  assert.equal(wantsProgress({ "x-aera-router-progress": "false" }), false);
