...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-command-code-auth-"));
...
test("Command Code auth assist keeps auth URL callback on CLI localhost contract", async () => {
  process.env.AERA_ROUTER_PUBLIC_BASE_URL = "https://aera-router.example.com/base-path";
