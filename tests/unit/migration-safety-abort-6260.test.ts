...
//      bypass the check (AERA_ROUTER_MAX_PENDING_MIGRATIONS=0) — e.g. after
...
test(
  "abort message tells the operator to set AERA_ROUTER_MAX_PENDING_MIGRATIONS=0 to bypass (#6260)",
  serial,
  async () => {
    ...
    try {
      ...
      assert.match(message, /AERA_ROUTER_MAX_PENDING_MIGRATIONS=0/);
    } finally {
      db.close();
    }
  }
