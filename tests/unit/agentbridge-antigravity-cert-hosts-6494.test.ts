...
test("generateCert() issues a cert whose SAN list covers all 4 antigravity hosts", async (t) => {
  // Isolate DATA_DIR so this test never touches (or reuses) a real ~/.aera-router cert.
  const tmpDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-mitm-cert-6494-"));
