...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/aera-router" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/aera-router");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/aera-router",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/aera-router/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "aera-router-base",
    ...
    assert.equal(config.repoDir, "/srv/aera-router");
    assert.equal(config.composeFile, "/srv/aera-router/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "aera-router-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart aera-router --update-env \|\| true/);
  });
});
