...
test("runFabricatedDocsCheck: index contains real Aera Router routes", () => {
  const result = runFabricatedDocsCheck();
  ...
  const found = findingsFor({
    files: {
      "open-sse/config/constants.ts":
        'const t = envInt("AERA_ROUTER_CIRCUIT_BREAKER_OAUTH_THRESHOLD", 8);\n',
    },
    docs: { "cfg.md": "Override with `AERA_ROUTER_CIRCUIT_BREAKER_OAUTH_THRESHOLD`.\n" },
  });
  assert.ok(
    !found.has("env-var::AERA_ROUTER_CIRCUIT_BREAKER_OAUTH_THRESHOLD"),
...
test("cli-cmd: an arg-bearing `.command('connect <host>')` registration is NOT flagged", () => {
  ...
  const found = findingsFor({
    ...
    docs: { "guides/remote.md": "You log in once with `aera-router connect <host>`.\n" },
  });
  assert.ok(
    !found.has("cli-cmd::aera-router connect"),
...
test("ANTI-OVER-SUPPRESSION: an unregistered subcommand IS still flagged", () => {
  ...
  const found = findingsFor({
    ...
    docs: { "guides/remote.md": "Then run `aera-router teleport <host>` to finish.\n" },
  });
  assert.ok(
    found.has("cli-cmd::aera-router teleport"),
