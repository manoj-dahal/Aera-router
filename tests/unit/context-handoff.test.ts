...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-context-handoff-"));
...
test("maybeGenerateHandoff persists a structured handoff once the threshold is reached", async () => {
  ...
  assert.equal(calls[0].body._aeraRouterSkipContextRelay, true);
  assert.equal(calls[0].body._aeraRouterInternalRequest, "context-handoff");
});
