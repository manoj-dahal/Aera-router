 ...
 *
 * Aera Router schedules every rate-limited request through Bottleneck with
 ...
 *
 * The fix rewrites that specific Bottleneck error into a clear, Aera-Router-owned
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-rl-queue-timeout-"));
...
test("#4165 queue-timeout surfaces a clear Aera Router error, not the raw upstream-looking string", async () => {
