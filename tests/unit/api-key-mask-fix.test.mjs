...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_aera_router when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_aera_router");
  });

  it("falls back to sk_aera_router when both are undefined", async () => {
    mockKeyStore.clear();
