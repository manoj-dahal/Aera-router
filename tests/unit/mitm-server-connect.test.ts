...
test("C2 header contract — server.cjs intercept must inject x-aera-router-source and x-aera-router-agent", async () => {
  ...
  assert.match(
    src,
    /"x-aera-router-source":\s*"agent-bridge"/,
    'server.cjs must inject "x-aera-router-source: agent-bridge"'
  );
  assert.match(
    src,
    /"x-aera-router-agent":\s*agentId/,
    'server.cjs must inject "x-aera-router-agent: <id>" from the host→agent map'
  );
