...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-model-sync-"));
...
test("modelSyncScheduler resolves only loopback origins and uses the dashboard port", async () => {
  const previous = {
    ...
    AERA_ROUTER_BASE_PATH: process.env.AERA_ROUTER_BASE_PATH,
    AERA_ROUTER_INTERNAL_SCHEME: process.env.AERA_ROUTER_INTERNAL_SCHEME,
    AERA_ROUTER_TLS_CERT: process.env.AERA_ROUTER_TLS_CERT,
    AERA_ROUTER_TLS_KEY: process.env.AERA_ROUTER_TLS_KEY,
  };
  ...
  process.env.AERA_ROUTER_BASE_PATH = "/aera-router/";
  delete process.env.AERA_ROUTER_INTERNAL_SCHEME;
  delete process.env.AERA_ROUTER_TLS_CERT;
  delete process.env.AERA_ROUTER_TLS_KEY;

  try {
    const scheduler = await loadScheduler("trusted-loopback-origin");
    assert.equal(scheduler.getModelSyncInternalBaseUrl(), "http://127.0.0.1:22128/aera-router");
    assert.equal(
      scheduler.resolveModelSyncInternalBaseUrl("https://attacker.example/steal"),
      "http://127.0.0.1:22128/aera-router"
    );
    assert.equal(
...
test("modelSyncScheduler uses the listener-declared TLS scheme without trusting candidates", async () => {
  ...
  delete process.env.NEXT_PUBLIC_APP_URL;
  process.env.AERA_ROUTER_BASE_PATH = "/aera-router";
  process.env.AERA_ROUTER_INTERNAL_SCHEME = "https";
  delete process.env.AERA_ROUTER_TLS_CERT;
  delete process.env.AERA_ROUTER_TLS_KEY;
  ...
  try {
    ...
    assert.equal(
      ...
      "https://localhost:22128/aera-router"
    );
    assert.equal(
...
test("runtime launchers publish the actual internal listener scheme", () => {
  ...
  assert.match(runNext, /AERA_ROUTER_INTERNAL_SCHEME\s*=\s*["']http["']/);
  assert.match(standalone, /AERA_ROUTER_INTERNAL_SCHEME\s*=\s*tlsOptions\s*\?\s*["']https["']/);
});
...
test("initCloudSync skips auto initialization during build and test processes unless explicitly re-enabled", () => {
  ...
  assert.equal(
    initCloudSync.shouldSkipCloudSyncInitialization(
      {
        NODE_ENV: "test",
        AERA_ROUTER_ENABLE_RUNTIME_BACKGROUND_TASKS: "1",
      },
      ["node", "--test"]
    ),
    false
