...
// shapes (null when no cache fields), and the _aera_router meta attachment/merge.
...
test("attachLogMeta merges compact meta into _aera_router", () => {
  const out = attachLogMeta({ a: 1, _aera_router: { keep: true } }, { added: 2, drop: null });
  assert.deepEqual(out, { a: 1, _aera_router: { keep: true, added: 2 } });
});

test("attachLogMeta wraps non-object payloads", () => {
  const out = attachLogMeta(null, { added: 2 });
  assert.deepEqual(out, { _aera_router: { added: 2 }, _payload: null });
});
