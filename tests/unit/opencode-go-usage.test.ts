...
// working default quota endpoint, so AERA_ROUTER_OPENCODE_GO_QUOTA_URL must be set
...
const ORIGINAL_OPENCODE_GO_QUOTA_URL = process.env.AERA_ROUTER_OPENCODE_GO_QUOTA_URL;
process.env.AERA_ROUTER_OPENCODE_GO_QUOTA_URL = "https://api.z.ai/api/monitor/usage/quota/limit";
...
after(() => {
  if (ORIGINAL_OPENCODE_GO_QUOTA_URL === undefined) {
    delete process.env.AERA_ROUTER_OPENCODE_GO_QUOTA_URL;
  } else {
    process.env.AERA_ROUTER_OPENCODE_GO_QUOTA_URL = ORIGINAL_OPENCODE_GO_QUOTA_URL;
  }
});
