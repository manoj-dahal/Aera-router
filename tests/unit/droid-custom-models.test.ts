...
test("buildDroidCustomModels emits one entry per model with sequential ids", () => {
  ...
  assert.equal(out[0].id, "custom:Aera-Router-0");
  ...
  assert.equal(out[0].apiKey, "sk_aera_router");
  assert.equal(out[1].id, "custom:Aera-Router-1");
...
test("buildDroidCustomModels promotes activeModel to index 0", () => {
  ...
  assert.equal(out[2].id, "custom:Aera-Router-2");
...
test("isAeraRouterCustomModel matches any custom:Aera Router-<i> id (multi-model)", () => {
  assert.equal(isAeraRouterCustomModel({ id: "custom:Aera-Router-0" }), true);
  assert.equal(isAeraRouterCustomModel({ id: "custom:Aera-Router-1" }), true);
  assert.equal(isAeraRouterCustomModel({ id: "custom:Aera-Router-42" }), true);
  assert.equal(isAeraRouterCustomModel({ id: "custom:Other-0" }), false);
  assert.equal(isAeraRouterCustomModel({ id: 42 as unknown }), false);
  assert.equal(isAeraRouterCustomModel(null), false);
  assert.equal(isAeraRouterCustomModel(undefined), false);
  assert.equal(isAeraRouterCustomModel({}), false);
});
