...
async function withTempDir(fn) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aera-router-build-next-isolated-"));
...
test("resolveNextBuildEnv honors the AERA_ROUTER_BUILD_MEMORY_MB override", () => {
  const env = resolveNextBuildEnv({ AERA_ROUTER_BUILD_MEMORY_MB: "6144" });
...
test("getTransientBuildPaths only moves _tasks when explicitly enabled", () => {
  const paths = getTransientBuildPaths("/repo", { AERA_ROUTER_BUILD_MOVE_TASKS: "1" });
