...
function withEnv(value: string | undefined, fn: () => void) {
  const prev = process.env.AERA_ROUTER_SSE_COMMENTS;
  try {
    if (value === undefined) delete process.env.AERA_ROUTER_SSE_COMMENTS;
    else process.env.AERA_ROUTER_SSE_COMMENTS = value;
    fn();
  } finally {
    if (prev === undefined) delete process.env.AERA_ROUTER_SSE_COMMENTS;
    else process.env.AERA_ROUTER_SSE_COMMENTS = prev;
  }
}
...
test("createSseHeartbeatTransform suppresses COMMENT heartbeats when AERA_ROUTER_SSE_COMMENTS=off", async () => {
  const prev = process.env.AERA_ROUTER_SSE_COMMENTS;
  process.env.AERA_ROUTER_SSE_COMMENTS = "off";
  try {
