...
describe("resolveCompressionHeader", () => {
  it("reads the raw value case-insensitively and trims it", () => {
    assert.equal(
      resolveCompressionHeader({ "x-aera-router-compression": "  engine:rtk " }),
      "engine:rtk"
    );
    assert.equal(
      resolveCompressionHeader(new Headers({ "X-Aera-Router-Compression": "off" })),
      "off"
    );
  });
  ...
  it("returns null when absent or blank", () => {
    ...
    assert.equal(resolveCompressionHeader({ "x-aera-router-compression": "   " }), null);
