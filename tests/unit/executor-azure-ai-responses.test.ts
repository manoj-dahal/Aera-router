 ...
 *      providerSpecificData._aeraRouterForceResponsesUpstream === true.
...
test("DefaultExecutor.buildUrl forces /responses path when _aeraRouterForceResponsesUpstream=true", () => {
  const executor = new DefaultExecutor("azure-ai");

  const forced = executor.buildUrl("gpt-4.1", true, 0, {
    providerSpecificData: {
      baseUrl: "https://example-resource.services.ai.azure.com/openai/v1",
      _aeraRouterForceResponsesUpstream: true,
    },
  });
