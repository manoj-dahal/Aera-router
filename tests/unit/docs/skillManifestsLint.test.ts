...
// coletava tests/unit/docs/) e apodreceu — filtrava dirs `aera-router*`, mas os skills
...
// invariante de uso é "referencia as env vars ($AERA_ROUTER_URL/AERA_ROUTER_KEY) OU
// comandos da CLI (`aera-router …`)" — 3 skills (health/keys/batches) usam só a CLI.
...
test("each skill dir has SKILL.md with frontmatter", async () => {
  ...
  for (const dir of dirs) {
    ...
    if (!dir.startsWith("omni-")) {
      assert.ok(
        content.includes("AERA_ROUTER_") || content.includes("aera-router "),
        `${dir}: missing usage references (AERA_ROUTER_* env vars or aera-router CLI commands)`
      );
    }
  }
});
