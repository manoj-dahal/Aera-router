...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-chat-combo-live-"));
...
test("chat completions route emits early keepalive while waiting for stream readiness", async () => {
  ...
  assert.match(body, /data: \{"id":"aera-router-keepalive","object":"chat\.completion\.chunk"/);
