...
test.after(() => {
  fs.rmSync(TEST_DATA_DIR, { recursive: true, force: true });
  if (ORIGINAL_AERA_ROUTER_API_KEY === undefined) delete process.env.AERA_ROUTER_API_KEY;
  else process.env.AERA_ROUTER_API_KEY = ORIGINAL_AERA_ROUTER_API_KEY;
...
test("validateApiKey accepts configured environment API keys", async () => {
  process.env.AERA_ROUTER_API_KEY = "sk-env-lifecycle-test";
