...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-health-autopilot-"));
...
const accountFallback = await import("@aera-router/open-sse/services/accountFallback");
...
test("provider health autopilot action accepts LAN dashboard requests (#6277)", async () => {
  ...
  assert.equal(request.headers.get("x-aera-router-peer-ip"), null);
