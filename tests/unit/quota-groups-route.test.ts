...
test("groups/route.ts: uses buildErrorBody from @aera-router/open-sse/utils/error", () => {
  assert.ok(listSrc.includes("buildErrorBody"), "route must use buildErrorBody — Hard Rule #12");
  assert.ok(
    listSrc.includes("@aera-router/open-sse/utils/error"),
    "route must import buildErrorBody from @aera-router/open-sse/utils/error"
  );
});
...
test("groups/[id]/route.ts: uses buildErrorBody from @aera-router/open-sse/utils/error", () => {
  assert.ok(idSrc.includes("buildErrorBody"), "route must use buildErrorBody — Hard Rule #12");
  assert.ok(
    idSrc.includes("@aera-router/open-sse/utils/error"),
    "route must import buildErrorBody from @aera-router/open-sse/utils/error"
  );
});
