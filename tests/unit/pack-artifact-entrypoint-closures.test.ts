...
const BIN_ENTRY = path.join(ROOT, "bin", "aera-router.mjs");
...
test("every bin/aera-router.mjs local import is enforced by check:pack-artifact", () => {
  // The CLI boot path (bin/aera-router.mjs → bin/cli/*) is covered by allowlist PREFIXES,
