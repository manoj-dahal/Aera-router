...
test("webFetchTool has the required McpToolDefinition shape", () => {
  assert.equal(webFetchTool.name, "aera_router_web_fetch");
...
test("webFetchTool is registered in MCP_TOOLS and MCP_TOOL_MAP", () => {
  ...
  assert.ok(toolNames.includes("aera_router_web_fetch"), "webFetchTool must be in MCP_TOOLS array");
  assert.ok("aera_router_web_fetch" in MCP_TOOL_MAP, "webFetchTool must be in MCP_TOOL_MAP");
});

// ── Scope mapping ──

test("aera_router_web_fetch is mapped in MCP_TOOL_SCOPES with execute:search", () => {
  const scopes = MCP_TOOL_SCOPES["aera_router_web_fetch"];
  assert.ok(scopes != null, "aera_router_web_fetch must have a scope mapping");
  assert.ok(
    scopes.includes("execute:search"),
    "aera_router_web_fetch must require execute:search scope"
  );
});
