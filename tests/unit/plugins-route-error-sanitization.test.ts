 ...
 *   2. Import and use `buildErrorBody` from `@aera-router/open-sse/utils/error`.
 *
...
for (const { rel, label } of PLUGIN_ROUTES) {
  ...
  test(`${label}: imports buildErrorBody from @aera-router/open-sse/utils/error`, () => {
    const src = readRoute(rel);
    assert.match(
      src,
      /import \{[^}]*buildErrorBody[^}]*\} from ["']@aera-router\/open-sse\/utils\/error["']/,
      `${rel}: must import buildErrorBody from @aera-router/open-sse/utils/error`
    );
  });
