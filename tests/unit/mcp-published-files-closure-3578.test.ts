...
// #3578 — `aera-router --mcp` crashed on npm installs with ERR_MODULE_NOT_FOUND for
...
function resolveImport(fromFile: string, spec: string): string | null {
  ...
  if (spec.startsWith("@/")) base = path.join("src", spec.slice(2));
    ...
    base = path.join("open-sse", spec.slice("@aera-router/open-sse/".length));
  else if (spec === "@aera-router/open-sse") base = path.join("open-sse", "index");
