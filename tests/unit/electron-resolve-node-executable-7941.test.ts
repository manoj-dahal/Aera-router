 ...
 * Helper.app bundles from build.productName ("Aera Router"). The old code used
 * app.getName() (package.json `name` = "aera-router-desktop"); the two diverged, so it
...
const EXEC = "/Applications/Aera Router.app/Contents/MacOS/Aera Router";

describe("#7941 — resolveDarwinHelperExecutable()", () => {
  it("resolves the productName-derived framework Helper even though app.getName() would say 'aera-router-desktop'", () => {
    const frameworkHelper = path.join(
      path.dirname(EXEC),
      "..",
      "Frameworks",
      "Aera Router Helper.app",
      "Contents",
      "MacOS",
      "Aera Router Helper"
    );
  ...
  it("prefers the sibling (unsuffixed) Helper next to the main binary when present", () => {
    const sibling = path.join(path.dirname(EXEC), "Aera Router Helper");
    const result = resolveDarwinHelperExecutable({
      execPath: EXEC,
