...
test("testImportsModule matches static, dynamic and require imports of the module path", () => {
  ...
  // static import-from
  assert.equal(
    testImportsModule(`import { x } from "@aera-router/open-sse/handlers/chatCore/headers";`, frag),
    true
  );
  ...
  assert.equal(
    testImportsModule(
      `import { a } from "@aera-router/open-sse/handlers/chatCore/idempotency";`,
      frag
    ),
    false
  );
...
test("findCoverageDrift flags covering unit tests absent from tap.testFiles", () => {
  ...
  const unitTests = [
    ...
    // covers idempotency, NOT in tap -> drift
    {
      ...
      content: `import { x } from "@aera-router/open-sse/handlers/chatCore/idempotency";`,
    },
