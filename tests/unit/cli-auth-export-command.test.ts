...
function createTempDataDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-cli-auth-export-"));
}
...
test("auth export --format env emits AERA_ROUTER_<PROVIDER>_<FIELD>=<value> lines", async () => {
  await withAuthExportEnv(async (_dataDir, dbPath) => {
    process.env.STORAGE_ENCRYPTION_KEY = TEST_KEY;
    const { encryptCredential } = await import("../../bin/cli/encryption.mjs");
    ...
    assert.match(output, new RegExp(`AERA_ROUTER_OPENAI_API_KEY=${PLAINTEXT_API_KEY}`));
  });
});
