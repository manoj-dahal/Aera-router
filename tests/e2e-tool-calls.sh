...
AERA_ROUTER_URL="${AERA_ROUTER_URL:-http://localhost:20128}"
MODEL="${1:-${AERA_ROUTER_MODEL:-}}"
AUTH_TOKEN="${AERA_ROUTER_AUTH_TOKEN:-dummy}"
FAILURES=0
...
echo "=== Aera Router Tool Call Integrity E2E Test ==="
echo "URL: $AERA_ROUTER_URL | Model: $MODEL"

echo
echo "[TEST 1] Non-streaming tool call integrity..."
response=$(curl -fsS -X POST "$AERA_ROUTER_URL/v1/chat/completions" \
...
stream_output=$(curl -fsS -X POST "$AERA_ROUTER_URL/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  -d @- <<JSON
{
...
models=$(curl -fsS "$AERA_ROUTER_URL/v1/models" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
