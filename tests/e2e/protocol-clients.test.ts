...
const BASE_URL = process.env.AERA_ROUTER_BASE_URL || "http://localhost:20128";
const API_KEY = process.env.AERA_ROUTER_API_KEY || "";
  ...
  it(
    ...
    async () => {
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          AERA_ROUTER_BASE_URL: BASE_URL,
          AERA_ROUTER_API_KEY: API_KEY,
      ...
      try {
        ...
        expect(toolNames).toContain("aera_router_get_health");
        expect(toolNames).toContain("aera_router_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=aera_router_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "aera_router_get_health")).toBe(
          true
        );
      }
    },
