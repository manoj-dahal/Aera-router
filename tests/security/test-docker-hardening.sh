...
IMAGE_NAME="${1:-aera-router-local-hardened}"
CONTAINER_NAME="${2:-aera-router-hardening-test}"
HOST_PORT="${3:-20129}"
...
docker build -t "${IMAGE_NAME}" "${WORKDIR}" >/tmp/aera_router_hardening_build.log
echo "      Build done."
...
docker run -d \
  ...
  -e DATA_DIR=/app/data \
  "${IMAGE_NAME}" >/tmp/aera_router_hardening_container_id.txt
