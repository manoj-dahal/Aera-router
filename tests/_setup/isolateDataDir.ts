...
// (bin/aera-router.mjs, proxyFetch.ts, proxyDispatcher.ts) where redirecting DATA_DIR
...
// but every process resolves DATA_DIR to the SAME default (~/.aera-router) when the env
...
if (!process.env.DATA_DIR) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-test-"));
...
process.env.AERA_ROUTER_SKIP_SYSTEM_TRUST = "1";
...
process.env.AERA_ROUTER_SKIP_DNS_WRITE = "1";
