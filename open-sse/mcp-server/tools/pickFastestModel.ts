...
export async function handlePickFastestModel(args: PickFastestModelArgs) {
  ...
  try {
    ...
    await logToolCall("aera_router_pick_fastest_model", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("aera_router_pick_fastest_model", args, null, Date.now() - start, false, msg);
