...
function consumeResponsesStoreMarker(body: Record<string, unknown>): unknown {
  const marker = body._aeraRouterResponsesStore;
  delete body._aeraRouterResponsesStore;
  return marker;
}

/**
 * Global Codex WebSocket kill-switch (feature flag AERA_ROUTER_CODEX_WS_ENABLED,
...
function isCodexWsGloballyEnabled(): boolean {
  try {
    return isFeatureFlagEnabled("AERA_ROUTER_CODEX_WS_ENABLED");
  } catch {
    return true;
  }
}
...
export function isCodexResponsesWebSocketRequired(_model: string, credentials: unknown): boolean {
  ...
  // Aera Router is an HTTP→SSE gateway — WebSocket transport is unnecessary and
...
function codexDropNonstandardEvents(): boolean {
  const v = process.env.AERA_ROUTER_CODEX_DROP_NONSTANDARD_EVENTS;
...
// streaming Response when the AERA_ROUTER_CODEX_DROP_NONSTANDARD_EVENTS kill-switch is on.
...
export function encodeResponseSseEvent(raw: string): { sse: string; terminal: boolean } {
  ...
  // AERA_ROUTER_CODEX_DROP_NONSTANDARD_EVENTS (the HTTP transport is handled
...
export class CodexExecutor extends BaseExecutor {
  ...
  transformRequest(
    ...
    // These are often injected by Aera Router's fallback logic for store=true,
    ...
    const RESPONSES_API_ALLOWLIST = new Set([
      ...
      "text",
      // Internal markers used by Aera Router pipeline
      "_aeraRouterResponsesStore",
    ]);
