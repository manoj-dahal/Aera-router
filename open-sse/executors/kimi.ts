...
function resolveKimiProtocol(
  ...
  const targetFormat = credentials?.providerSpecificData?._aeraRouterKimiTargetFormat;
...
function getThinkingPolicy(credentials: ProviderCredentials): KimiThinkingPolicy {
  return (asRecord(credentials.providerSpecificData?._aeraRouterKimiThinking) ||
