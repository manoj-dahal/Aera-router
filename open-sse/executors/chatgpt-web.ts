...
// Aera Router model ID → ChatGPT internal slug. The public ChatGPT Web catalog
// keeps Aera Router's historical dot-form IDs (e.g. "gpt-5.5-pro"), while
...
function configuredProPollTimeoutMs(): number {
  const raw = Number(process.env.AERA_ROUTER_CGPT_WEB_PRO_TIMEOUT_MS);
  if (!Number.isFinite(raw) || raw <= 0) return DEFAULT_PRO_POLL_TIMEOUT_MS;
  return Math.floor(raw);
}

function configuredProPollIntervalMs(): number {
  const raw = Number(process.env.AERA_ROUTER_CGPT_WEB_PRO_POLL_INTERVAL_MS);
 ...
 * dozen "Aera Router" entries clutter the sidebar and can interact with
 ...
 * chat but returned to Aera Router as a bare "completed without image markdown".
 */
 ...
 * often reach Aera Router from a container while the user's browser needs a
...
function derivePublicBaseUrl(
  ...
  const explicitPublicBase = normalizePublicBaseUrl(process.env.AERA_ROUTER_PUBLIC_BASE_URL);
  if (explicitPublicBase) {
    log?.debug?.("CGPT-WEB", `derivePublicBaseUrl: using AERA_ROUTER_PUBLIC_BASE_URL`);
    return explicitPublicBase;
  }

  const headerBase = deriveHeaderBaseUrl(clientHeaders);
  const configuredBase =
    normalizePublicBaseUrl(process.env.AERA_ROUTER_BASE_URL) ||
...
/**
 * Download a chatgpt.com signed image URL and re-serve it from Aera Router's
 ...
 * and return a browser-fetchable Aera Router URL.
 */
...
function configuredAsyncImageTimeoutMs(): number {
  const raw = Number(process.env.AERA_ROUTER_CGPT_WEB_IMAGE_TIMEOUT_MS);
...
function makeImageResolver(ctx: ResolverContext): ImageResolver {
  ...
  return async (assetPointer, conversationId, parentMessageId) => {
    ...
    if (fileId) {
      ...
      // Aera Router's short-lived image cache.
      //
    ...
    if (signedUrl) {
      ...
      // so we materialize the bytes into our own cache and emit an Aera Router
