...
function buildHeartbeatPayload(
  ...
  switch (shape) {
    ...
    case HEARTBEAT_SHAPES.OPENAI_CHUNK: {
      const payload = {
        id: opts.chunkId ?? "aera-router-keepalive",
        object: "chat.completion.chunk",
        created: Math.floor(Date.now() / 1000),
        model: opts.chunkModel ?? "aera-router",
...
/**
 * Whether Aera Router may emit SSE `:` comment lines (e.g. the `: keepalive` heartbeat).
 * Some strict OpenAI-compatible clients parse every SSE line as JSON and crash on `:` comments.
 * Set AERA_ROUTER_SSE_COMMENTS=off to suppress comment-shaped heartbeats (they become a no-op).
...
export function sseCommentsEnabled(): boolean {
  ...
  const v = process.env.AERA_ROUTER_SSE_COMMENTS;
...
export function createSseHeartbeatTransform({
  ...
  // crash on `:` comment heartbeats. AERA_ROUTER_SSE_COMMENTS=off disables comment-shaped
