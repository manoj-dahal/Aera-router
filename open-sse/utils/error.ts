 ...
 * `x-aera-router-combo-*` headers and a `diagnostics` field in the OpenAI-shaped
 ...
 * carries a `recovery` hint it is mirrored as `x-aera-router-recovery-action` /
 * `x-aera-router-recovery-next-step` / `x-aera-router-retry-after-seconds` headers and as a
...
export function errorResponseWithComboDiagnostics(
  ...
  const headers: Record<string, string> = {
    ...
    "x-aera-router-combo-pool-size": String(safe.poolSize),
    "x-aera-router-combo-attempted": String(safe.attempted),
    "x-aera-router-combo-excluded": excludedHeader,
    "x-aera-router-combo-terminal-reason": toHeaderSafeAscii(safe.terminalReason.slice(0, 200)),
  };

  if (safe.recovery) {
    headers["x-aera-router-recovery-action"] = safe.recovery.action;
    ...
    headers["x-aera-router-recovery-next-step"] = toHeaderSafeAscii(safe.recovery.next_step).slice(
      0,
      128
    );
    if (
      ...
      headers["x-aera-router-retry-after-seconds"] = String(safe.recovery.retry_after_seconds);
    }
  }
  ...
  return new Response(
    ...
    }),
    {
      ...
      headers: {
        "Content-Type": "application/json",
        "Retry-After": String(retryAfterSec),
        "X-Aera-Router-Provider-Breaker": "open",
      },
    }
  );
}
