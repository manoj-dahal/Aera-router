 ...
 * `stream`. Without this hint, Aera Router previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // Aera Router accumulates the provider stream and converts it to JSON for the client
  ...
  // Aera Router fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve Aera Router's streaming default unless
