export const PROVIDER_PLUGIN_MANIFEST_HEADER = "X-Aera-Router-Provider-Manifest-Url";
...
export function resolveProviderPluginManifestUrl(origin?: string | null): string {
  const configured = process.env.AERA_ROUTER_PROVIDER_MANIFEST_URL?.trim();
  ...
  const protocol = process.env.AERA_ROUTER_PUBLIC_PROTOCOL || "http";
