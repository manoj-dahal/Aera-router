...
/**
 * Handler for the `aera_router_ccr_retrieve` MCP tool.
 *
 ...
 * ENTIRE prompt for any caller that cannot resolve `aera_router_ccr_retrieve`
...
function processMessages(
  ...
  const result = messages.map((msg) => {
    ...
    // Aera Router is used as a chat-completion PROVIDER, the upstream LLM has no
    // way to call `aera_router_ccr_retrieve` and expand markers — replacing tool
...
export const ccrEngine: CompressionEngine = {
  ...
  description:
    ...
    "retrievable via the `aera_router_ccr_retrieve` MCP tool (H4). " +
