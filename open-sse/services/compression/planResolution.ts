...
/**
 * Interprets the `x-aera-router-compression` request header into a plan, or null when the
...
/** Renders the X-Aera-Router-Compression response header value. */
export function formatCompressionMeta(plan: DerivedPlan): string {
  return `${plan.mode}; source=${plan.source ?? "off"}`;
}

/**
 * Builds the annotation suffix for X-Aera-Router-Compression from compression stats.
 ...
 * ASCII-only: this string is appended to the X-Aera-Router-Compression HTTP header,
