 ...
 *
 * Goal: ensure the final request body Aera Router sends to Anthropic's
 ...
 *
 * Related: Aera Router issue #2260.
 */
...
export interface InjectBillingHeaderOp {
   ...
   *   - aera-router-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "aera-router-daystamp";
  /**
 ...
 * proven against the live Aera Router deployment (call log
...
/**
 * Compute the `cc_version` suffix per the Aera Router native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    options.versionFormat === "aera-router-daystamp"
