 ...
 * startup cost. Set AERA_ROUTER_BROWSER_POOL=off to fully disable.
 */
...
// (and the aera_router_browser_pool_status MCP tool), giving the previously
...
function isPoolEnabled(): boolean {
  const flag = process.env.AERA_ROUTER_BROWSER_POOL;
...
export async function acquireBrowserContext(
  ...
  if (!isPoolEnabled()) {
    throw new Error(
      "browserPool: AERA_ROUTER_BROWSER_POOL=off — context requested but pool is disabled"
    );
  }
 ...
 * failures, shutdowns). Surfaced via the aera_router_browser_pool_status MCP tool.
 */
