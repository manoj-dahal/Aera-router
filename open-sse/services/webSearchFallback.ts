...
export const AERA_ROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME = "aera_router_web_search";
...
function buildFallbackTool(tool: JsonRecord, targetFormat?: string | null): JsonRecord {
  const name = AERA_ROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME;
...
// built-in web-search tool has to be converted to the aera_router_web_search function
...
export function supportsNativeWebSearchFallbackBypass({
  ...
  // native tool untouched instead of rewriting it to aera_router_web_search. Mirrors the
...
export function prepareWebSearchFallbackBody<T extends JsonRecord>(
  ...
  if (!toolNames.has(AERA_ROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    ...
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: AERA_ROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: AERA_ROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
