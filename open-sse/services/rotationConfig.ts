 ...
 *
 * Aera Router's account-fallback engine (accountFallback.ts) historically rotated accounts using
 ...
 *   - master enable                            AERA_ROUTER_ROTATION_ENABLED               (default true)
 *   - rate-limit reset/cooldown seconds        AERA_ROUTER_ROTATION_RATE_LIMIT_RESET_SECONDS (0 => engine default)
 *   - per-status fallback enable               AERA_ROUTER_ROTATE_ON_{429,500,502,400}    (429/500/502 default true, 400 default false)
 *   - per-status threshold (errors in window)  AERA_ROUTER_ROTATE_{status}_THRESHOLD      (default 1 => immediate, current behavior)
 *   - per-status window seconds                AERA_ROUTER_ROTATE_{status}_WINDOW_SECONDS (default 120)
 *
...
const GLOBAL_KEY = "__aera_router_rotation_config__";
...
function buildFromEnv(): RotationConfig {
  return {
    enabled: envBool("AERA_ROUTER_ROTATION_ENABLED", true),
    rateLimitResetMs: envInt("AERA_ROUTER_ROTATION_RATE_LIMIT_RESET_SECONDS", 0, 0) * 1000,
    disableTagWithoutReset: envBool("AERA_ROUTER_ROTATION_DISABLE_TAG_WITHOUT_RESET", true),
    rateLimit429: buildClass(
      "AERA_ROUTER_ROTATE_ON_429",
      "AERA_ROUTER_ROTATE_429_THRESHOLD",
      "AERA_ROUTER_ROTATE_429_WINDOW_SECONDS",
      true
    ),
    serverError500: buildClass(
      "AERA_ROUTER_ROTATE_ON_500",
      "AERA_ROUTER_ROTATE_500_THRESHOLD",
      "AERA_ROUTER_ROTATE_500_WINDOW_SECONDS",
      true
    ),
    badGateway502: buildClass(
      "AERA_ROUTER_ROTATE_ON_502",
      "AERA_ROUTER_ROTATE_502_THRESHOLD",
      "AERA_ROUTER_ROTATE_502_WINDOW_SECONDS",
      true
    ),
    badRequest400: buildClass(
      "AERA_ROUTER_ROTATE_ON_400",
      "AERA_ROUTER_ROTATE_400_THRESHOLD",
      "AERA_ROUTER_ROTATE_400_WINDOW_SECONDS",
      false
    ),
  };
}
...
const COUNTER_KEY = "__aera_router_rotation_counters__";
