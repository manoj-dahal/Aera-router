 ...
 *
 * Reference: Aera Router issue #2260 + comment 4459544580 (Open WebUI bypass).
 */
 ...
 * verified against opencode→Aera Router→Anthropic with claude-opus-4-7 OAuth.
 */
...
export const DEFAULT_SYSTEM_TRANSFORMS_CONFIG: SystemTransformsConfig = {
  providers: {
    [PROVIDER_CLAUDE]: {
      ...
      // usage` when opencode (or any non-claude-cli client) hits Aera Router's
 ...
 *
 * `providerId` matches Aera Router's provider key (`claude`,
...
const GLOBAL_KEY = "__aera_router_systemTransforms_config__";
