...
// #1731v2 guard: a provider circuit-breaker-open response (503 + `X-Aera-Router-Provider-Breaker`
// header / `provider_circuit_open` error code, see providerCircuitOpenResponse) is an Aera Router
...
export function isProviderCircuitOpenResult(
  ...
  const breakerHeader = result.headers?.get?.("x-aera-router-provider-breaker");
