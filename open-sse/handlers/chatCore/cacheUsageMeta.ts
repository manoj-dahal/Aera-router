 ...
 * shapes), and attach an `_aera_router` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _aera_router: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._aera_router &&
    typeof payload._aera_router === "object" &&
    !Array.isArray(payload._aera_router)
      ? payload._aera_router
      : {};
  return {
    ...payload,
