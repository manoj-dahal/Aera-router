...
const contentSecurityPolicy = [
  ...
  // Aera Router is reached from a non-loopback host. Same-origin HTTP fetches stay covered
...
// AERA_ROUTER_BUILD_PROFILE=minimal physically removes four optional privileged
...
// The resulting artifact is intended to be published as `aera-router-secure`
// for security-sensitive environments. See docs/security/SOCKET_DEV_FINDINGS.md.
const isMinimalBuild = process.env.AERA_ROUTER_BUILD_PROFILE === "minimal";
...
const nextConfig = {
  ...
  // Aera Router under https://host/aera-router/). Empty by default so root-path
  ...
  basePath: process.env.AERA_ROUTER_BASE_PATH || "",
  distDir,
  ...
  turbopack: {
    ...
    resolveAlias: {
      ...
      // (Docker sets AERA_ROUTER_MITM_STUB=1 — #3390 graceful degradation). The
  ...
  // Aera Router is a proxy for AI APIs — request bodies routinely include
  ...
  experimental: {
    serverActions: {
      bodySizeLimit: process.env.AERA_ROUTER_SERVER_ACTIONS_BODY_LIMIT || "50mb",
    },
    ...
    // Next's internal router proxy defaults to 30s when this is unset. Aera Router
    ...
    // PR-2 of manoj-dahal/Aera-router#3932: tree-shake barrel re-exports so
    ...
    // the internal `@aera-router/open-sse` workspace here: optimizePackageImports
  ...
  transpilePackages: ["@aera-router/open-sse", "@lobehub/icons", "fumadocs-ui", "fumadocs-core"],
  ...
  webpack(config, { webpack }) {
    ...
    config.optimization.splitChunks = {
      ...
      cacheGroups: {
        ...
        // PR-2 of manoj-dahal/Aera-router#3932: isolate the heavy long-tail
  ...
  async headers() {
    return [
      ...
      // G-10: allow Aera Router's own dashboard to embed the 9Router UI via our reverse proxy.
  ...
  async rewrites() {
    return [
      ...
      // so they hit the /api/[...aeraRouterApiCatchAll] route (#6424) — which
