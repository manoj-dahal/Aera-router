# bin/_ops-common.sh — shared helpers for the Aera Router ops runbook scripts.
#
...
ops_set_data_dir() {
  AERA_ROUTER_DATA_DIR="$1"
  AERA_ROUTER_SQLITE="${AERA_ROUTER_DATA_DIR}/storage.sqlite"
  AERA_ROUTER_BACKUPS_DIR="${DB_BACKUPS_DIR:-${AERA_ROUTER_DATA_DIR}/db_backups}"
}
ops_set_data_dir "${DATA_DIR:-$HOME/.aera-router}"
...
ops_find_snapshot() {
  ...
  for cand in \
    "$id" \
    "$id/" \
    "$AERA_ROUTER_BACKUPS_DIR/$id" \
    "$AERA_ROUTER_BACKUPS_DIR/snapshot_$id"; do
  ...
  if [ -d "$AERA_ROUTER_BACKUPS_DIR" ]; then
    for cand in "$AERA_ROUTER_BACKUPS_DIR"/snapshot_*"$id"*; do
      [ -f "$cand/storage.sqlite" ] && { printf '%s\n' "$cand"; return 0; }
    done
  fi
  ops_die "no snapshot matching '$id' under $AERA_ROUTER_BACKUPS_DIR (run bin/snapshot-data.sh first)"
}
