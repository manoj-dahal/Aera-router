...
const AERA_ROUTER_IPC_PORT_BASE = 29128;

export function initWinTray({ port, onQuit, onOpenDashboard, onShowLogs }) {
  if (process.platform !== "win32") return null;

  const ipcPort = AERA_ROUTER_IPC_PORT_BASE + (port % 1000);
  const scriptPath = join(tmpdir(), `aera-router-tray-${process.pid}.ps1`);
...
$tray.Text = "Aera Router - Port ${port}"
...
$mQuit = $menu.Items.Add("Quit Aera Router")
