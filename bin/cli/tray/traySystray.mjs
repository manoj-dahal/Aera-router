...
// ~/.aera-router/runtime by trayRuntime.ts (loadSystray). The previous inline
...
export async function initSystrayUnix(
  ...
  const items = [
    { title: `Aera Router  •  port ${port}`, tooltip: "Server running", enabled: false },
    ...
    { title: "Quit Aera Router", enabled: true },
  ];
  ...
  tray.ready().catch((err) => {
    process.stderr.write(`[aera-router][tray] systray2 failed: ${err?.message ?? String(err)}\n`);
  });
