/**
 * aera-router setup-claude — Remote-aware Claude Code profile generator.
 *
 ...
 * fetches the live /v1/models catalog from a (possibly remote) Aera Router and
 ...
 *
 * Launch a profile with:  aera-router launch --profile <name>
...
export function buildProfileSettings(modelId, baseUrl, cfg) {
  ...
  // NOTE: ANTHROPIC_AUTH_TOKEN is intentionally NOT written here — `aera-router
...
export async function runSetupClaudeCommand(opts = {}) {
  ...
  printHeading("Aera Router → Claude Code profile generator");
  ...
  if (!dryRun) {
    ...
    console.log("\nTo use a profile:");
    console.log(
      "  aera-router launch --profile <name>     # e.g. aera-router launch --profile glm52"
    );
    console.log(
