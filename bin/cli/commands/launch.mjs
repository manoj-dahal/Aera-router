...
/**
 * Build a clean child env for Claude Code pointed at Aera Router.
 *
...
export function buildClaudeEnv(baseEnv, baseUrlOrPort, authToken, opts = {}) {
  ...
  // from stopping at its local login gate before it ever contacts Aera Router (an
  ...
  env.ANTHROPIC_AUTH_TOKEN = (authToken && String(authToken).trim()) || "aera-router-no-auth";
...
/**
 * Resolve the Aera Router base URL + auth for launch, honouring (in order):
...
export function resolveLaunchTarget(opts = {}) {
  ...
  if (!authToken) authToken = process.env.ANTHROPIC_AUTH_TOKEN ?? process.env.AERA_ROUTER_API_KEY;
...
export function registerLaunch(program) {
  program
    .command("launch")
    .description(
      t("launch.description") || "Launch Claude Code pointed at Aera Router (local or remote)"
    )
    .option("--port <port>", t("serve.port") || "Proxy port", "20128")
    .option(
      "--remote <url>",
      "Remote Aera Router base URL (overrides --port and the active context)"
    )
    .option(
      "--profile <name>",
    ...
    .option("--api-key <key>", "Alias for --token (Aera Router access token / API key)")
