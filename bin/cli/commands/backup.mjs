...
export function registerBackup(program) {
  ...
  // Legacy: `aera-router backup` without subcommand still creates a backup
...
async function pruneBackups(backupDir, retention) {
  ...
  try {
    const dirs = readdirSync(backupDir)
      .filter((f) => f.startsWith("aera-router-backup-"))
      .sort()
      .reverse();
...
export async function runBackupCommand(opts = {}) {
  ...
  const backupName = safeName
    ? `aera-router-backup-${safeName}`
    : `aera-router-backup-${timestamp}`;
...
async function _uploadBackupToCloud(backupPath, info) {
  ...
  try {
    const boundary = `aera-router-backup-${Date.now().toString(36)}-${randomBytes(8).toString("hex")}`;
    const headers = new Headers({
      accept: "application/json",
      "content-type": `multipart/form-data; boundary=${boundary}`,
    });
    const apiKey = process.env.AERA_ROUTER_API_KEY;
    if (apiKey) headers.set("authorization", `Bearer ${apiKey}`);
    const cliToken = process.env.AERA_ROUTER_CLI_TOKEN ?? (await getCliToken());
...
export async function runRestoreCommand(backupId, opts = {}) {
  ...
  if (opts.list || !backupId) {
    ...
    if (!backupId) console.log("\nUsage: aera-router restore <backup-id>");
    return 0;
  }

  const safeBackupId = String(backupId).replace(/[/\\]/g, "_");
  const backupPath = join(backupDir, `aera-router-backup-${safeBackupId}`);
