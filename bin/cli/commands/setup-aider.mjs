/**
 * aera-router setup-aider — configure Aider (aider.chat) for Aera Router.
 *
...
export async function runSetupAiderCommand(opts = {}) {
  ...
  printHeading("Aera Router → Aider (openai-compatible via LiteLLM)");
...
export function registerSetupAider(program) {
  program
    .command("setup-aider")
    .description("Configure Aider for Aera Router: write ~/.aider.conf.yml + print the env recipe")
