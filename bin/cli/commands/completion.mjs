...
function readCache() {
  try {
    ...
    if (process.env.AERA_ROUTER_DEBUG_COMPLETION) {
      console.error("[aera-router completion] readCache failed:", err?.message ?? err);
    }
  }
  return null;
}
...
async function refreshCache(opts = {}) {
  ...
  try {
    ...
    if (process.env.AERA_ROUTER_DEBUG_COMPLETION) {
      console.error("[aera-router completion] refreshCache failed:", err?.message ?? err);
    }
  }
  ...
  try {
    ...
    if (process.env.AERA_ROUTER_DEBUG_COMPLETION) {
      console.error("[aera-router completion] writeCache failed:", err?.message ?? err);
    }
  }
  return data;
}
...
function installPath(shell) {
  const home = homedir();
  if (shell === "zsh") return join(home, ".zsh", "completions", "_aera_router");
  if (shell === "fish") return join(home, ".config", "fish", "completions", "aera-router.fish");
  return join(home, ".bash_completion.d", "aera-router");
}

function generateZshScript() {
  return `#compdef aera-router

# Aera Router zsh completion (dynamic)
_aera_router_get_cache() {
  local key="$1"
  local cache="$HOME/.aera-router/completion-cache.json"
  ...
  if [[ $((now - mtime)) -gt 3600 ]]; then
    aera-router completion refresh --quiet >/dev/null 2>&1
  fi
...
_aera_router() {
  local -a commands
  commands=(
    'serve:Start the Aera Router server'
    'stop:Stop the server'
    'restart:Restart the server'
    'setup:Configure Aera Router'
  ...
  case $state in
    ...
    arg)
      case $words[1] in
        combo)
          case $words[2] in
            switch|delete|show)
              local -a combos
              combos=($(_aera_router_get_cache combos))
        ...
        providers|keys)
          case $words[2] in
            add|remove|test)
              local -a providers
              providers=($(_aera_router_get_cache providers))
      ...
      case $state in
        models)
          local -a models
          models=($(_aera_router_get_cache models))
...
compdef _aera_router aera-router
`;
}

function generateBashScript() {
  return `#!/bin/bash
# Aera Router CLI bash completion (dynamic)
...
complete -F _aera_router aera-router
`;
}

function generateFishScript() {
  return `# Aera Router CLI fish completion (dynamic)
complete -c aera-router -f
...
for cmd in $commands
  complete -c aera-router -n '__fish_is_nth_token 1' -a $cmd
end

# Subcommands
complete -c aera-router -n '__fish_seen_subcommand_from combo' -a 'list switch create delete show suggest'
complete -c aera-router -n '__fish_seen_subcommand_from keys' -a 'add list remove regenerate revoke reveal usage'
complete -c aera-router -n '__fish_seen_subcommand_from providers' -a 'available list test test-all'
complete -c aera-router -n '__fish_seen_subcommand_from config' -a 'list get set validate contexts'
complete -c aera-router -n '__fish_seen_subcommand_from completion' -a 'zsh bash fish install refresh'
complete -c aera-router -n '__fish_seen_subcommand_from open' -a 'combos providers api-manager cli-tools agents settings logs memory skills evals audit cost resilience'

# Dynamic completions from cache (requires python3)
function __aera_router_cache_get
  set -l key $argv[1]
  set -l cache "$HOME/.aera-router/completion-cache.json"
...
complete -c aera-router -n '__fish_seen_subcommand_from combo; and __fish_seen_subcommand_from switch delete' -a '(__aera_router_cache_get combos)'
complete -c aera-router -l model -a '(__aera_router_cache_get models)'
complete -c aera-router -l combo -a '(__aera_router_cache_get combos)'
`;
}
...
export function registerCompletion(program) {
  ...
  // Backward-compat: `aera-router completion <shell>` (positional arg form)
  comp
    .command("<shell>")
