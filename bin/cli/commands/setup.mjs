...
async function resolveProviderInput(opts, prompt, nonInteractive) {
  ...
  if (!apiKey) {
    throw new Error("Provider API key is required. Pass --api-key or AERA_ROUTER_API_KEY.");
  }
...
export function registerSetup(program) {
  ...
  // Wire up `aera-router setup opencode` subcommand. Kept inside registerSetup
...
export async function runSetupCommand(opts = {}) {
  ...
  try {
    printHeading("Aera Router Setup");
    const { db, dbPath } = await openAeraRouterDb();
