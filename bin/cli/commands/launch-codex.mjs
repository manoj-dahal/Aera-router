 ...
 *  in the shell can't shadow the aera-router provider (defense-in-depth). Mirrors
...
const NO_AUTH_SENTINEL = "aera-router-no-auth";
...
/**
 * Resolve the Aera Router root base URL + auth for codex, honouring (in order):
...
export function resolveCodexTarget(opts = {}) {
  ...
  if (explicit) {
    ...
    let fromCtx;
    try {
      fromCtx = resolveActiveContext(opts.context ?? process.env.AERA_ROUTER_CONTEXT)?.baseUrl;
    } catch {
      /* no context */
    }
    baseUrl = fromCtx
  ...
  if (!authToken) authToken = process.env.AERA_ROUTER_API_KEY;
  return { baseUrl, authToken };
}

/** Health-check an Aera Router root URL before launching Codex. */
 ...
 * AERA_ROUTER_API_KEY (the provider env_key) to the resolved token or a sentinel.
...
export function buildCodexEnv(baseEnv, authToken) {
  ...
  env.AERA_ROUTER_API_KEY = (authToken && String(authToken).trim()) || NO_AUTH_SENTINEL;
  return env;
}

/**
 * Codex `-c` flags that define the `aera-router` provider inline, so launch works
 * WITHOUT a pre-existing ~/.codex/config.toml. Mirrors free-claude-code.
 * @param {string} baseUrl  Aera Router root URL (no /v1)
 * @returns {string[]}
 */
export function buildCodexProviderArgs(baseUrl) {
  return [
    "-c",
    tomlAssign("model_provider", "aera-router"),
    "-c",
    tomlAssign("model_providers.aera-router.name", "Aera Router"),
    "-c",
    tomlAssign("model_providers.aera-router.base_url", `${baseUrl}/v1`),
    "-c",
    tomlAssign("model_providers.aera-router.env_key", "AERA_ROUTER_API_KEY"),
    "-c",
    tomlAssign("model_providers.aera-router.wire_api", "responses"),
    "-c",
    tomlAssign("model_providers.aera-router.requires_openai_auth", false),
  ];
}
...
export async function runLaunchCodexCommand(opts = {}, codexArgs = []) {
  ...
  if (!(await healthCheck(baseUrl))) {
    console.error(
      (
        t("launch.notRunning") ||
        "Aera Router is not reachable at {port}. Start it with 'aera-router serve'."
  ...
  program
    ...
    .description(
      t("launchCodex.description") ||
        "Launch Codex CLI pointed at Aera Router (local or remote VPS)"
    )
    .option("--port <port>", "Local Aera Router port (ignored when --remote is set)", "20128")
    .option(
      "--remote <url>",
      "Remote Aera Router base URL, e.g. http://192.168.0.15:20128 (overrides --port + context)"
    )
    .option("--profile <name>", "Codex profile to activate (passed as --profile <name>)")
    .option("-p, --p <name>", "Alias for --profile")
    .option(
      "--api-key <key>",
      "Aera Router API key (overrides AERA_ROUTER_API_KEY env var for this invocation)"
    )
