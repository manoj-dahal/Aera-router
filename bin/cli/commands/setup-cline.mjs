/**
 * aera-router setup-cline — configure the Cline AI coding agent to use Aera Router.
 *
 ...
 * writes the CLI-mode files (matching the Aera Router dashboard) AND prints the
...
/** Merge Aera Router openai-compatible settings into Cline's globalState (Plan + Act). */
...
export function buildClineSecrets(existing, { apiKey }) {
  return { ...(existing || {}), openAiApiKey: apiKey || "sk_aera_router" };
}
...
export async function runSetupClineCommand(opts = {}) {
  ...
  printHeading("Aera Router → Cline (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(
      `\n── [dry-run] ${secPath} ── (openAiApiKey: ${apiKey ? "set" : "sk_aera_router"})`
    );
  } else {
...
export function registerSetupCline(program) {
  program
    .command("setup-cline")
    .description(
      "Configure Cline for Aera Router: write ~/.cline/data (CLI mode) + print VS Code extension settings"
    )
