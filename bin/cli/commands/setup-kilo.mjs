/**
 * aera-router setup-kilo — configure Kilo Code to use Aera Router.
 *
 ...
 * This writes BOTH (matching the Aera Router dashboard) and prints the UI settings.
 *
...
/** Merge the Aera Router openai-compatible provider into Kilo's CLI auth.json. */
...
export async function runSetupKiloCommand(opts = {}) {
  ...
  printHeading("Aera Router → Kilo Code (OpenAI-compatible)");
      ...
      JSON.stringify(
        {
          "openai-compatible": {
            ...auth["openai-compatible"],
            apiKey: apiKey ? "set" : "sk_aera_router",
          },
        },
        null,
        2
...
export function registerSetupKilo(program) {
  program
    .command("setup-kilo")
    .description(
      "Configure Kilo Code for Aera Router: write ~/.local/share/kilo/auth.json (CLI) + VS Code kilocode.* settings"
    )
