/**
 * aera-router setup-codex — Remote-aware Codex CLI profile generator.
 *
 * Connects to a running Aera Router instance (local or remote VPS), fetches the
 ...
 *   aera-router setup-codex --remote http://100.67.86.91:20128 --api-key sk-xxx
 *
...
export async function runSetupCodexCommand(opts = {}) {
  ...
  const apiKey = opts.apiKey ?? opts["api-key"] ?? process.env.AERA_ROUTER_API_KEY ?? "";
  const codexHome = opts.codexHome ?? opts["codex-home"] ?? join(os.homedir(), ".codex");
  const dryRun = Boolean(opts.dryRun ?? opts["dry-run"]);
  const onlyFilter = opts.only ? opts.only.split(",").map((s) => s.trim()) : null;

  printHeading(`Aera Router → Codex CLI profile generator`);
  ...
  try {
    ...
    printInfo(
      "Make sure Aera Router is running and the --remote URL is correct.\n" +
        "You may also need --api-key if Aera Router requires authentication."
    );
    return 1;
  }
...
export function registerSetupCodex(program) {
  program
    .command("setup-codex")
    .description(
      "Fetch the live model catalog from Aera Router (local or remote VPS) and generate " +
    ...
    .option(
      "--remote <url>",
      "Remote Aera Router URL, e.g. http://100.67.86.91:20128 — fetches models from there"
    )
    .option(
      "--api-key <key>",
      "Aera Router API key for the remote instance (defaults to AERA_ROUTER_API_KEY env var)"
    )
