# bin/cli — Aera Router CLI internals

This directory contains the CLI runtime, helpers, and commands for the `aera-router` binary.
...
├── sqlite.mjs              ← openAeraRouterDb() — DB bootstrap
...
All HTTP calls to the Aera Router server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `AERA_ROUTER_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `AERA_ROUTER_API_KEY`)
...
Locale detection order: `AERA_ROUTER_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `AERA_ROUTER_LANG` environment variable
...
```bash
aera-router config lang set pt-BR       # saves to ~/.aera-router/.env
aera-router config lang list            # show all 42 available locales
aera-router config lang get             # show currently active locale
```

**One-time override:**

```bash
aera-router --lang de providers list    # run in German, not persisted
AERA_ROUTER_LANG=ja aera-router status    # same effect via env
```
