...
const RUNTIME_DIR = join(homedir(), ".aera-router", "runtime");
...
export async function loadSqliteRuntime() {
  ...
  try {
    ...
    console.warn(`[aera-router] runtime install failed: ${err.message}`);
  }
  ...
  if (existsSync(buildDir)) {
    ...
    if (nodeFile) {
      ...
      if (!magic || (magic !== expected && magic !== "macho-le" && magic !== "macho-fat")) {
        console.warn(
          `[aera-router] runtime sqlite binary magic mismatch (${magic} ≠ ${expected}) — skipping`
        );
        return null;
      }
    }
...
function ensureRuntimeDir() {
  ...
  if (!existsSync(pkg)) {
    writeFileSync(
      pkg,
      JSON.stringify({ name: "aera-router-runtime", private: true, type: "commonjs" }),
      "utf-8"
    );
  }
}
