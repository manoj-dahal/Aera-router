 ...
 *
 * Purely informational invocations must NOT create `~/.aera-router/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `aera-router --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `aera-router` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `aera-router` runs the default `serve` command → must provision.
