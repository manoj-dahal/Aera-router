...
export function getBaseUrl(opts = {}) {
  ...
  const envUrl = process.env.AERA_ROUTER_BASE_URL;
  if (envUrl) return stripTrailingSlash(envUrl);

  // Resolve from the active context (canonical store + legacy profile fallback).
  // This is what makes "remote mode" work: `aera-router contexts use <remote>`
...
export async function buildHeaders(opts) {
  ...
  //
  // The active context's scoped token MUST win over the ambient AERA_ROUTER_API_KEY:
  // `aera-router connect <remote>` saves the context's token, but users keep
  // AERA_ROUTER_API_KEY in their shell. The global `--api-key` option is bound to
  // that env var (.env("AERA_ROUTER_API_KEY")), so commands that spread
  ...
  const ambientKey = process.env.AERA_ROUTER_API_KEY || null;
  ...
  const cliToken = opts.cliToken ?? process.env.AERA_ROUTER_CLI_TOKEN ?? (await getCliToken());
...
export async function apiFetch(path, opts = {}) {
  ...
  const timeout =
    opts.timeout ?? (Number.parseInt(process.env.AERA_ROUTER_HTTP_TIMEOUT_MS || "", 10) || 30000);
  const maxAttempts = opts.retry === false ? 1 : (opts.retryMax ?? RETRY_DEFAULTS.maxAttempts);
  const verbose = opts.verbose ?? process.env.AERA_ROUTER_VERBOSE === "1";
