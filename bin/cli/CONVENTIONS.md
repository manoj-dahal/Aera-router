# Aera Router CLI — Internal Conventions
...
```
aera-router keys add openai sk-xxx
aera-router combo switch fastest
aera-router memory search "react hooks"
```

**Not allowed**:

```
aera-router --add-key openai sk-xxx     # ❌ flag-as-verb
aera-router add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `AERA_ROUTER_BASE_URL` env or `~/.aera-router/config.json`
  (active profile).
- Injects `Authorization: Bearer ${AERA_ROUTER_API_KEY}` when available.
- Injects `x-aera-router-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `AERA_ROUTER_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `AERA_ROUTER_LANG` to `~/.aera-router/.env`.
