#!/usr/bin/env bash
# bin/rollback.sh — roll Aera Router back to a previous release to mitigate a bad
# deploy. Part of the deploy-rollback incident-recovery flow.
#
# Methods (auto-detected; override with --method):
#   • npm    — `npm install -g aera-router@<version>` and, if PM2 manages it,
#              `pm2 restart aera-router`. This is how the VPS deploy runs.
#   • docker — re-tag the local image aera-router:<version> to aera-router:prod and
...
Rolls Aera Router back to <version> (e.g. 3.8.35 or v3.8.35). With no version,
...
if [ -z "$VERSION" ]; then
  ...
  VERSION="$(npm view aera-router versions --json 2>/dev/null | node -e '
    let s = "";
...
ops_log "target: aera-router@$VERSION via $METHOD"
ops_confirm "Roll Aera Router back to $VERSION via $METHOD?" || ops_die "aborted"

case "$METHOD" in
  npm)
    ops_require_cmd npm
    npm install -g "aera-router@$VERSION"
    if command -v pm2 >/dev/null 2>&1 && pm2 jlist 2>/dev/null | grep -q '"name":"aera-router"'; then
      pm2 restart aera-router --update-env
      ops_log "pm2 restarted aera-router"
    else
      ops_log "installed aera-router@$VERSION — restart the service to apply (no PM2 'aera-router' process found)"
    fi
    ;;
  docker)
    ops_require_cmd docker
    if ! docker image inspect "aera-router:$VERSION" >/dev/null 2>&1; then
      ops_die "local image aera-router:$VERSION not found — build it from the $VERSION checkout first (this compose builds the 'prod' tag, it does not pull a registry tag)"
    fi
    docker tag "aera-router:$VERSION" aera-router:prod
    docker compose -f "$REPO_ROOT/docker-compose.prod.yml" up -d --no-build
    ops_log "recreated prod service from aera-router:$VERSION"
    ;;
