 ...
 *   aera-router reset-password
 *
 * Non-interactive / scripted usage (piped stdin, e.g. CI or Docker):
 *   printf 'NewPass123\nNewPass123\n' | aera-router reset-password
 *   printf 'NewPass123' | aera-router reset-password --password-stdin
 *
 * Resets the admin password for Aera Router.
...
async function main() {
  ...
  if (!passwordState.exists) {
    ...
    console.error(`   Make sure Aera Router has been started at least once.`);
  ...
  console.log("   Restart Aera Router for changes to take effect.\n");
}

main()
  .then(() => {
    // Explicit exit(0) so a caller that imports this module (bin/aera-router.mjs
    // routes `aera-router reset-password` here) terminates cleanly instead of
