...
import {
  buildAeraRouterOpenCodeConfig,
  createAeraRouterAgentBlock,
  createAeraRouterComboConfig,
  createAeraRouterMCPEntry,
  createAeraRouterModesBlock,
  createAeraRouterProvider,
  fetchLiveModels,
  listCombos,
  mergeIntoExistingConfig,
  normalizeBaseURL,
  AERA_ROUTER_DEFAULT_MODEL_CAPABILITIES,
  AERA_ROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS,
  AERA_ROUTER_DEFAULT_OPENCODE_MODELS,
  AERA_ROUTER_MCP_DEFAULT_SCOPES,
  AERA_ROUTER_PROVIDER_NPM,
...
test("createAeraRouterProvider validates required fields", () => {
  assert.throws(
    () => createAeraRouterProvider({ baseURL: "", apiKey: "x" } as never),
    /baseURL is required/
  );
  assert.throws(
    () => createAeraRouterProvider({ baseURL: "http://x", apiKey: "" } as never),
    /apiKey is required/
  );
});

test("createAeraRouterProvider produces the OpenCode-compatible shape", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
  });

  assert.equal(provider.npm, AERA_ROUTER_PROVIDER_NPM);
  assert.equal(provider.name, "Aera Router");
  assert.equal(provider.options.baseURL, "http://localhost:20128/v1");
  assert.equal(provider.options.apiKey, "sk_aera_router");
  assert.equal(typeof provider.models, "object");
});

test("createAeraRouterProvider seeds the default model catalog", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
  });
  ...
  const defaultIds = [...AERA_ROUTER_DEFAULT_OPENCODE_MODELS].sort();
...
test("createAeraRouterProvider honours a custom models list and labels", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
...
test("createAeraRouterProvider deduplicates and trims model ids", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
...
test("createAeraRouterProvider honours displayName override", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
    displayName: "Local Aera Router",
  });
  assert.equal(provider.name, "Local Aera Router");
});

test("buildAeraRouterOpenCodeConfig wraps the provider with the OpenCode schema", () => {
  const doc = buildAeraRouterOpenCodeConfig({
    baseURL: "http://localhost:20128/v1",
    apiKey: "sk_aera_router",
  });

  assert.equal(doc.$schema, OPENCODE_CONFIG_SCHEMA);
  assert.equal(typeof doc.provider["aera-router"], "object");
  assert.equal(doc.provider["aera-router"].options.baseURL, "http://localhost:20128/v1");
});
...
test("buildAeraRouterOpenCodeConfig emits model and small_model prefixed with provider key", () => {
  const doc = buildAeraRouterOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
  ...
  assert.equal(doc.model, "aera-router/claude-sonnet-4-5-thinking");
  assert.equal(doc.small_model, "aera-router/gemini-3-flash");
});

test("buildAeraRouterOpenCodeConfig omits model and small_model when not supplied", () => {
  const doc = buildAeraRouterOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
  });
...
test("buildAeraRouterOpenCodeConfig ignores blank model strings", () => {
  const doc = buildAeraRouterOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
    model: "   ",
...
test("mergeIntoExistingConfig preserves existing provider entries", () => {
  ...
  assert.ok("aera-router" in (result.provider as Record<string, unknown>));
  assert.deepEqual((result as Record<string, unknown>).keybinds, { submit: "enter" });
});

test("mergeIntoExistingConfig overwrites existing aera-router entry", () => {
  const existing = {
    provider: {
      "aera-router": {
  ...
  const aeraRouter = (result.provider as Record<string, unknown>)["aera-router"] as {
    name: string;
  };
  assert.equal(aeraRouter.name, "NEW");
});
...
test("mergeIntoExistingConfig writes model and small_model when supplied", () => {
  ...
  assert.equal(result.model, "aera-router/claude-sonnet-4-5-thinking");
  assert.equal(result.small_model, "aera-router/gemini-3-flash");
});

test("mergeIntoExistingConfig does not add model keys when not supplied", () => {
  const result = mergeIntoExistingConfig(
    {},
    { baseURL: "http://localhost:20128", apiKey: "sk_aera_router" }
  );
  assert.ok(!("model" in result));
  assert.ok(!("small_model" in result));
});

test("AERA_ROUTER_MCP_DEFAULT_SCOPES contains 7 read-only scopes", () => {
  assert.equal(AERA_ROUTER_MCP_DEFAULT_SCOPES.length, 7);
  assert.ok(AERA_ROUTER_MCP_DEFAULT_SCOPES.every((s) => s.startsWith("read:")));
});

test("createAeraRouterMCPEntry defaults to tsx runtime", () => {
  const entry = createAeraRouterMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_aera_router",
  });
  assert.equal(entry.command, "npx");
  assert.deepEqual(entry.args, ["tsx", "/path/to/server.ts"]);
  assert.equal(entry.env.AERA_ROUTER_API_KEY, "sk_aera_router");
  assert.ok(!("AERA_ROUTER_MCP_ENFORCE_SCOPES" in entry.env));
  assert.ok(!("AERA_ROUTER_MANAGEMENT_API_KEY" in entry.env));
});

test("createAeraRouterMCPEntry uses node runtime when specified", () => {
  const entry = createAeraRouterMCPEntry({
...
test("createAeraRouterMCPEntry sets management key and scopes when supplied", () => {
  const entry = createAeraRouterMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_aera_router",
  ...
  assert.equal(entry.env.AERA_ROUTER_MANAGEMENT_API_KEY, "sk_manage");
  assert.equal(entry.env.AERA_ROUTER_MCP_ENFORCE_SCOPES, "true");
  assert.equal(entry.env.AERA_ROUTER_MCP_SCOPES, "read:health,read:combos,execute:completions");
});

test("createAeraRouterMCPEntry rejects missing required fields", () => {
  assert.throws(
    () => createAeraRouterMCPEntry({ serverPath: "", apiKey: "x" }),
    /serverPath is required/
  );
  assert.throws(
    () => createAeraRouterMCPEntry({ serverPath: "/p", apiKey: "" }),
    /apiKey is required/
  );
});
...
test("createAeraRouterComboConfig builds minimal payload", () => {
  const payload = createAeraRouterComboConfig({ name: "my-combo", strategy: "priority" });
...
test("createAeraRouterComboConfig includes optional fields when supplied", () => {
...
test("AERA_ROUTER_DEFAULT_OPENCODE_MODELS includes cc/ prefixed models", () => {
  const defaults = [...AERA_ROUTER_DEFAULT_OPENCODE_MODELS];
...
test("AERA_ROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS covers every default model id", () => {
  for (const id of AERA_ROUTER_DEFAULT_OPENCODE_MODELS) {
    const ctx = AERA_ROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.ok(
...
test("createAeraRouterProvider emits limit.context on default model entries", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
  });
...
test("createAeraRouterProvider omits limit.context for unknown model ids", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
...
test("createAeraRouterProvider reads contextLength from a live model entry for ids absent from the static map", () => {
  // #3298 regression guard: the static AERA_ROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS
...
test("createAeraRouterProvider: a live model contextLength wins over the static default map", () => {
...
test("createAeraRouterProvider serialises limit.context to JSON", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
  });
  ...
  for (const id of AERA_ROUTER_DEFAULT_OPENCODE_MODELS) {
    const expectedContext = AERA_ROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.equal(
...
test("AERA_ROUTER_DEFAULT_MODEL_CAPABILITIES covers every default model id", () => {
  for (const id of AERA_ROUTER_DEFAULT_OPENCODE_MODELS) {
    const caps = AERA_ROUTER_DEFAULT_MODEL_CAPABILITIES[id];
...
test("createAeraRouterProvider emits default capability flags inline with the model entry", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
  });
...
test("createAeraRouterProvider modelCapabilities overrides defaults and merges per id", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
    modelCapabilities: {
...
test("createAeraRouterProvider applies capability overrides to non-default model ids", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
    models: ["custom-model"],
...
test("createAeraRouterProvider modelLabels still works when modelCapabilities omits label", () => {
  const provider = createAeraRouterProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_aera_router",
...
test("createAeraRouterAgentBlock builds provider-prefixed entries per role", () => {
  const block = createAeraRouterAgentBlock({
    roles: {
  ...
  assert.equal(block.build.model, "aera-router/claude-sonnet-4-5-thinking");
  assert.equal(block.build.temperature, 0.2);
  assert.equal(block.plan.model, "aera-router/claude-opus-4-5-thinking");
  assert.equal(block.plan.top_p, 0.95);
  assert.equal(block.review.model, "aera-router/gemini-3-flash");
  assert.equal(block.review.temperature, 0.0);
});

test("createAeraRouterAgentBlock omits optional fields when not supplied", () => {
  const block = createAeraRouterAgentBlock({
...
test("createAeraRouterAgentBlock skips roles with empty modelId", () => {
  const block = createAeraRouterAgentBlock({
    roles: {
...
test("createAeraRouterAgentBlock emits tools as Record<string, boolean> per OC schema", () => {
  const block = createAeraRouterAgentBlock({
    roles: {
      build: {
...
test("createAeraRouterAgentBlock filters invalid tool entries and omits empty maps", () => {
  const block = createAeraRouterAgentBlock({
    roles: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
test("createAeraRouterModesBlock builds provider-prefixed mode entries", () => {
  const block = createAeraRouterModesBlock({
    modes: {
...
test("createAeraRouterModesBlock skips modes with empty modelId", () => {
  const block = createAeraRouterModesBlock({
    modes: {
      build: { modelId: "claude-sonnet-4-5-thinking" },
      plan: { modelId: "" },
...
test("createAeraRouterModesBlock honours numeric overrides limited to OC schema", () => {
  const block = createAeraRouterModesBlock({
    modes: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
// #3419 — soft-deprecation in favour of @aera-router/opencode-plugin. Guard the
...
test("package is marked deprecated in favour of @aera-router/opencode-plugin (#3419)", () => {
  const here = dirname(fileURLToPath(import.meta.url));
  const pkg = JSON.parse(readFileSync(join(here, "..", "package.json"), "utf8"));
  assert.match(pkg.description, /DEPRECATED/);
  assert.match(pkg.description, /@aera-router\/opencode-plugin/);

  const readme = readFileSync(join(here, "..", "README.md"), "utf8");
  assert.match(readme, /Deprecated/i);
  assert.match(readme, /@aera-router\/opencode-plugin/);
});
