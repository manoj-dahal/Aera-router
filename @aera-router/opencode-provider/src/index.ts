/**
 * OpenCode provider plugin for Aera Router AI Gateway.
 *
 * Generates an OpenCode-compatible provider object that points to a running
 * Aera Router instance. The output follows the OpenCode config schema
 * (https://opencode.ai/config.json) and delegates the runtime to
 * `@ai-sdk/openai-compatible` so OpenCode can drive any Aera Router-exposed
 ...
 *     });
 *     // config -> { $schema, provider: { aera-router: { npm, name, options, models } } }
 *     ```
 *
 *  2. As a single-provider entry to merge into an existing opencode.json:
 *
 *     ```ts
 *     import { createAeraRouterProvider } from "@aera-router/opencode-provider";
 *     const provider = createAeraRouterProvider({ baseURL, apiKey });
 *     // provider -> the value to place under provider.aera-router in opencode.json
 *     ```
 *
...
export const AERA_ROUTER_PROVIDER_NPM = "@ai-sdk/openai-compatible" as const;
 ...
 *
 * Curated set covering the most commonly deployed Aera Router models. Synced
 * with the Alph4d0g/opencode-aera-router-auth AERA_ROUTER_DEFAULT_MODELS constant
 * (https://github.com/Alph4d0g/opencode-aera-router-auth, MIT) and extended
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-aera-router-auth
 * (https://github.com/Alph4d0g/opencode-aera-router-auth, MIT).
 */
 ...
 * Matches the context lengths used by Aera Router's provider registry.
 */
export const AERA_ROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS: Record<string, number> = {
 ...
 * model via `AeraRouterProviderOptions.modelCapabilities`.
 */
export const AERA_ROUTER_DEFAULT_MODEL_CAPABILITIES: Record<string, ModelCapabilities> = {
...
export interface AeraRouterProviderOptions {
  /** Aera Router base URL, with or without trailing `/v1`. Required. */
  baseURL: string;
  /** Aera Router API key. Required. Use `sk_aera_router` for local instances without REQUIRE_API_KEY. */
  apiKey: string;
  /** Override the display name shown in OpenCode. Default: `"Aera Router"`. */
  displayName?: string;
   ...
   * `AERA_ROUTER_DEFAULT_MODEL_CAPABILITIES` for ids in the default catalog;
   * for custom ids the override is used verbatim.
   */
  modelCapabilities?: Record<string, ModelCapabilities>;
  /**
   * Optional per-model context-length overrides (tokens). Takes precedence
   * over the static `AERA_ROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS` map but is
   ...
   * Emitted as `"aera-router/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.aera-router.models[id]`. */
...
export interface OpenCodeProviderEntry {
  /** Identifier of the OpenCode runtime package that will speak to Aera Router. */
  npm: typeof AERA_ROUTER_PROVIDER_NPM;
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"aera-router/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"aera-router/gemini-3-flash"`. */
  small_model?: string;
  provider: {
    [AERA_ROUTER_PROVIDER_KEY]: OpenCodeProviderEntry;
  };
}

function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@aera-router/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@aera-router/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@aera-router/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
/**
 * Build the `provider.aera-router` entry for an OpenCode config document.
 * The returned object is JSON-serialisable and safe to embed verbatim.
 */
export function createAeraRouterProvider(
  options: AeraRouterProviderOptions
  ...
  const modelList =
    options.models && options.models.length > 0
      ? [...options.models]
      : [...AERA_ROUTER_DEFAULT_OPENCODE_MODELS];
  ...
  for (const raw of modelList) {
    ...
    seen.add(id);
    const defaults = AERA_ROUTER_DEFAULT_MODEL_CAPABILITIES[id] ?? {};
    ...
    const rawContextLength =
      ...
      AERA_ROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    const contextLength =
  ...
  return {
    npm: AERA_ROUTER_PROVIDER_NPM,
    name: options.displayName?.trim() || "Aera Router",
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.aera-router`).
 * Useful when scaffolding a fresh `opencode.json`.
 *
 * When `options.model` / `options.smallModel` are supplied they are emitted as
 * top-level `model` / `small_model` keys prefixed with `"aera-router/"` so
 * OpenCode resolves them through the configured provider.
 */
export function buildAeraRouterOpenCodeConfig(
  options: AeraRouterProviderOptions
): OpenCodeConfigDocument {
  const doc: OpenCodeConfigDocument = {
    $schema: OPENCODE_CONFIG_SCHEMA,
    provider: {
      [AERA_ROUTER_PROVIDER_KEY]: createAeraRouterProvider(options),
    },
  };

  if (options.model !== undefined) {
    const id = options.model.trim();
    if (id) doc.model = `${AERA_ROUTER_PROVIDER_KEY}/${id}`;
  }

  if (options.smallModel !== undefined) {
    const id = options.smallModel.trim();
    if (id) doc.small_model = `${AERA_ROUTER_PROVIDER_KEY}/${id}`;
  }
...
/**
 * Merge the Aera Router provider entry (and optional `model` / `small_model`
 ...
 * present are not removed. If `existing.provider.aera-router` already exists it
...
export function mergeIntoExistingConfig(
  ...
  const partial = buildAeraRouterOpenCodeConfig(options);
  ...
  merged.provider = {
    ...existingProvider,
    [AERA_ROUTER_PROVIDER_KEY]: partial.provider[AERA_ROUTER_PROVIDER_KEY],
  };
 ...
 */
export const AERA_ROUTER_MCP_DEFAULT_SCOPES = [
  "read:health",
  "read:combos",
  "read:quota",
  "read:usage",
...
export type AeraRouterMCPScope = (typeof AERA_ROUTER_MCP_DEFAULT_SCOPES)[number] | string;

export interface AeraRouterMCPOptions {
  /** Absolute path to the MCP server entry point (TypeScript or compiled JS). */
  serverPath: string;
  /** Aera Router API key forwarded to the MCP server as `AERA_ROUTER_API_KEY`. */
  apiKey: string;
  /**
   * Management API key used for management-scoped operations.
   * When supplied it is forwarded as `AERA_ROUTER_MANAGEMENT_API_KEY`.
   */
  managementApiKey?: string;
  /**
   * Comma-separated scope list passed as `AERA_ROUTER_MCP_SCOPES`.
   * When omitted `AERA_ROUTER_MCP_ENFORCE_SCOPES` is not set and all scopes are
   * available (development default). Pass an explicit list to restrict access.
   */
  scopes?: AeraRouterMCPScope[];
  /**
...
/**
 * Build the `mcp.servers.aera-router` entry for an OpenCode config document.
 *
 * @example
 * ```ts
 * const mcpEntry = createAeraRouterMCPEntry({
 *   serverPath: "/home/user/.local/share/aera-router/open-sse/mcp-server/server.ts",
 *   apiKey: "sk_aera_router",
 *   managementApiKey: "sk_manage_...",
 *   scopes: ["read:health", "read:combos", "execute:completions"],
 * });
 * // Place at config.mcp.servers.aera-router
 * ```
 */
export function createAeraRouterMCPEntry(options: AeraRouterMCPOptions): OpenCodeMCPServerEntry {
  ...
  const env: Record<string, string> = {
    AERA_ROUTER_API_KEY: apiKey,
  };

  if (options.managementApiKey !== undefined) {
    const mgmtKey = options.managementApiKey.trim();
    if (mgmtKey) env.AERA_ROUTER_MANAGEMENT_API_KEY = mgmtKey;
  }

  if (options.scopes !== undefined && options.scopes.length > 0) {
    env.AERA_ROUTER_MCP_ENFORCE_SCOPES = "true";
    env.AERA_ROUTER_MCP_SCOPES = options.scopes.join(",");
  }
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@aera-router/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
 ...
 * The shape mirrors the subset of fields that Aera Router's `/v1/models`
 * endpoint reliably provides across versions, normalised from both
 * camelCase and snake_case variants used by different Aera Router releases.
 *
 * Attribution: field-variant normalisation logic adapted from
 * https://github.com/Alph4d0g/opencode-aera-router-auth (MIT).
 */
export interface AeraRouterLiveModel {
  id: string;
  name: string;
...
/**
 * Fetch the live model catalog from a running Aera Router instance.
 *
 ...
 * `display_name`) field variants across Aera Router versions.
 *
 * Useful for dynamically populating the `models` option of
 * `createAeraRouterProvider` / `buildAeraRouterOpenCodeConfig` instead of
 * relying on `AERA_ROUTER_DEFAULT_OPENCODE_MODELS`.
 *
 * @param baseURL   - Aera Router base URL (with or without `/v1`).
 * @param apiKey    - Aera Router API key.
 * @param timeoutMs - Request timeout in milliseconds (default 5000).
 *
 * @example
 * ```ts
 * const models = await fetchLiveModels("http://localhost:20128", "sk_aera_router");
 * const config = buildAeraRouterOpenCodeConfig({
 *   baseURL: "http://localhost:20128",
 *   apiKey: "sk_aera_router",
 *   models,                    // AeraRouterLiveModel[] — contextLength auto-extracted
...
export async function fetchLiveModels(
  ...
  const models: AeraRouterLiveModel[] = [];
  for (const raw of rawList) {
    ...
    // Extract context_length from Aera Router's /v1/models response.
    // Aera Router returns context_length in snake_case for both synced
 ...
 */
export type AeraRouterCompressionOverride =
...
export interface AeraRouterCombo {
  id: string;
  name: string;
  strategy: string;
  active: boolean;
  compressionOverride: AeraRouterCompressionOverride;
}

/**
 * Fetch the active routing combo list from a running Aera Router instance.
 *
...
export async function listCombos(
  ...
  for (const raw of rawList) {
    ...
    const compressionOverride = VALID_COMPRESSION_OVERRIDES.has(rawOverride)
      ? (rawOverride as AeraRouterCompressionOverride)
      : "";
...
/**
 * Options for `createAeraRouterComboConfig`.
 * Mirrors the subset of combo fields exposed by the Aera Router `/api/combos`
 * PATCH / POST payload that are safe to set programmatically.
 */
export interface AeraRouterComboConfigOptions {
...
/**
 * Build a typed combo payload suitable for Aera Router's management API.
 *
 ...
 * @example
 * ```ts
 * const payload = createAeraRouterComboConfig({
 *   name: "claude-primary",
 *   strategy: "priority",
 ...
 * ```
 */
export function createAeraRouterComboConfig(
  options: AeraRouterComboConfigOptions
): Record<string, unknown> {
 ...
 */
export interface AeraRouterRoleOverrides {
...
/** Per-role binding used by `createAeraRouterAgentBlock`. */
export interface AeraRouterAgentRole extends AeraRouterRoleOverrides {
  /** Aera Router model id, e.g. `"claude-sonnet-4-5-thinking"`. */
  modelId: string;
...
/** Options for `createAeraRouterAgentBlock`. */
export interface AeraRouterAgentBlockOptions {
  /** Per-role bindings. Keys become entries under OpenCode's `agent` block. */
  roles: Record<string, AeraRouterAgentRole>;
}

/** Single entry inside the emitted OpenCode `agent` block. */
export interface OpenCodeAgentEntry extends AeraRouterRoleOverrides {
  /** Always emitted as `"aera-router/<modelId>"`. */
  model: string;
...
function buildAgentEntry(role: AeraRouterAgentRole): OpenCodeAgentEntry | undefined {
  if (!role || typeof role.modelId !== "string") return undefined;
  const modelId = role.modelId.trim();
  if (!modelId) return undefined;
  const entry: OpenCodeAgentEntry = { model: `${AERA_ROUTER_PROVIDER_KEY}/${modelId}` };
 ...
 * specific Aera Router model. Useful for `.opencode/agent/*.md` defaults and
 ...
 * @example
 * ```ts
 * const agentBlock = createAeraRouterAgentBlock({
 *   roles: {
 ...
 *   },
 * });
 * // -> { build: { model: "aera-router/claude-sonnet-4-5-thinking", temperature: 0.2 }, ... }
 * ```
 */
export function createAeraRouterAgentBlock(
  options: AeraRouterAgentBlockOptions
...
/**
 * Per-mode binding used by `createAeraRouterModesBlock`.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `AeraRouterAgentRole` + `createAeraRouterAgentBlock`. This
 * type and the corresponding helper are kept for back-compat with configs
 * still using `mode:`.
 */
export interface AeraRouterMode extends AeraRouterAgentRole {}

/**
 * Options for `createAeraRouterModesBlock`.
 *
 * @deprecated See `AeraRouterMode`.
 */
export interface AeraRouterModesBlockOptions {
  /** Per-mode bindings. Keys become entries under OpenCode's deprecated top-level `mode` block. */
  modes: Record<string, AeraRouterMode>;
}
 ...
 * a specific Aera Router model. Emits the same shape as the `agent` block since
 * OpenCode's schema treats them identically (both reference `AgentConfig`).
 *
 * Modes with empty / missing `modelId` are skipped.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `createAeraRouterAgentBlock`. This helper is kept for
 * back-compat with configs still using `mode:`.
 *
 * @example
 * ```ts
 * const modesBlock = createAeraRouterModesBlock({
 *   modes: {
 ...
 * ```
 */
export function createAeraRouterModesBlock(
  options: AeraRouterModesBlockOptions
...
export default createAeraRouterProvider;
