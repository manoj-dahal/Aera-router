# @aera-router/opencode-plugin

> **Recommended way to use Aera Router with OpenCode.** Pulls a live model catalog from `/v1/models` (including `-low`/`-medium`/`-high`/`-thinking` variants as first-class IDs), aggregates combos via `/api/combos` using a least-common-denominator capability/limit join, sanitizes Gemini tool schemas in flight, and supports multiple side-by-side Aera Router instances out of the box.

## Why this and not `@aera-router/opencode-provider`?

`@aera-router/opencode-provider` is the legacy config-generator package — it writes a frozen `provider.aera-router` block into `opencode.json` with a **hardcoded list of 8 models** ([`AERA_ROUTER_DEFAULT_OPENCODE_MODELS`](https://github.com/manoj-dahal/Aera-router/blob/main/%40aera_router/opencode-provider/src/index.ts#L48-L56)). It works on the CLI but in the **OpenCode Desktop / Web** builds (Tauri / Electron) the runtime re-runs the model picker and the static block surfaces only a few of those — and they drift behind the live Aera Router catalog.
...
- Emitting the provider block **dynamically** in the plugin's `config`/`provider` hook — so `opencode.json` only needs the plugin entry, not a static `provider.aera-router`
- Re-fetching on a configurable TTL (default 5 min), so new models / combo changes in the Aera Router UI appear without restarting OpenCode
...
The plugin ships **pre-built inside the `aera-router` npm package** since v3.8.23.
If you have Aera Router installed, the plugin is already on disk:

```sh
# 1. One command — copy the plugin into OpenCode and update opencode.json
aera-router setup opencode --auth

# 2. Follow the interactive prompt to enter your Aera Router API key
# 3. Restart OpenCode — /models lists the full live catalog
```

The `--auth` flag runs `opencode auth login --provider aera-router` automatically.
Use `--base-url` to point at a non-default Aera Router address:

```sh
aera-router setup opencode --base-url https://or.example.com --auth
```

### What it does

1. Locates the bundled plugin inside the aera-router installation
2. Copies `dist/` + `package.json` to `~/.config/opencode/plugins/aera-router/`
...
`@aera-router/opencode-provider` or the legacy `opencode-aera-router-auth` package are
automatically cleaned up.

### Manual install (without aera-router CLI)

If you cannot run `aera-router setup opencode` (local dev, CI, air-gapped), reference
the built artifact directly:

```sh
cd @aera-router/opencode-plugin && npm run build && npm pack
# then extract into ~/.config/opencode/plugins/aera-router-opencode-plugin/
```
...
// opencode.json
{
  ...
  "plugin": [
    [
      "./plugins/aera-router-opencode-plugin/dist/index.js",
      {
...
```sh
opencode auth login --provider aera-router
# prompts for the Aera Router API key, writes to ~/.local/share/opencode/auth.json
```

> ⚠ Use the `--provider` flag explicitly. `opencode auth login aera-router` is parsed as a positional `url` argument by current OC releases (≤1.15.5) and fails with `fetch() URL is invalid`. Tracked upstream.

Restart OpenCode. `/models` lists the full live catalog. Variants (`-low`, `-medium`, `-high`, `-thinking`) and combos appear as first-class IDs — Aera Router is the source of truth, no client-side synthesis.
...
cd /path/to/Aera Router/@aera-router/opencode-plugin
npm run build
npm pack
# produces aera-router-opencode-plugin-0.1.0.tgz

# 2. Extract one copy per Aera Router endpoint
mkdir -p ~/.config/opencode/plugins/aera-router-opencode-plugin-prod
mkdir -p ~/.config/opencode/plugins/aera-router-opencode-plugin-preprod
tar -xzf aera-router-opencode-plugin-0.1.0.tgz -C ~/.config/opencode/plugins/aera-router-opencode-plugin-prod    --strip-components=1
tar -xzf aera-router-opencode-plugin-0.1.0.tgz -C ~/.config/opencode/plugins/aera-router-opencode-plugin-preprod --strip-components=1
```
...
```jsonc
{
  ...
  "plugin": [
    [
      "./plugins/aera-router-opencode-plugin-prod/dist/index.js",
      {
        "providerId": "aera-router",
    ...
    [
      "./plugins/aera-router-opencode-plugin-preprod/dist/index.js",
      {
        "providerId": "aera-router-preprod",
        "displayName": "Aera Router Preprod",
...
```sh
opencode auth login --provider aera-router
opencode auth login --provider aera-router-preprod
```
...
### After publish (`@aera-router/opencode-plugin` npm)
...
mkdir -p ~/.config/opencode/plugins/aera-router-opencode-plugin-prod
mkdir -p ~/.config/opencode/plugins/aera-router-opencode-plugin-preprod
npm install --prefix ~/.config/opencode/plugins/aera-router-opencode-plugin-prod    @aera-router/opencode-plugin
npm install --prefix ~/.config/opencode/plugins/aera-router-opencode-plugin-preprod @aera-router/opencode-plugin
```

`opencode.json` paths become `./plugins/aera-router-opencode-plugin-prod/node_modules/@aera-router/opencode-plugin/dist/index.js` (and the preprod equivalent).
...
| Variants pass-through                      | `-low`/`-medium`/`-high`/`-thinking` ship as first-class IDs from Aera Router (no client synthesis)                                                                                                                                                                                                                                                                                                         | `provider.models`            |
...
| --------------------- | -------- | ------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| `providerId`          | `string` | `"aera-router"`                            | OpenCode provider id; must be unique across plugin entries                                            |
| `displayName`         | `string` | `"Aera Router"` or `Aera Router (<id>)`    | Label in the OC UI                                                                                    |
| `modelCacheTtl`       | `number` | `300000` (5 min)                           | `/v1/models` TTL in ms                                                                                |
| `baseURL`             | `string` | resolved from `auth.json` after `/connect` | Override Aera Router base URL                                                                         |
...
| `diskCache`           | `boolean` | `true`  | Persist the last successful `/v1/models` + `/api/combos` + enrichment + connections + compression snapshot to `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/aera-router-<providerId>.json`. On a subsequent cold start where `/v1/models` throws (network down / IP whitelist drop / 5xx) the static block hydrates from the snapshot so OC's model picker survives offline. Soft-fail on read/write — never blocks publishing.                                                                                                                          |
...
With `mcpAutoEmit: true`, the plugin synthesises an `mcp.aera-router` entry equivalent to a manual:
...
If you want a narrower-scoped Bearer for MCP (different from the chat/inference key), set `features.mcpToken`. Operator overrides win: if you already set `mcp.aera-router` in `opencode.json`, the plugin will not overwrite it.
...
- `usableOnly: true` drops models whose canonical provider has no healthy connection in your Aera Router instance — your `/models` picker stays focused on what you can actually call.
- `diskCache: true` (default) writes a snapshot to `${OPENCODE_DATA_DIR}/plugins/aera-router-<providerId>.json` on every healthy refresh. On a cold start where `/v1/models` is unreachable (laptop offline, IP whitelist drop), the snapshot hydrates the static block so OC still shows the catalog instead of a stub.
- `compressionMetadata: true` annotates combo display names with their pipeline using traffic-light emoji for intensity (e.g. `Combo: claude-primary [rtk🟡 → caveman🟠]`) so the picker advertises which compression each combo applies and how heavy it is at a glance. Palette: 🟢 lite/minimal · 🟡 standard · 🟠 aggressive/full · 🔴 ultra. Unknown intensities fall through to raw text (`[rtk:custom-thing]`) so the plugin never hides a value Aera Router knows but the plugin doesn't.
...
## Comparison vs `@aera-router/opencode-provider`

[`@aera-router/opencode-provider`](https://github.com/manoj-dahal/Aera-router/tree/main/%40aera_router/opencode-provider) is the existing config-generator package — it writes a frozen `provider.<id>` block into `opencode.json` at build time. This plugin is the runtime integration.

|                   | `@aera-router/opencode-plugin` (this) | `@aera-router/opencode-provider`  |
| ----------------- | ------------------------------------- | --------------------------------- |
