/**
 * OpenCode plugin for the Aera Router AI Gateway.
 *
 * Implements the official `@opencode-ai/plugin` Plugin contract (auth +
 * provider + config hooks) to drive a running Aera Router instance from
 ...
 *                  pass-through (Aera Router is the source of truth — no
 ...
 *
 *  1. Single-instance (default `providerId: "aera-router"`):
 *
 *     ```json
 *     {
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": ["@aera-router/opencode-plugin"]
 *     }
 *     ```
 *
 ...
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": [
 *         ["@aera-router/opencode-plugin", { "providerId": "aera-router" }],
 *         ["@aera-router/opencode-plugin", { "providerId": "aera-router-preprod" }]
 *       ]
 *     }
 *     ```
 *
 ...
 *
 * Companion library: `@aera-router/opencode-provider` (build-time config generator)
 ...
 * @see https://github.com/manoj-dahal/Aera-router for the AI Gateway.
 */
 ...
 *                     different `providerId` ("aera-router", "aera-router-preprod",
 *                     "aera-router-local"). Maps to `ProviderHook.id` and
 ...
 *  - `baseURL`        Override base URL for this Aera Router instance. When
 ...
 *                           JSONL file at
 *                           `~/.local/share/opencode/plugins/aera-router-debug-{providerId}.jsonl`.
 ...
 *                           (covers Aera Router's canonical Anthropic aliases).
 *
 ...
 * import `AeraRouterPluginOptions` as before; only the source of truth
 * shifted from a hand-written interface to `z.infer<typeof optionsSchema>`.
 */
export type AeraRouterPluginOptions = z.infer<typeof optionsSchema>;

export const AERA_ROUTER_PROVIDER_KEY = "aera-router" as const;
 ...
 */
export function resolveAeraRouterPluginOptions(opts?: AeraRouterPluginOptions): Required<
  Pick<AeraRouterPluginOptions, "providerId" | "displayName" | "modelCacheTtl">
> & {
  /**
   * #6859: the UNPREFIXED provider id ("aera-router", "aera-router-preprod", …).
   ...
   * `provider.<id>` block). `aeraRouterProviderId` MUST be used everywhere an
   * identifier reaches or represents something Aera Router's own server parses
   * (model `id` prefix, `ModelV2.providerID`, combo catalog keys in the
   * dynamic provider hook) — Aera Router's `parseModel()` has no alias for
   * "opencode-<x>", so a prefixed id there is unrecoverable and credential
   * lookup fails with "No credentials for opencode-<x>".
   */
  aeraRouterProviderId: string;
} & Pick<AeraRouterPluginOptions, "baseURL" | "managementReadToken" | "features"> {
  const rawProviderId = opts?.providerId ?? AERA_ROUTER_PROVIDER_KEY;
  const aeraRouterProviderId = trimLeadingOpencodePrefix(rawProviderId);
  // OC 1.17.8+ native-adapter gate rejects providerID not in
  // {openai, anthropic, opencode*}. Silently prefix so existing
  // configs (providerId: "aera-router") keep working.
  const providerId = rawProviderId.startsWith("opencode-")
    ? rawProviderId
    : `opencode-${rawProviderId}`;
  const displayName =
    opts?.displayName ??
    (providerId === `opencode-${AERA_ROUTER_PROVIDER_KEY}`
      ? "Aera Router"
      : `Aera Router (${providerId})`);
  const modelCacheTtl =
 ...
 * gate — see `resolveAeraRouterPluginOptions`) so the returned id is safe to
 * embed in anything Aera Router's own server parses. A user-supplied
 * `providerId: "opencode-aera-router"` (already prefixed) resolves to the same
 * unprefixed "aera-router" as the default, matching `providerId`'s own
 ...
 * direct factory call) into the validated `AeraRouterPluginOptions` shape.
 *
 ...
 *
 * Validation happens at plugin invocation time (inside `AeraRouterPlugin`),
 ...
 */
export function parseAeraRouterPluginOptions(opts: unknown): AeraRouterPluginOptions {
  ...
  if (!result.success) {
    const errs = result.error.issues
      ...
      .join("; ");
    throw new Error(`Invalid @aera-router/opencode-plugin options: ${errs}`);
  }
  return result.data;
}

/**
 * Internal coercion shim. Delegates to `parseAeraRouterPluginOptions` to keep
 ...
 */
function coercePluginOptions(opts?: PluginOptions): AeraRouterPluginOptions {
  return parseAeraRouterPluginOptions(opts);
}
 ...
 * Covers Aera Router's canonical Anthropic aliases: `cc/`, `claude/`,
 ...
 *     the multi-instance bug in opencode-aera-router-auth@1.2.1 which pinned
 *     `AERA_ROUTER_PROVIDER_ID = "aera-router"` at module scope).
 *   - `methods[0]` is the `api` flavor (no OAuth flow; Aera Router issues bearer
 ...
 */
export function createAeraRouterAuthHook(opts?: AeraRouterPluginOptions): AuthHook {
  const { providerId, displayName, baseURL, features } = resolveAeraRouterPluginOptions(opts);
        ...
        prompts: [
          {
            type: "text",
            key: "apiKey",
            message: `Aera Router API key (${providerId})`,
          },
        ],
      },
    ],
    loader: async (getAuth, _provider) => {
      ...
      if (
        ...
        if (wantFetchInterceptor) {
          composedFetch = createAeraRouterFetchInterceptor({
            apiKey,
 ...
 */
export const AeraRouterPlugin: Plugin = async (_input, options) => {
  ...
  // Each `AeraRouterPlugin(...)` invocation gets its OWN cache via closure,
  // so prod + preprod side-by-side instances do NOT collide.
  const sharedCache: AeraRouterFetchCache = new Map();
  ...
  return {
    auth: createAeraRouterAuthHook(resolved),
    provider: createAeraRouterProviderHook(resolved, { cache: sharedCache }),
    config: createAeraRouterConfigHook(resolved, { cache: sharedCache }),
  };
};
 ...
 * constants (AERA_ROUTER_PROVIDER_KEY, DEFAULT_MODEL_CACHE_TTL_MS) + types +
 ...
 */
const AeraRouterV1Plugin = {
  id: "@aera-router/opencode-plugin",
  server: AeraRouterPlugin,
};

export default AeraRouterV1Plugin;
...
/**
 * Raw shape of a `/v1/models` entry from Aera Router. Captured verbatim from
 ...
 */
export interface AeraRouterRawModelEntry {
  id: string;
  object?: string;
  owned_by?: string;
  root?: string | null;
 ...
 * Aera Router instance. Surfaced as a dependency so unit tests can inject a
 * stub without monkey-patching global `fetch`.
 *
 * Why we inline this instead of using `@aera-router/opencode-provider`'s
 ...
 */
export type AeraRouterModelsFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AeraRouterRawModelEntry[]>;

/**
 * Default fetcher: `GET <baseURL>/v1/models` with bearer auth + AbortController
 * timeout. Accepts both the `{object:"list", data:[…]}` envelope Aera Router
 * emits today and a bare-array envelope (defensive — keeps the plugin
 * working if a future Aera Router build trims the wrapper). Anything that
 * isn't an object with a string `id` is filtered out silently.
 */
export const defaultAeraRouterModelsFetcher: AeraRouterModelsFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@aera-router/opencode-plugin: apiKey required to fetch /v1/models");
  if (!baseURL)
    throw new Error("@aera-router/opencode-plugin: baseURL required to fetch /v1/models");
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `@aera-router/opencode-plugin: GET ${url} failed: ${res.status} ${res.statusText}`
      );
    }
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { data?: unknown }).data)
        ...
        : [];
    const out: AeraRouterRawModelEntry[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as AeraRouterRawModelEntry);
      }
    }
    return out;
  } finally {
 ...
 *   5. `thinking` from Aera Router has no 1:1 ModelV2 slot. We OR it into
 *      `reasoning` so thinking-only models still surface a non-false
 *      reasoning flag.
 *   6. `last_updated` from Aera Router has no ModelV2 slot — dropped (the
 ...
 *   7. `temperature: true` per Aera Router convention (OpenAI-compat mode
 ...
 *      Unknown strings (future Aera Router additions) are ignored — when the
 *      server adds new modalities we can map them here without breaking
 *      existing entries.
 *   9. `status: "active"` — Aera Router doesn't tier models alpha/beta on
...
/**
 * Raw shape of a single combo entry as returned by Aera Router's `/api/combos`.
 *
 * Schema established via a live probe against
 * an Aera Router `/api/combos` endpoint with a management-scoped key
 ...
 * bare array envelope so the plugin keeps working if a future Aera Router
 * build trims the wrapper (mirrors the same pattern in the sibling
 * `@aera-router/opencode-provider#listCombos`).
 *
 ...
 */
export interface AeraRouterRawComboMemberRef {
...
export interface AeraRouterRawCombo {
  id: string;
  name?: string;
  ...
  models?: AeraRouterRawComboMemberRef[];
  /** Hidden combos are excluded from the OC model picker. */
  isHidden?: boolean;
  /** When Aera Router attaches a lifecycle hint we forward it; today it doesn't. */
  release_date?: string;
  /**
 ...
 * `AeraRouterModelsFetcher` so unit tests can inject a stub instead of
 * monkey-patching global `fetch`.
 */
export type AeraRouterCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AeraRouterRawCombo[]>;
 ...
 * plugin working if a future Aera Router build trims the wrapper).
 *
 * Differences from `defaultAeraRouterModelsFetcher`:
 ...
 */
export const defaultAeraRouterCombosFetcher: AeraRouterCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey)
    throw new Error("@aera-router/opencode-plugin: apiKey required to fetch /api/combos");
  if (!baseURL)
    throw new Error("@aera-router/opencode-plugin: baseURL required to fetch /api/combos");
  ...
  try {
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ...
        : [];
    const out: AeraRouterRawCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as AeraRouterRawCombo);
      }
    }
    return out;
  } finally {
...
export function mapComboToModelV2(
  combo: AeraRouterRawCombo,
  members: AeraRouterRawModelEntry[],
  providerId: string,
  baseURL: string,
  apiFormat?: { anthropicPrefixes?: string[] }
): ModelV2 {
...
/**
 * Raw shape of an auto combo entry as returned by Aera Router's
 ...
 */
export interface AeraRouterRawAutoCombo {
  ...
  /** MAX of candidates' context windows, served by newer Aera Router builds.
 ...
 */
export type AeraRouterAutoCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AeraRouterRawAutoCombo[]>;
 ...
 */
export const defaultAeraRouterAutoCombosFetcher: AeraRouterAutoCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 5_000
) => {
  ...
  try {
    ...
    if (res.status === 404) {
      console.warn(
        `[aera-router-plugin] /api/combos/auto not available (404) — auto combos disabled`
      );
      return [];
    }
    if (!res.ok) {
      console.warn(
        `[aera-router-plugin] /api/combos/auto failed: ${res.status} ${res.statusText} — auto combos disabled`
      );
      return [];
    }
    const body = (await res.json()) as unknown;
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ? ((body as { combos: unknown[] }).combos as unknown[])
        : [];
    const out: AeraRouterRawAutoCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as AeraRouterRawAutoCombo);
      }
    }
    return out;
  } catch (err) {
    ...
    console.warn(
      `[aera-router-plugin] /api/combos/auto fetch failed: ${err instanceof Error ? err.message : String(err)} — auto combos disabled`
    );
    return [];
  } finally {
    clearTimeout(timer);
 ...
 * Aera Router builds). MUST be positive: OpenCode's overflow guard treats
...
export function mapAutoComboToStaticEntry(
  autoCombo: AeraRouterRawAutoCombo
): AeraRouterStaticModelEntry {
...
/**
 * Per-model enrichment overlay derived from Aera Router's
 ...
 */
export interface AeraRouterEnrichmentEntry {
   ...
   * Populated by `defaultAeraRouterEnrichmentFetcher` from
...
export type AeraRouterEnrichmentMap = Map<string, AeraRouterEnrichmentEntry>;

export type AeraRouterEnrichmentFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AeraRouterEnrichmentMap>;
 ...
 * other.
 */
export const defaultAeraRouterEnrichmentFetcher: AeraRouterEnrichmentFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const out: AeraRouterEnrichmentMap = new Map();
    ...
    if (res.ok) {
      ...
      if (providers && typeof providers === "object") {
        for (const [providerAlias, slot] of Object.entries(providers)) {
          ...
          // Aera Router hasn't curated a label for this slot.
          ...
          for (const m of models) {
            ...
            const entry: AeraRouterEnrichmentEntry = {
              providerAlias,
              providerCanonical,
            };
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        for (const [providerAlias, slot] of Object.entries(body as Record<string, unknown>)) {
          ...
          for (const [modelId, raw] of Object.entries(slot as Record<string, unknown>)) {
            ...
            const parsed: NonNullable<AeraRouterEnrichmentEntry["pricing"]> = {};
            // Aera Router `/api/pricing` keys:
            //   input         → cost.input
            //   output        → cost.output
...
async function writeStartupDiagnostics(params: {
  ...
  enrichment: AeraRouterEnrichmentMap;
  autoCombos: AeraRouterRawAutoCombo[];
}): Promise<void> {
  const {
    providerId,
    baseURL,
  ...
  try {
    ...
    const diagPath = path.join(diagDir, "plugins", "aera-router-startup-diagnostics.log");
    let existing = "";
    try {
...
// Re-export from naming.ts — thin wrapper preserving AeraRouterEnrichmentEntry signature
export function shortProviderLabel(
  enrichment: AeraRouterEnrichmentEntry | undefined
): string | undefined {
 ...
 *
 * Aera Router's `/api/pricing/models` is keyed by short ALIAS (`cc`, `cx`,
...
export function buildCanonicalToAliasMap(
  enrichment: AeraRouterEnrichmentMap | undefined
): Map<string, string> {
...
): AeraRouterEnrichmentEntry | undefined {
...
export function canonicalDedupSet(
  rawModels: ReadonlyArray<AeraRouterRawModelEntry>,
  canonicalToAlias: Map<string, string>
): Set<string> {
 ...
 *
 * Real example: Aera Router's `pricing['cohere']` slot lists 10 curated
 ...
 * a minimal `AeraRouterEnrichmentEntry` whenever the direct lookup
...
export function buildAliasIndex(
  enrichment: AeraRouterEnrichmentMap | undefined
): Map<string, AeraRouterEnrichmentEntry> {
  const out = new Map<string, AeraRouterEnrichmentEntry>();
  if (!enrichment) return out;
  for (const entry of enrichment.values()) {
    const alias = typeof entry.providerAlias === "string" ? entry.providerAlias.trim() : "";
...
export function resolveProviderTagEntry(
  rawId: string,
  direct: AeraRouterEnrichmentEntry | undefined,
  aliasIndex: Map<string, AeraRouterEnrichmentEntry>,
  canonicalToAlias?: Map<string, string>
): AeraRouterEnrichmentEntry | undefined {
  if (direct) {
...
export interface AeraRouterCompressionStep {
...
export interface AeraRouterCompressionCombo {
  id: string;
  name?: string;
  description?: string;
  pipeline: AeraRouterCompressionStep[];
  isDefault?: boolean;
}

export type AeraRouterCompressionMetaFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AeraRouterCompressionCombo[]>;
 ...
 */
export const defaultAeraRouterCompressionMetaFetcher: AeraRouterCompressionMetaFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: AeraRouterCompressionCombo[] = [];
  ...
  try {
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { combos?: unknown[] })?.combos)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ...
          : [];
    const out: AeraRouterCompressionCombo[] = [];
    for (const raw of list) {
      ...
      const steps: AeraRouterCompressionStep[] = [];
      for (const step of pipeline) {
        ...
        const entry: AeraRouterCompressionStep = { engine };
        if (typeof intensity === "string" && intensity.length > 0) {
          entry.intensity = intensity;
        }
        steps.push(entry);
      }
      const combo: AeraRouterCompressionCombo = { id, pipeline: steps };
 ...
 * Aera Router knows but the plugin doesn't.
 *
 ...
 */
export function formatCompressionPipeline(pipeline: AeraRouterCompressionStep[]): string {
...
// `features.usableOnly` filter. Returns the operator's installed Aera Router
...
export interface AeraRouterProviderConnection {
  /** Connection UUID. */
  id: string;
...
export type AeraRouterProvidersFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<AeraRouterProviderConnection[]>;
 ...
 */
export const defaultAeraRouterProvidersFetcher: AeraRouterProvidersFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: AeraRouterProviderConnection[] = [];
  if (!baseURL || !apiKey) return empty;
  const root = baseURL.replace(/\/v1\/?$/, "").replace(/\/$/, "");
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { connections?: unknown[] })?.connections)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ? (body as { data: unknown[] }).data
          : [];
    const out: AeraRouterProviderConnection[] = [];
    for (const raw of list) {
      if (!raw || typeof raw !== "object") continue;
 ...
 * (`claude/sonnet-4`) depending on the Aera Router deployment's `/v1/models`
 * surface shape.
 *
...
export function usableProviderAliasSet(
  connections: AeraRouterProviderConnection[],
  enrichment: AeraRouterEnrichmentMap | undefined
): {
  aliases: Set<string>;
  ...
  if (enrichment) {
    ...
    // populated by `defaultAeraRouterEnrichmentFetcher`. Every entry carries
...
export function isUsableCombo(
  ...
  // The provider id is folded INTO the full model string by Aera Router's
 ...
 * (e.g. `aera-router/MASTER`, `aera-router/MASTER-LIGHT`), guaranteeing uniqueness
 ...
 *
 * NOTE: the key MUST carry the OWNING provider prefix (`aera-router/…`), never a
 ...
 * "Unable to determine provider", whereas `aera-router/MASTER` resolves provider=
 * `aera-router` and the openai-compatible adapter strips the prefix and sends the
 ...
 * OC user may register prod + preprod Aera Router side-by-side with distinct
 ...
 *     AeraRouterPlugin instance — sharing the cache means /v1/models +
 *     /api/combos each hit the gateway exactly ONCE per TTL refresh, not
 *     twice.
 */
export interface AeraRouterFetchCacheEntry {
  rawModels: AeraRouterRawModelEntry[];
  rawCombos: AeraRouterRawCombo[];
  rawAutoCombos: AeraRouterRawAutoCombo[];
  /** Display-name + pricing overlay from /api/pricing/models. Empty Map when feature is disabled or fetch failed. */
  rawEnrichment: AeraRouterEnrichmentMap;
  /** Compression combos from /api/context/combos. Empty array when feature is disabled or fetch failed. */
  rawCompressionCombos: AeraRouterCompressionCombo[];
  /** Provider connections from /api/providers. Empty array when feature is disabled or fetch failed. */
  rawConnections: AeraRouterProviderConnection[];
  expiresAt: number;
}

export type AeraRouterFetchCache = Map<string, AeraRouterFetchCacheEntry>;
 ...
 *   - **Combo / model ID collisions: combos win.** Aera Router treats combos
 ...
 *     `/api/combos` requires a management-scoped key and Aera Router may
 ...
 *             outside tests today).
 */
export function createAeraRouterProviderHook(
  opts?: AeraRouterPluginOptions,
  deps: {
    fetcher?: AeraRouterModelsFetcher;
    combosFetcher?: AeraRouterCombosFetcher;
    autoCombosFetcher?: AeraRouterAutoCombosFetcher;
    enrichmentFetcher?: AeraRouterEnrichmentFetcher;
    compressionMetaFetcher?: AeraRouterCompressionMetaFetcher;
    providersFetcher?: AeraRouterProvidersFetcher;
    now?: () => number;
    cache?: AeraRouterFetchCache;
  } = {}
): ProviderHook {
  const resolved = resolveAeraRouterPluginOptions(opts);
  const fetcher = deps.fetcher ?? defaultAeraRouterModelsFetcher;
  ...
  const combosFetcher = deps.combosFetcher ?? defaultAeraRouterCombosFetcher;
  const autoCombosFetcher = deps.autoCombosFetcher ?? defaultAeraRouterAutoCombosFetcher;
  const enrichmentFetcher = deps.enrichmentFetcher ?? defaultAeraRouterEnrichmentFetcher;
  const compressionMetaFetcher =
    deps.compressionMetaFetcher ?? defaultAeraRouterCompressionMetaFetcher;
  const providersFetcher = deps.providersFetcher ?? defaultAeraRouterProvidersFetcher;
  ...
  const cache: AeraRouterFetchCache = deps.cache ?? new Map();
  ...
  return {
    ...
    async models(_provider, ctx) {
      ...
      if (!baseURL) {
        console.warn(
          `[aera-router-plugin] provider.models(${resolved.providerId}): ` +
      ...
      let rawModels: AeraRouterRawModelEntry[];
      let rawCombos: AeraRouterRawCombo[];
      let rawAutoCombos: AeraRouterRawAutoCombo[];
      let rawEnrichment: AeraRouterEnrichmentMap;
      let rawCompressionCombos: AeraRouterCompressionCombo[];
      let rawConnections: AeraRouterProviderConnection[];
      if (cached && cached.expiresAt > t) {
        ...
        // management-scoped key and Aera Router may not have any combos
        ...
        if (wantCombos) {
          try {
            ...
            console.warn(
              "[aera-router-plugin] combos fetch failed, falling back to models-only catalog",
              err
            );
          }
        }
        ...
        // may not exist yet on older Aera Router versions).
        rawAutoCombos = [];
        if (wantAutoCombos) {
          try {
        ...
        if (wantEnrichment) {
          try {
            ...
            console.warn(
              "[aera-router-plugin] enrichment fetch failed, falling back to raw ids",
              err
            );
          }
        }
        ...
        if (wantCompressionMeta) {
          try {
            ...
            console.warn("[aera-router-plugin] compression-metadata fetch failed", err);
          }
        }
        ...
        if (wantUsableOnly) {
          try {
            ...
            console.warn(
              "[aera-router-plugin] /api/providers fetch failed; usableOnly filter disabled for this refresh",
              err
            );
          }
        }
        ...
        // the dynamic pipeline fired and how much catalog Aera Router returned.
        // Emitted once per cache miss (TTL refresh) — quiet on cache hits.
        console.warn(
          `[aera-router-plugin] catalog refreshed for providerId=${resolved.providerId} baseURL=${baseURL}: ` +
      ...
      const rawModelById = new Map<string, AeraRouterRawModelEntry>();
      ...
      for (const entry of rawModels) {
        ...
        const model = mapRawModelToModelV2(entry, {
          ...
          providerId: resolved.aeraRouterProviderId,
          baseURL,
        ...
        // prefixed id on `model.id` (e.g. `aera-router/claude-primary`), so we
      ...
      // compression metadata is present). Aera Router returns at most one
      ...
      for (let pass = 0; pass < MAX_COMBO_PASSES && pending.length > 0; pass++) {
        ...
        for (const combo of pending) {
          ...
          const memberEntries: AeraRouterRawModelEntry[] = [];
          ...
          for (const step of memberSteps) {
            ...
            if (stepKind === "combo-ref") {
              ...
              } as unknown as AeraRouterRawModelEntry);
              continue;
            }
          ...
          // Apply enrichment overlay to combos too (Aera Router's
          ...
          const comboKey = buildComboKey(combo, usedComboKeys, resolved.aeraRouterProviderId);
          ...
          if (Object.prototype.hasOwnProperty.call(models, comboKey)) {
            ...
            if (!isIntentionalDedup) {
              ...
              if (!collisionWarned.has(dedupeKey)) {
                ...
                console.warn(
                  `[aera-router-plugin] combo key "${comboKey}" collides with a model id; combo wins.`
                );
              }
            }
          }
      ...
      if (pending.length > 0) {
        console.warn(
          `[aera-router-plugin] ${pending.length} combo(s) could not resolve all nested combo-refs after ${MAX_COMBO_PASSES} passes; they will advertise context=0 to avoid over-claiming.`
        );
      }
...
// same-origin Aera Router inference requests only
// ────────────────────────────────────────────────────────────────────────────
 ...
 * a secret bound to both the configured Aera Router origin and these intended
 * inference endpoints, and MUST NOT leak elsewhere.
 *
 * Ported from Alph4d0g's `opencode-aera-router-auth@1.2.1` `createFetchInterceptor`
 ...
 *     any caller-supplied Bearer for the same Aera Router provider.
 *
 ...
 */
export function createAeraRouterFetchInterceptor(config: {
  apiKey: string;
  baseURL: string;
}): typeof fetch {
 ...
 * validator throws on them, breaking Aera Router → Gemini chains transparently.
 *
 * Source: behavioural reverse-engineering from Alph4d0g's
 * opencode-aera-router-auth@1.2.1 (dist/src/plugin.js:517).
 */
 ...
 *              interceptor from `createAeraRouterFetchInterceptor`).
 */
    ...
    try {
      ...
      if (typeof initBody === "string") {
        ...
        if (!geminiStreamingWarningEmitted) {
          ...
          console.warn(
            "[aera-router-plugin] sanitizeGemini: streaming Request body, skipping schema strip (Gemini may reject)"
          );
        }
        return inner(input, init);
      } else if (
...
// `@aera-router/opencode-provider/src/index.ts` (`createAeraRouterProvider`,
 ...
 * `OpenCodeModelEntry` exported by `@aera-router/opencode-provider`. Stripped
 ...
 * when Aera Router didn't surface a value, NOT emitted as `undefined` — the
 * resulting JSON must be diffable across Aera Router deployments without
 * `undefined` noise.
 */
/** Modalities accepted by OC's static catalog reader (see `@opencode-ai/sdk`). */
export type AeraRouterModalityKind = "text" | "audio" | "image" | "video" | "pdf";

const STATIC_MODALITY_VALUES: ReadonlySet<AeraRouterModalityKind> = new Set([
  "text",
  "audio",
  "image",
  "video",
...
function normaliseModalities(raw: unknown): AeraRouterModalityKind[] {
  if (!Array.isArray(raw)) return [];
  const out: AeraRouterModalityKind[] = [];
  const seen = new Set<string>();
  for (const v of raw) {
    if (typeof v !== "string") continue;
    const lower = v.toLowerCase() as AeraRouterModalityKind;
...
export interface AeraRouterStaticModelEntry {
   ...
   * model KEY already carries the provider prefix (e.g. `aera-router/MASTER`),
  ...
  tool_call?: boolean;
  /**
   * Per-million-token cost. Maps from Aera Router `/api/pricing` shape:
   ...
   * Aera Router's `input_modalities` / `output_modalities` on `/v1/models`.
   ...
   */
  modalities?: {
    input: AeraRouterModalityKind[];
    output: AeraRouterModalityKind[];
  };
}

/**
 * Static `provider.<id>` block written to `input.provider` by the config hook.
 * Mirrors `OpenCodeProviderEntry` from `@aera-router/opencode-provider`.
 *
 *   - `npm` is always `"@ai-sdk/openai-compatible"` — Aera Router exposes an
 ...
 */
export interface AeraRouterStaticProviderEntry {
  npm: "@ai-sdk/openai-compatible";
  name: string;
  options: {
    baseURL: string;
  ...
  models: Record<string, AeraRouterStaticModelEntry>;
}
 ...
 *
 * Mapping rules (per the sibling `createAeraRouterProvider` output spec):
 *
...
export function buildStaticProviderEntry(
  rawModels: AeraRouterRawModelEntry[],
  rawCombos: AeraRouterRawCombo[],
  opts: ReturnType<typeof resolveAeraRouterPluginOptions>,
  baseURL: string,
  apiKey: string,
  enrichment?: AeraRouterEnrichmentMap,
  compressionCombos?: AeraRouterCompressionCombo[],
  connections?: AeraRouterProviderConnection[],
  rawAutoCombos?: AeraRouterRawAutoCombo[]
): AeraRouterStaticProviderEntry {
  const models: Record<string, AeraRouterStaticModelEntry> = {};
  ...
  // Build a name-set of every non-hidden combo from `/api/combos`. Aera Router
  ...
  for (const raw of rawModels) {
    ...
    // model KEY (e.g. `aera-router/claude-opus-4`) is what OC uses to recover
    // (providerID, modelID) when the user selects a model.
    const entry: AeraRouterStaticModelEntry = { name: displayName };
    ...
    // Modalities — emit when Aera Router surfaced any. Without this field
    ...
    // Aera Router field names to OC's static-schema field names.
    const pricing = enrichmentEntry?.pricing;
    if (pricing && (typeof pricing.input === "number" || typeof pricing.output === "number")) {
      const cost: NonNullable<AeraRouterStaticModelEntry["cost"]> = {
  ...
  const resolvedStaticCombosByName = new Map<string, AeraRouterStaticModelEntry>();
  ...
  for (let pass = 0; pass < MAX_STATIC_COMBO_PASSES && pendingStatic.length > 0; pass++) {
    ...
    for (const combo of pendingStatic) {
      ...
      // provider and the KEY prefix (`aera-router/<slug>`) is what OC parses.
      const entry: AeraRouterStaticModelEntry = { name: displayName };
      ...
      if (hasMembers) {
        ...
        const intersect = (sets: Set<AeraRouterModalityKind>[]): AeraRouterModalityKind[] => {
  ...
  if (pendingStatic.length > 0) {
    console.warn(
      `[aera-router-plugin] ${pendingStatic.length} combo(s) in the static catalog could not resolve all nested combo-refs after ${MAX_STATIC_COMBO_PASSES} passes; they will be omitted.`
    );
  }
    ...
    for (const autoCombo of rawAutoCombos) {
      ...
      if (models[key]) {
        ...
        if (!reportedCollisions.has(key)) {
          ...
          console.warn(
            `[aera-router-plugin] auto combo key "${key}" collides with an existing model; auto combo wins.`
          );
        }
      }
      models[key] = entry;
 ...
 *
 * Exported as a dependency-injectable function on `createAeraRouterConfigHook`
...
// `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/aera-router-<providerId>.json`.
...
interface AeraRouterDiskSnapshot {
  v: 2;
  ...
  rawEnrichment: Array<[string, AeraRouterEnrichmentEntry]>;
  rawCompressionCombos: AeraRouterCompressionCombo[];
  rawConnections: AeraRouterProviderConnection[];
...
export function diskSnapshotPath(providerId: string): string {
  ...
  return path.join(dir, "plugins", `aera-router-${providerId}.json`);
}

export type AeraRouterDiskSnapshotWriter = (
  providerId: string,
  entry: Omit<AeraRouterFetchCacheEntry, "expiresAt">,
  identityFingerprint: string
) => Promise<void>;

export type AeraRouterDiskSnapshotReader = (
  providerId: string,
  identityFingerprint: string
) => Promise<Omit<AeraRouterFetchCacheEntry, "expiresAt"> | undefined>;
...
export const defaultDiskSnapshotWriter: AeraRouterDiskSnapshotWriter = async (
  providerId,
  entry,
  identityFingerprint
) => {
  try {
    ...
    const snapshot: AeraRouterDiskSnapshot = {
      v: 2,
      identityFingerprint,
...
export const defaultDiskSnapshotReader: AeraRouterDiskSnapshotReader = async (
  providerId,
  identityFingerprint
) => {
  try {
    ...
    const parsed = JSON.parse(body) as Partial<AeraRouterDiskSnapshot>;
    if (
      !parsed ||
      parsed.v !== 2 ||
...
export const noopDiskSnapshotWriter: AeraRouterDiskSnapshotWriter = async () => {};
...
function debugLogPath(providerId: string): string {
  return join(debugLogDir(), `aera-router-debug-${providerId}.jsonl`);
}

function debugStatePath(providerId: string): string {
  return join(debugLogDir(), `aera-router-debug-${providerId}.state.json`);
}
...
export function debugLogSetEnabled(providerId: string, enabled: boolean): void {
  try {
    ...
    console.warn(`[aera-router-plugin] debugLogSetEnabled failed: ${(err as Error).message}`);
  }
}
...
export function debugLogAppend(entry: DebugLogEntry): void {
  try {
    ...
    console.warn(`[aera-router-plugin] debugLogAppend failed: ${(err as Error).message}`);
  }
}
...
export function debugLogClear(providerId: string): void {
  try {
    ...
    console.warn(`[aera-router-plugin] debugLogClear failed: ${(err as Error).message}`);
  }
}
...
export const noopDiskSnapshotReader: AeraRouterDiskSnapshotReader = async () => undefined;

export type AeraRouterReadAuthJson = () => Promise<AuthJsonShape | undefined | null>;

export const defaultReadAuthJson: AeraRouterReadAuthJson = async () => {
  const dir = process.env.OPENCODE_DATA_DIR ?? path.join(os.homedir(), ".local/share/opencode");
 ...
 *   - `fetcher`          — replaces `defaultAeraRouterModelsFetcher`.
 *   - `combosFetcher`    — replaces `defaultAeraRouterCombosFetcher`.
 *   - `now`              — clock for cache TTL (default `Date.now`).
 *   - `cache`            — shared fetch-result cache (see
 *                          `AeraRouterFetchCache`). Pass the same Map the
 *                          provider hook owns to dedupe round-trips.
 *   - `logger`           — `{warn}` sink for breadcrumb capture in tests.
 *                          Defaults to `console`.
 */
export function createAeraRouterConfigHook(
  opts?: AeraRouterPluginOptions,
  deps: {
    readAuthJson?: AeraRouterReadAuthJson;
    fetcher?: AeraRouterModelsFetcher;
    combosFetcher?: AeraRouterCombosFetcher;
    autoCombosFetcher?: AeraRouterAutoCombosFetcher;
    enrichmentFetcher?: AeraRouterEnrichmentFetcher;
    compressionMetaFetcher?: AeraRouterCompressionMetaFetcher;
    providersFetcher?: AeraRouterProvidersFetcher;
    diskSnapshotReader?: AeraRouterDiskSnapshotReader;
    diskSnapshotWriter?: AeraRouterDiskSnapshotWriter;
    now?: () => number;
    cache?: AeraRouterFetchCache;
  ...
  return async (input: Config) => {
    ...
    if (existingProviders && existingProviders[resolved.providerId] !== undefined) {
      logger.warn(
        `[aera-router-plugin] config shim skipped: provider.${resolved.providerId} already set by user`
      );
      return;
    }
    ...
    if (authJson === null) {
      logger.warn(
        "[aera-router-plugin] config shim: auth.json failed to parse; treating as missing"
      );
      authJson = undefined;
    }

    // Try both prefixed (e.g. opencode-aera-router) and unprefixed (e.g. aera-router)
    // keys so a user who ran `/connect aera-router` before the auto-prefix fix
    ...
    if (!apiKey) {
      ...
      logger.warn(
        `[aera-router-plugin] config shim skipped: no apiKey for providerId=${resolved.providerId}`
      );
      return;
    }
    // Management-plane catalog reads may use a narrower read-only token.
    ...
    if (!baseURL) {
      logger.warn(
        `[aera-router-plugin] config shim skipped: no baseURL for providerId=${resolved.providerId}`
      );
      return;
    }
    ...
    if (cached && cached.expiresAt > t) {
      ...
      try {
        ...
        logger.warn(
          "[aera-router-plugin] config shim: /v1/models fetch failed; publishing stub provider entry",
          err
        );
        rawModels = [];
      ...
      try {
        ...
        logger.warn(
          "[aera-router-plugin] config shim: /api/combos fetch failed; publishing models-only static catalog",
          err
        );
      }
      ...
      if (wantEnrichment) {
        try {
          ...
          logger.warn(
            "[aera-router-plugin] config shim: /api/pricing/models fetch failed; publishing raw-id static catalog",
            err
          );
        }
      }
      ...
      if (wantCompressionMeta) {
        try {
          ...
          logger.warn(
            "[aera-router-plugin] config shim: /api/context/combos fetch failed; publishing combos without compression suffix",
            err
          );
        }
      }
      ...
      if (wantUsableOnly) {
        try {
          ...
          logger.warn(
            "[aera-router-plugin] config shim: /api/providers fetch failed; usableOnly filter disabled for this refresh",
            err
          );
        }
      }
      ...
      if (modelsFetchThrew && wantDiskCache) {
        ...
        if (snapshot && snapshot.rawModels.length > 0) {
          logger.warn(
            `[aera-router-plugin] config shim: /v1/models unreachable; using stale disk cache (${snapshot.rawModels.length} models)`
          );
    ...
    if (features.mcpAutoEmit === true) {
      ...
      if (!mcpKey) {
        logger.warn(
          `[aera-router-plugin] mcp auto-emit skipped: no Bearer token for providerId=${resolved.providerId}`
        );
      } else {
        ...
        if (inputWithMcp.mcp[resolved.providerId] !== undefined) {
          logger.warn(
            `[aera-router-plugin] mcp auto-emit skipped: mcp.${resolved.providerId} already set by user`
          );
        } else {
