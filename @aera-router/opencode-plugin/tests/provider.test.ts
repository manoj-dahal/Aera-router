 ...
 *
 * Covers `createAeraRouterProviderHook(opts, deps)`:
...
const FIXTURE: AeraRouterRawModelEntry[] = [
  {
    id: "claude-primary",
    object: "model",
    owned_by: "combo",
...
function stubFetcher(payload: AeraRouterRawModelEntry[]): AeraRouterModelsFetcher & {
  callCount: () => number;
...
test("createAeraRouterProviderHook: default providerId is 'aera-router'", () => {
  const hook = createAeraRouterProviderHook(undefined, { combosFetcher: async () => [] });
  assert.equal(hook.id, "opencode-aera-router");
});

test("createAeraRouterProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  const a = createAeraRouterProviderHook(
    { providerId: "aera-router-preprod" },
    { combosFetcher: async () => [] }
  );
  const b = createAeraRouterProviderHook(
    { providerId: "aera-router-local" },
    { combosFetcher: async () => [] }
  );
  assert.equal(a.id, "opencode-aera-router-preprod");
  assert.equal(b.id, "opencode-aera-router-local");
});
...
test("models: extracts apiKey from ctx.auth (type=api) and calls fetcher with it", async () => {
  ...
  // ("aera-router"), not the OC-gate-prefixed hook.id ("opencode-aera-router") —
  // that prefix must never leak into anything Aera Router's server parses.
...
test("models: returns {} when no baseURL resolvable (no opts.baseURL and no auth.baseURL)", async () => {
  const fetcher = stubFetcher(FIXTURE);
  const hook = createAeraRouterProviderHook({}, { fetcher, combosFetcher: async () => [] });
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  // aeraRouterProviderId ("aera-router") — the OC-gate prefix ("opencode-")
  // must stay OC-internal (hook.id / AuthHook.provider) and never leak into
  // anything Aera Router's own server parses for credential lookup.
  const claude = out["aera-router/claude-primary"];
  ...
  assert.equal(claude.id, "aera-router/claude-primary");
  assert.equal(claude.name, "claude-primary");
  assert.equal(claude.providerID, "aera-router");
  ...
  // cost is zeroed (Aera Router /v1/models has no pricing)
