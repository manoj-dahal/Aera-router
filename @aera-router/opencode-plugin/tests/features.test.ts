 ...
 *
 * Surfaces tested:
 *   - `parseAeraRouterPluginOptions({ features: ... })`  → schema accept/reject
 *   - `applyEnrichment(model, entry)`                   → mutation semantics
 *   - `formatCompressionPipeline(steps)`                → display formatting
 *   - `createAeraRouterProviderHook` with mocked
 *     `enrichmentFetcher` / `compressionMetaFetcher`    → overlay applied,
 *                                                         off-by-default
 *                                                         gating works.
 *   - `createAeraRouterConfigHook` with `features.mcpAutoEmit:true`
 *                                                       → emits mcp entry
 *                                                       → falls back to
 *                                                         provider apiKey
 *                                                         when mcpToken
...
import {
  ...
  defaultAeraRouterEnrichmentFetcher,
  defaultAeraRouterCompressionMetaFetcher,
  ...
  type AeraRouterCompressionCombo,
...
test("parseAeraRouterPluginOptions: empty features object → preserved", () => {
  const r = parseAeraRouterPluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseAeraRouterPluginOptions: all boolean features set → preserved", () => {
  const r = parseAeraRouterPluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseAeraRouterPluginOptions: mcpToken string → preserved", () => {
  const r = parseAeraRouterPluginOptions({
...
test("parseAeraRouterPluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseAeraRouterPluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @aera-router\/opencode-plugin options/
  );
});

test("parseAeraRouterPluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseAeraRouterPluginOptions({
...
test("parseAeraRouterPluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseAeraRouterPluginOptions({ features: { mcpToken: "" } }),
    /Invalid @aera-router\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: AeraRouterRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  ...
  // #6859: dynamic-hook catalog keys use the unprefixed aeraRouterProviderId.
  const m = out["aera-router/claude-sonnet-4-6"];
...
test("provider hook: enrichment fetcher NOT called when features.enrichment:false", async () => {
  ...
  assert.equal(out["aera-router/claude-sonnet-4-6"].name, "claude-sonnet-4-6", "raw id preserved");
});
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: AeraRouterCompressionCombo[] = [
    {
      id: "default-caveman",
  ...
  const combo = out["aera-router/claude-primary"];
...
const stubAuthJson = (apiKey: string) => async () => ({
  "opencode-aera-router": { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.["opencode-aera-router"], "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-aera-router"] as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-aera-router"] as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: {
      "opencode-aera-router": { type: "custom-user-entry", url: "https://manual.example/mcp" },
    },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.["opencode-aera-router"],
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createAeraRouterConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-aera-router-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-aera-router-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultAeraRouterEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultAeraRouterEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultAeraRouterEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultAeraRouterEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultAeraRouterCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultAeraRouterCompressionMetaFetcher("", "sk", 100);
...
test("defaultAeraRouterEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultAeraRouterEnrichmentFetcher(
...
test("defaultAeraRouterEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
...
test("defaultAeraRouterEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
): AeraRouterEnrichmentMap {
  const map: AeraRouterEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: AeraRouterRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as AeraRouterRawModelEntry,
    { id: "claude/claude-opus-4-7" } as AeraRouterRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: AeraRouterRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as AeraRouterRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: AeraRouterRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as AeraRouterRawModelEntry,
    { id: "claude/claude-opus-4-7" } as AeraRouterRawModelEntry,
    { id: "cx/gpt-5.5" } as AeraRouterRawModelEntry,
    { id: "codex/gpt-5.5" } as AeraRouterRawModelEntry,
    { id: "pol/openai-large" } as AeraRouterRawModelEntry,
    { id: "pollinations/openai-large" } as AeraRouterRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);
