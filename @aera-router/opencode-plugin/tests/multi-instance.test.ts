 ...
 *
 * Validates that two `AeraRouterPlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@aera-router/opencode-plugin", {"providerId": "aera-router-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@aera-router/opencode-plugin", {"providerId": "aera-router-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { AeraRouterPlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof AeraRouterPlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await AeraRouterPlugin(fakeInput, {
    providerId: "aera-router-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await AeraRouterPlugin(fakeInput, {
    providerId: "aera-router-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "opencode-aera-router-prod");
  assert.equal(b.auth?.provider, "opencode-aera-router-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await AeraRouterPlugin(fakeInput, { ...opts });
  const second = await AeraRouterPlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => AeraRouterPlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await AeraRouterPlugin(fakeInput, {
