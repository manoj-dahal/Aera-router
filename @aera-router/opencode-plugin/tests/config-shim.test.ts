 ...
 *
 * Covers `createAeraRouterConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@aera-router/opencode-provider`).
...
import {
  ...
  createAeraRouterConfigHook,
  createAeraRouterProviderHook,
  AeraRouterPlugin,
  resolveAeraRouterPluginOptions,
  type AeraRouterCombosFetcher,
  type AeraRouterEnrichmentEntry,
  type AeraRouterEnrichmentFetcher,
  type AeraRouterEnrichmentMap,
  type AeraRouterFetchCache,
  type AeraRouterModelsFetcher,
  type AeraRouterProviderConnection,
  type AeraRouterProvidersFetcher,
  type AeraRouterRawCombo,
  type AeraRouterRawModelEntry,
  type AeraRouterReadAuthJson,
  type AeraRouterStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: AeraRouterRawModelEntry = {
...
const MODEL_GEMINI: AeraRouterRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: AeraRouterRawCombo = {
...
): AeraRouterReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: AeraRouterReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): AeraRouterReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: AeraRouterReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: AeraRouterRawModelEntry[]
): AeraRouterModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AeraRouterModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: AeraRouterRawCombo[]
): AeraRouterCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AeraRouterCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): AeraRouterModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: AeraRouterModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): AeraRouterCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: AeraRouterCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: AeraRouterEnrichmentMap): AeraRouterEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AeraRouterEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): AeraRouterEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: AeraRouterEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createAeraRouterConfigHook(
  ...
  const provider = (input as { provider: Record<string, AeraRouterStaticProviderEntry> }).provider;
  const entry = provider["opencode-aera-router"];
  assert.ok(entry, "input.provider['opencode-aera-router'] set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "Aera Router");
  ...
  const claude = entry.models["opencode-aera-router/claude-sonnet-4-6"];
  ...
  const combo = entry.models["opencode-aera-router/claude-tier"];
...
//     prefixed (`opencode-aera-router`). Without the fallback the lookup misses
...
test("config: auth.json under bare key (pre-prefix login) resolves via dual-key fallback", async () => {
  // Stored under bare `aera-router` (the key OC wrote before the auto-prefix fix),
  // but the resolved providerId is now `opencode-aera-router`.
  const readAuthJson = stubReadAuthJson({
    "aera-router": { type: "api", key: "sk-bare-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router" }, // resolves to opencode-aera-router internally
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: prefixed key wins over bare key when both present (dual-key precedence)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "api", key: "sk-prefixed", baseURL: "https://pref.example/v1" },
    "aera-router": { type: "api", key: "sk-bare", baseURL: "https://bare.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const entry = (input as { provider: Record<string, AeraRouterStaticProviderEntry> }).provider[
    "opencode-aera-router"
  ];
  assert.ok(entry);
  assert.equal(
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  const manual = {
    ...
    name: "Manual Aera Router",
    options: { baseURL: "http://manual/v1", apiKey: "manual-key" },
    models: { "manual-model": { name: "manual-model" } },
  };
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "api", key: "sk-test", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const input = makeInput({ "opencode-aera-router": manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider["opencode-aera-router"], manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: combos fetcher throws → emit models-only catalog (no combos in models block)", async () => {
  ...
  assert.deepEqual(ids, [
    "opencode-aera-router/claude-sonnet-4-6",
    "opencode-aera-router/gemini-3-flash",
  ]);
  assert.equal(entry.models["opencode-aera-router/claude-tier"], undefined, "no combo entry");
  assert.ok(
    logger.entries.some((e) => String(e[0]).includes("/api/combos fetch failed")),
    "combos-fetch breadcrumb emitted"
  );
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router-prod": {
      type: "api",
      key: "sk-prod",
      baseURL: "https://prod.example/v1",
    },
    "opencode-aera-router-preprod": {
      type: "api",
      key: "sk-preprod",
  ...
  const hookA = createAeraRouterConfigHook(
    { providerId: "aera-router-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createAeraRouterConfigHook(
    { providerId: "aera-router-preprod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  ...
  const provider = (input as { provider: Record<string, AeraRouterStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-aera-router-prod"], "prod block present");
  assert.ok(provider["opencode-aera-router-preprod"], "preprod block present");
  assert.equal(provider["opencode-aera-router-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["opencode-aera-router-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["opencode-aera-router-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(
    provider["opencode-aera-router-preprod"].options.baseURL,
    "https://preprod.example/v1"
  );
  assert.notEqual(
    provider["opencode-aera-router-prod"],
    provider["opencode-aera-router-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createAeraRouterConfigHook(
    { providerId: "aera-router", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createAeraRouterProviderHook(
    { providerId: "aera-router", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: AeraRouterFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @aera-router/opencode-provider", () => {
  const resolved = resolveAeraRouterPluginOptions({
    providerId: "aera-router",
    displayName: "Aera Router",
  });
  ...
  assert.equal(block.name, "Aera Router");
  ...
  const claude = block.models["opencode-aera-router/claude-sonnet-4-6"];
...
test("buildStaticProviderEntry: empty fetch results → stub block with models: {}", () => {
  const resolved = resolveAeraRouterPluginOptions({ providerId: "aera-router" });
...
test("buildStaticProviderEntry: hidden combos are excluded", () => {
  ...
  assert.equal(block.models["opencode-aera-router/claude-tier"], undefined);
  assert.ok(block.models["opencode-aera-router/claude-sonnet-4-6"]);
});
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveAeraRouterPluginOptions({ providerId: "aera-router" });
  const withDate: AeraRouterRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
  ...
  assert.equal(block.models["opencode-aera-router/claude-with-date"].release_date, "2026-02-19");
  assert.equal(block.models["opencode-aera-router/gemini-3-flash"].release_date, undefined);
});

test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveAeraRouterPluginOptions({ providerId: "aera-router" });
  const TEXT_ONLY: AeraRouterRawModelEntry = {
    id: "text-only",
  ...
  const combo = block.models["opencode-aera-router/mixed-tier"];
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: AeraRouterPlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("AeraRouterPlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await AeraRouterPlugin({} as never, { providerId: "aera-router" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-aera-router": { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  assert.ok(provider!["opencode-aera-router"]);
});
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, AeraRouterEnrichmentEntry>([
  ...
  assert.ok(entry);
  assert.equal(entry.models["opencode-aera-router/claude-sonnet-4-6"].name, "Claude Sonnet 4.6");
  assert.equal(entry.models["opencode-aera-router/gemini-3-flash"].name, "Gemini 3 Flash");
  // Combo names still come from /api/combos — enrichment overlay does NOT touch combos.
  assert.equal(entry.models["opencode-aera-router/claude-tier"].name, "Claude Tier");
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
  ...
  assert.equal(
    entry.models["opencode-aera-router/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id retained"
  );
});
...
function stubProvidersFetcher(
  payload: AeraRouterProviderConnection[]
): AeraRouterProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: AeraRouterProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): AeraRouterProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: AeraRouterProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
const MODEL_CC_OPUS: AeraRouterRawModelEntry = {
  id: "cc/claude-opus-4-7",
  capabilities: { tool_calling: true, reasoning: true, temperature: true },
  context_length: 200_000,
};
const MODEL_NV_LLAMA: AeraRouterRawModelEntry = {
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
  ...
  assert.ok(
    entry.models["opencode-aera-router/claude-sonnet-4-6"],
...
test("config: providerTag (default-on) prepends '<provider> - ' to enriched raw-model names", async () => {
  ...
  assert.equal(entry.models["opencode-aera-router/gemini-3-flash"].name, "Gemini - Gemini 3 Flash");
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot Aera Router hasn't curated a human label for yet. We still
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createAeraRouterConfigHook(
    { providerId: "aera-router", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, AeraRouterStaticProviderEntry> }).provider[
    "opencode-aera-router"
  ];
  assert.equal(
    entryA.models["opencode-aera-router/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, AeraRouterStaticProviderEntry> }).provider[
    "opencode-aera-router"
  ];
  assert.equal(
    entryB.models["opencode-aera-router/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
});
...
test("buildStaticProviderEntry: nested combo-ref context is the bottleneck across the graph", () => {
  const resolved = resolveAeraRouterPluginOptions({ providerId: "aera-router" });
  const rawModels: AeraRouterRawModelEntry[] = [
    {
      id: "raw-big",
      context_length: 200_000,
      max_output_tokens: 64_000,
  ...
  const rawCombos: AeraRouterRawCombo[] = [
    {
      id: "tiny-combo",
      name: "TinyCombo",
  ...
  const parent = block.models["opencode-aera-router/parent"];
