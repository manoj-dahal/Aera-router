 ...
 *
 * Covers `createAeraRouterFetchInterceptor` (URL-prefix gating, header merge,
...
import { createAeraRouterAuthHook, createAeraRouterFetchInterceptor } from "../src/index.js";
...
test("createAeraRouterFetchInterceptor: targets baseURL → Authorization header injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAeraRouterFetchInterceptor: path-prefixed baseURL scopes auth to its normalized inference paths", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const prefixedBase = "https://or.example.com/tenant-a/v1";
    const f = createAeraRouterFetchInterceptor({
      apiKey: KEY,
    ...
    const suffixingInterceptor = createAeraRouterFetchInterceptor({
      apiKey: KEY,
...
test("createAeraRouterFetchInterceptor: targets baseURL → Authorization OVERRIDES caller-supplied Bearer", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createAeraRouterFetchInterceptor: targets baseURL + body → Content-Type defaults to application/json", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createAeraRouterFetchInterceptor: caller-set Content-Type is NOT overwritten", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAeraRouterFetchInterceptor: non-baseURL host → passthrough, no Authorization injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAeraRouterFetchInterceptor: refuses suffix-spoof — `${base}-attacker.evil` does NOT match baseURL", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAeraRouterFetchInterceptor: URL object input is handled", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAeraRouterFetchInterceptor: Request input is handled (reads .url)", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAeraRouterFetchInterceptor: trailing slash in baseURL is normalized", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({
      apiKey: KEY,
...
test("createAeraRouterFetchInterceptor: GET without body does NOT set Content-Type", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAeraRouterFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("loader: returns fetch fn when apiKey + baseURL both present (via opts)", async () => {
  const hook = createAeraRouterAuthHook({ baseURL: BASE });
...
test("loader: returns fetch fn when baseURL is stashed on the auth credential", async () => {
  ...
  const hook = createAeraRouterAuthHook();
...
test("loader: omits fetch fn when baseURL missing (apiKey-only return)", async () => {
  const hook = createAeraRouterAuthHook(); // no baseURL opt
  const result = await hook.loader!(async () => ({ type: "api", key: KEY }) as never, {} as never);
