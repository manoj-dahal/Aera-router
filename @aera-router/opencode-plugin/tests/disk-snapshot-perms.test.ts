...
import {
  ...
  diskSnapshotPath,
  type AeraRouterFetchCacheEntry,
} from "../src/index.js";

function makeEntry(): Omit<AeraRouterFetchCacheEntry, "expiresAt"> {
  return {
    rawModels: [],
    rawCombos: [],
...
test("defaultDiskSnapshotWriter writes an owner-only (no group/other) snapshot", async (t) => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-disk-perms-"));
