...
const RAW_MODELS: AeraRouterRawModelEntry[] = [
  {
    id: "openai/gpt-test",
...
test("options: managementReadToken is accepted and preserved", () => {
  const parsed = parseAeraRouterPluginOptions({ managementReadToken: MANAGEMENT_READ_TOKEN });
...
test("provider hook: management GET fetchers use managementReadToken while /v1 uses apiKey", async () => {
  ...
  const enrichmentFetcher: AeraRouterEnrichmentFetcher = async (_baseURL, token) => {
    calls.push(["pricing", token]);
    return new Map();
  };
  const compressionMetaFetcher: AeraRouterCompressionMetaFetcher = async (_baseURL, token) => {
    calls.push(["context", token]);
    return [];
  };
  const providersFetcher: AeraRouterProvidersFetcher = async (_baseURL, token) => {
...
test("config hook: managementReadToken stays out of provider inference and MCP config", async () => {
  ...
  const hook = createAeraRouterConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-aera-router": { type: "api" as const, key: API_KEY },
      }),
      fetcher: async (_baseURL, token) => {
        calls.push(["models", token]);
        return RAW_MODELS;
  ...
  assert.equal(input.provider?.["opencode-aera-router"]?.options?.apiKey, API_KEY);
  assert.equal(
    input.mcp?.["opencode-aera-router"]?.headers?.Authorization,
    `Bearer ${API_KEY}`,
...
test("disk cache: snapshot written under management token A is rejected under token B", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-token-snapshot-"));
  ...
  try {
    ...
    const tokenAHook = createAeraRouterConfigHook(
    ...
    const tokenBHook = createAeraRouterConfigHook(
    ...
    assert.deepEqual(
      input.provider?.["opencode-aera-router"]?.models,
      {},
