 ...
 *
 * Covers:
 *   - `defaultAeraRouterCombosFetcher(baseURL, apiKey, timeoutMs?)`
 ...
 *   - `createAeraRouterProviderHook(opts, deps)` extension
...
import {
  ...
  defaultAeraRouterCombosFetcher,
  mapComboToModelV2,
...
const MODEL_PRIMARY: AeraRouterRawModelEntry = {
  id: "claude-primary",
  capabilities: {
    tool_calling: true,
    reasoning: true,
...
const MODEL_SECONDARY: AeraRouterRawModelEntry = {
  id: "claude-secondary",
  capabilities: {
    tool_calling: true,
    reasoning: false,
...
const MODEL_NO_TOOLS: AeraRouterRawModelEntry = {
  id: "gemini-3-flash",
...
): AeraRouterModelsFetcher & { callCount: () => number } {
  let n = 0;
...
function failingCombosFetcher(
  err = new Error("boom")
): AeraRouterCombosFetcher & { callCount: () => number } {
  let n = 0;
...
// ────────────────────────────────────────────────────────────────────────────
// defaultAeraRouterCombosFetcher — envelope tolerance + error surfacing
// ────────────────────────────────────────────────────────────────────────────

test("defaultAeraRouterCombosFetcher: parses {combos:[…]} envelope", async () => {
  ...
  try {
    const combos = await defaultAeraRouterCombosFetcher("https://or.example.com", "sk-test");
...
test("defaultAeraRouterCombosFetcher: parses bare array envelope", async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    const combos = await defaultAeraRouterCombosFetcher("https://or.example.com/v1", "sk-test");
...
test("defaultAeraRouterCombosFetcher: strips trailing /v1 before /api/combos", async () => {
  const originalFetch = globalThis.fetch;
  let observedUrl = "";
  globalThis.fetch = (async (input: unknown) => {
  ...
  try {
    await defaultAeraRouterCombosFetcher("https://or.example.com/v1/", "sk-test");
...
test("defaultAeraRouterCombosFetcher: throws on non-2xx with status code in message", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (async () => {
  ...
  try {
    await assert.rejects(
      async () => {
        await defaultAeraRouterCombosFetcher("https://or.example.com", "sk-bad");
      },
      (err: unknown) => {
...
test("defaultAeraRouterCombosFetcher: throws when apiKey missing", async () => {
  await assert.rejects(
    async () => defaultAeraRouterCombosFetcher("https://or.example.com", ""),
    /apiKey required/
  );
});

test("defaultAeraRouterCombosFetcher: throws when baseURL missing", async () => {
  await assert.rejects(
    async () => defaultAeraRouterCombosFetcher("", "sk-test"),
    /baseURL required/
  );
});
...
test("mapComboToModelV2: api block matches providerId + baseURL", () => {
  ...
  assert.equal(m.providerID, "aera-router-preprod");
...
test("mapComboToModelV2: explicit member temperature=false drops combo temperature=false", () => {
  const tempFalse: AeraRouterRawModelEntry = {
    id: "no-temp",
...
// ────────────────────────────────────────────────────────────────────────────
// createAeraRouterProviderHook — combos merge + collision + soft-fail + cache
// ────────────────────────────────────────────────────────────────────────────
...
test("models() returns combo entries merged into the map", async () => {
  ...
  assert.ok(out["aera-router/claude-primary"]);
  assert.ok(out["aera-router/claude-secondary"]);
  assert.ok(out["aera-router/gemini-3-flash"]);
  assert.ok(out["aera-router/claude-tier"]);

  const combo = out["aera-router/claude-tier"];
  assert.equal(combo.name, "Claude Tier");
  assert.equal(combo.providerID, "aera-router");
...
test("models(): combo with unknown member ids degrades to all-false LCD posture", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["aera-router/phantom-combo"]);
  // With zero resolvable members, LCD = all-false (defensive posture).
  assert.equal(out["aera-router/phantom-combo"].capabilities.toolcall, false);
  assert.equal(out["aera-router/phantom-combo"].capabilities.reasoning, false);
  assert.equal(out["aera-router/phantom-combo"].limit.context, 0);
});
...
test("models(): hidden combos are excluded from the map", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["aera-router/visible"]);
  assert.ok(!out["aera-router/hidden"], "hidden combo must be omitted");
});
...
test("models(): combo name exactly matches raw model id → raw deleted, raw deleted, no warn", async () => {
  ...
  // the real Aera Router payload where /v1/models pre-mirrors combos as
  // no-slash raw entries whose ids match /api/combos friendly names.
  const colliderCombo: AeraRouterRawCombo = {
    id: "uuid-collider",
  ...
  assert.ok(out["aera-router/claude-primary"], "combo surfaces under prefixed key");
  assert.equal(out["aera-router/claude-primary"].name, "claude-primary");
...
test("models(): two combos with same slug → second gets disambiguator suffix", async () => {
  ...
  const combos: AeraRouterRawCombo[] = [
    {
      id: "uuid-a",
      name: "Claude",
  ...
  assert.ok(out["aera-router/claude"], "first combo at prefixed slug");
  assert.ok(out["aera-router/claude-uuid"], "second combo disambiguated by id prefix");
});
...
test("models(): combos cached + reused within TTL (one combo fetch per TTL window)", async () => {
  ...
  assert.ok(second["aera-router/claude-tier"]);
});
...
test("models(): nested combo-ref context is the min of nested + raw members", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  const masterLight = out["aera-router/master-light"];
