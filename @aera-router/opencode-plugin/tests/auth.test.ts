 ...
 *
 * Covers the `createAeraRouterAuthHook(opts)` factory and its loader behaviour
...
import { createAeraRouterAuthHook } from "../src/index.js";

test("createAeraRouterAuthHook: default providerId is 'aera-router'", () => {
  const hook = createAeraRouterAuthHook();
  assert.equal(hook.provider, "opencode-aera-router");
});

test("createAeraRouterAuthHook: custom providerId binds to hook.provider (multi-instance)", () => {
  const hook = createAeraRouterAuthHook({ providerId: "aera-router-preprod" });
  assert.equal(hook.provider, "opencode-aera-router-preprod");
});

test("createAeraRouterAuthHook: methods[0] is type 'api' with label including displayName", () => {
  const hook = createAeraRouterAuthHook();
  ...
  assert.equal(m.label, "Aera Router API Key");

  const custom = createAeraRouterAuthHook({ providerId: "aera-router-preprod" });
  assert.equal(custom.methods[0].label, "Aera Router (opencode-aera-router-preprod) API Key");
});

test("createAeraRouterAuthHook: prompts[0] uses key='apiKey' per @opencode-ai/plugin contract", () => {
  ...
  assert.ok(
    typeof (p as { message: string }).message === "string" &&
      (p as { message: string }).message.includes("aera-router"),
...
test("loader: valid api auth → {apiKey, baseURL, fetch} when baseURL option set (T-04)", async () => {
  const hook = createAeraRouterAuthHook({ baseURL: "https://or.example.com/v1" });
  const result = await hook.loader!(
