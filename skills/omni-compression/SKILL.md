...
<!-- skill:custom-start -->
<!-- Migrated from skills/aera-router-compression/SKILL.md (preserved curated content) -->
...
Aera Router compresses token payloads before forwarding to providers. No code changes required — set it once, it applies to all requests transparently.
...
```bash
curl $AERA_ROUTER_URL/api/settings/compression \
...
```bash
curl -X PUT $AERA_ROUTER_URL/api/settings/compression \
  -H "Authorization: Bearer $AERA_ROUTER_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST $AERA_ROUTER_URL/api/compression/preview \
  -H "Authorization: Bearer $AERA_ROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "rtk",
...
When Aera Router is used with browser/Playwright MCP tools, it automatically compresses verbose accessibility-tree tool results. Enabled by default; configure thresholds:
...
```
aera_router_compression_status     → current settings + savings analytics
aera_router_compression_configure  → update mode/threshold/language
aera_router_set_compression_engine → switch engine at runtime
```
