...
<!-- skill:custom-start -->
<!-- Migrated from skills/aera-router-monitoring/SKILL.md (preserved curated content) -->

# Aera Router — Monitoring & Health
...
```bash
curl $AERA_ROUTER_URL/api/health \
...
```bash
curl $AERA_ROUTER_URL/api/monitoring/health \
  -H "Authorization: Bearer $AERA_ROUTER_KEY"
```
...
```bash
curl $AERA_ROUTER_URL/api/providers/metrics \
  -H "Authorization: Bearer $AERA_ROUTER_KEY"
```
...
## Via MCP (if Aera Router is your MCP server)

```
aera_router_get_health            → full system health snapshot
aera_router_get_provider_metrics  → p50/p95/p99 + circuit state per provider
aera_router_get_session_snapshot  → cost, tokens, errors for current session
aera_router_check_quota           → quota balance + percent remaining + reset time
aera_router_db_health_check       → diagnose + auto-repair database drift
```

## Quota check

```bash
curl $AERA_ROUTER_URL/api/quota \
  -H "Authorization: Bearer $AERA_ROUTER_KEY"
```
...
```bash
curl -X POST $AERA_ROUTER_URL/api/budget/guard \
  -H "Authorization: Bearer $AERA_ROUTER_KEY" \
...
Aera Router logs every MCP tool call to `mcp_audit` table. Query via API:

```bash
curl "$AERA_ROUTER_URL/api/mcp/status" \
  -H "Authorization: Bearer $AERA_ROUTER_KEY"
```
...
- `503` on health endpoint → Aera Router is starting up; retry in 5s
