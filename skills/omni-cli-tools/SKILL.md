---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend Aera Router's automation surface."
---
...
Read Crush CLI Aera Router config

Local-only. Reads the Aera Router provider block in Crush's config.
...
Write Crush CLI Aera Router config

Local-only. Registers Aera Router as an `openai-compat` provider in Crush's config.
...
Remove Aera Router from Crush CLI config

Local-only. Removes the Aera Router provider block from Crush's config.
...
Read CodeWhale CLI Aera Router config

Local-only. Reads the Aera Router config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI Aera Router config

Local-only. Writes the Aera Router config block in CodeWhale TOML format.
...
Remove Aera Router from CodeWhale CLI config

Local-only. Removes the Aera Router config block from CodeWhale's config.
