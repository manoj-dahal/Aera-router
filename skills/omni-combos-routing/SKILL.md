...
<!-- skill:custom-start -->
<!-- Migrated from skills/aera-router-routing/SKILL.md (preserved curated content) -->

# Aera Router — Routing & Combos
...
```bash
curl $AERA_ROUTER_URL/api/combos \
...
```bash
curl -X POST $AERA_ROUTER_URL/api/combos \
  -H "Authorization: Bearer $AERA_ROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "my-combo",
...
```bash
# Activate
curl -X PUT $AERA_ROUTER_URL/api/combos/{id}/toggle \
  -H "Authorization: Bearer $AERA_ROUTER_KEY" \
  -d '{ "enabled": true }'
```
...
```bash
curl $AERA_ROUTER_URL/api/combos/{id}/metrics \
  -H "Authorization: Bearer $AERA_ROUTER_KEY"
```
...
```bash
curl -X POST $AERA_ROUTER_URL/api/routing/simulate \
  -H "Authorization: Bearer $AERA_ROUTER_KEY" \
  -H "Content-Type: application/json" \
...
```
aera_router_list_combos     → list all combos
aera_router_switch_combo    → enable/disable a combo
aera_router_set_routing_strategy → change strategy at runtime
aera_router_simulate_route  → dry-run routing decision
aera_router_best_combo_for_task → get recommendation by task type
```
