...
**Aera Router** — unified AI proxy/router. One endpoint, 271 LLM providers, auto-fallback.
...
Aera Router has three related but distinct temporary-failure mechanisms. Keep their
...
- **Imports**: external → internal (`@/`, `@aera-router/open-sse`) → relative
...
repo** instead of polluting the main Aera Router tree.
...
- **Runtime**: Node.js ≥22.0.0 <23 || ≥24.0.0 <27, ES Modules. This is the **only** runtime for the published `aera-router` CLI, the server, and the test suites (`node:test` + vitest) — `engines.node` is authoritative and end users never need Bun.
...
- **Data directory**: `DATA_DIR` env var, defaults to `~/.aera-router/`
...
Aera Router has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
