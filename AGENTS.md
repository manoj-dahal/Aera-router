...
- **Database**: better-sqlite3 (SQLite) — `DATA_DIR` configurable, default `~/.aera-router/`
...
`/usr/lib/node_modules/aera-router/app/` directory (VPS image path is unchanged).
...
- Path aliases: `@/*` → `src/`, `@aera-router/open-sse` → `open-sse/`, `@aera-router/open-sse/*` → `open-sse/*`
...
- **Order**: external → internal (`@/`, `@aera-router/open-sse`) → relative (`./`, `../`)
...
- **`core.ts`**: `getDbInstance()` returns a singleton `better-sqlite3` instance with WAL
  journaling. `SCHEMA_SQL` defines **17 base tables** (verify with `grep -c "CREATE TABLE" src/lib/db/core.ts` minus 1 for the bookkeeping `_aera_router_migrations` table). Helpers: `rowToCamel`, `encryptConnectionFields`.
- **`migrationRunner.ts`**: Applies versioned SQL files from `db/migrations/` inside transactions.
  Tracks applied migrations in `_aera_router_migrations` table.
...
- **Transports**: stdio (CLI `aera-router --mcp`), SSE (`/api/mcp/sse`), Streamable HTTP
...
- **Built-in skills**: Ship with Aera Router (e.g., quota management, routing). Located
...
This repository is a fork of `manoj-dahal/Aera-router`. Keep fork-only operational
