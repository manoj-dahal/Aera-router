...
Emits [Langfuse](https://langfuse.com/) generation traces for every LLM completion routed through Aera Router.
...
Copy the `examples/plugins/langfuse/` directory to your Aera Router plugins path, or install directly from the marketplace UI.
...
- **Trace:** `aera-router:<model>` with `userId`, `provider`, `requestId` metadata
...
- API keys are stored in the plugin config, encrypted at rest by Aera Router
...
- Aera Router plugin SDK: `docs/frameworks/PLUGIN_SDK.md`
