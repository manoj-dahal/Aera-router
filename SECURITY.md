...
If you discover a security vulnerability in Aera Router, please report it responsibly:

1. **DO NOT** open a public GitHub issue
2. Use [GitHub Security Advisories](https://github.com/manoj-dahal/Aera-router/security/advisories/new)
...
Aera Router implements a multi-layered security model:
...
Aera Router ships a hot-reloadable **guardrails registry** (`src/lib/guardrails/`) with 3 built-in guardrails ordered by priority:
...
Custom guardrails register via `registerGuardrail(new MyGuardrail())`. The model is fail-open (exceptions never block traffic). Per-request opt-out via `x-aera-router-disabled-guardrails` header. → See [`docs/security/GUARDRAILS.md`](docs/security/GUARDRAILS.md).
...
The published `aera-router` npm artifact bundles the Next.js `output: "standalone"`
...
`AERA_ROUTER_BUILD_PROFILE=minimal npm run build`. That replaces the four
