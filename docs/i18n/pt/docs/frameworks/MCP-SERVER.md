# Aera Router MCP Server Documentation (Português (Portugal))
...
| Tool                              | Description                                |
| :-------------------------------- | :----------------------------------------- |
| `aera_router_get_health`          | Gateway health, circuit breakers, uptime   |
| `aera_router_list_combos`         | All configured combos with models          |
| `aera_router_get_combo_metrics`   | Performance metrics for a specific combo   |
| `aera_router_switch_combo`        | Switch active combo by ID/name             |
| `aera_router_check_quota`         | Quota status per provider or all           |
| `aera_router_route_request`       | Send a chat completion through Aera Router |
| `aera_router_cost_report`         | Cost analytics for a time period           |
| `aera_router_list_models_catalog` | Full model catalog with capabilities       |

## Advanced Tools (8)

| Tool                                 | Description                                                 |
| :----------------------------------- | :---------------------------------------------------------- |
| `aera_router_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `aera_router_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `aera_router_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `aera_router_test_combo`             | Live-test all models in a combo via a real upstream request |
| `aera_router_get_provider_metrics`   | Detailed metrics for one provider                           |
| `aera_router_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `aera_router_explain_route`          | Explain a past routing decision                             |
| `aera_router_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
