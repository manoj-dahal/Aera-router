...
**Aera Router** — jednotný AI proxy/router. Jeden koncový bod, 160+ poskytovatelů LLM, automatické zálohování.
...
Aera Router má tři související, ale odlišné mechanismy dočasného selhání. Udržujte jejich rozsah oddělený při ladění chování routování. Viz
...
- **Importy**: externí → interní (`@/`, `@aera-router/open-sse`) → relativní
...
- **Cestovní aliasy**: `@/*` → `src/`, `@aera-router/open-sse` → `open-sse/`, `@aera-router/open-sse/*` → `open-sse/*`
- **Výchozí port**: 20128 (API + dashboard na stejném portu)
- **Adresář dat**: `DATA_DIR` env var, výchozí hodnota `~/.aera-router/`
