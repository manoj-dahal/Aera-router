# Dokumentasi Server MCP Aera Router (Bahasa Indonesia)
...
Aera Router MCP sudah tersedia secara bawaan. Jalankan dengan:
...
| Alat                              | Deskripsi                                       |
| :-------------------------------- | :---------------------------------------------- |
| `aera_router_get_health`          | Kesehatan gateway, pemutus sirkuit, uptime      |
| `aera_router_list_combos`         | Semua combo yang dikonfigurasi beserta modelnya |
| `aera_router_get_combo_metrics`   | Metrik performa untuk combo tertentu            |
| `aera_router_switch_combo`        | Ganti combo aktif berdasarkan ID/nama           |
| `aera_router_check_quota`         | Status kuota per penyedia atau semua penyedia   |
| `aera_router_route_request`       | Kirim penyelesaian chat melalui Aera Router     |
| `aera_router_cost_report`         | Analitik biaya untuk periode waktu tertentu     |
| `aera_router_list_models_catalog` | Katalog model lengkap beserta kemampuannya      |

## Alat Lanjutan (8)

| Alat                                 | Deskripsi                                                              |
| :----------------------------------- | :--------------------------------------------------------------------- |
| `aera_router_simulate_route`         | Simulasi routing percobaan dengan pohon fallback                       |
| `aera_router_set_budget_guard`       | Anggaran sesi dengan tindakan degrade/block/alert                      |
| `aera_router_set_resilience_profile` | Terapkan preset conservative/balanced/aggressive                       |
| `aera_router_test_combo`             | Uji langsung semua model dalam combo melalui permintaan upstream nyata |
| `aera_router_get_provider_metrics`   | Metrik terperinci untuk satu penyedia                                  |
| `aera_router_best_combo_for_task`    | Rekomendasi kesesuaian tugas beserta alternatifnya                     |
| `aera_router_explain_route`          | Jelaskan keputusan routing yang lalu                                   |
| `aera_router_get_session_snapshot`   | Status sesi lengkap: biaya, token, kesalahan                           |
