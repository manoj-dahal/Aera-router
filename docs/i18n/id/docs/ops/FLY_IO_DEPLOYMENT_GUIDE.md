# Panduan Deployment Aera Router di Fly.io
...
Dokumen ini menjelaskan metode deployment Aera Router di Fly.io yang telah terbukti berhasil, mencakup dua skenario utama:
...
Dokumen ini disusun berdasarkan konfigurasi yang telah diverifikasi pada proyek saat ini, dengan nama aplikasi `aera-router`.
...
- Alamat akses: `https://aera-router.fly.dev/`
...
- `app = 'aera-router'` menentukan aplikasi Fly mana yang menjadi target deployment
...
- Jika sebelumnya Anda menggunakan nama lain seperti `oroute`, jangan sampai tertukar dengan `aera-router`
...
Jika Anda sudah mengganti nama aplikasi, ganti `aera-router` dengan nama Anda.
...
Parameter-parameter berikut telah digunakan secara nyata pada aplikasi `aera-router` saat ini:
...
- Berlaku untuk proyek saat ini yaitu `aera-router`
...
```powershell
flyctl secrets set INITIAL_PASSWORD=kata-sandi-kuat-anda -a aera-router
```
...
- Apakah aplikasi yang sedang dilihat adalah `aera-router`
...
Jika repositori saat ini adalah fork dan Anda ingin menyinkronkan pembaruan dari upstream `https://github.com/manoj-dahal/Aera-router`, ikuti alur berikut.
...
- Anda membuka aplikasi yang salah, misalnya `oroute`, bukan `aera-router`
