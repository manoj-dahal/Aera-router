# Panduan Pengaturan Alat CLI — Aera Router (Bahasa Indonesia)
...
untuk menggunakan **Aera Router** sebagai backend terpadu, memberikan manajemen kunci terpusat,
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Kiro / Cursor / Copilot
           │
           ▼  (semua mengarah ke Aera Router)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (Aera Router meneruskan ke penyedia yang tepat)
...
| **Antigravity**    | `antigravity` | internal   | mitm            | Aera Router      |
...
## Langkah 1 — Dapatkan API Key Aera Router

1. Buka dashboard Aera Router → **API Manager** (`/dashboard/api-manager`)
...
export ANTHROPIC_API_KEY="sk-your-aera-router-key"
...
api_key = "sk-your-aera-router-key"
EOF
```
...
Atau gunakan dashboard Aera Router → **CLI Tools → Cline → Apply Config**.
...
Atau gunakan dashboard Aera Router → **CLI Tools → KiloCode → Apply Config**.
...
# CLI ini menggunakan autentikasinya sendiri — Aera Router tidak diperlukan sebagai backend untuk Kiro CLI itu sendiri.
# Gunakan kiro-cli bersama Aera Router untuk alat lainnya.
kiro-cli status
```
...
> **Catatan:** Cursor merutekan permintaan melalui cloudnya sendiri. Untuk integrasi Aera Router,
> aktifkan **Cloud Endpoint** di Pengaturan Aera Router dan gunakan URL domain publik Anda.

Melalui GUI: **Settings → Models → OpenAI API Key**

- Base URL: `https://your-domain.com/v1`
- API Key: kunci Aera Router Anda

---

## Konfigurasi Otomatis Dashboard

Dashboard Aera Router mengotomatiskan konfigurasi untuk sebagian besar alat:
...
**Droid** dan **OpenClaw** adalah agen AI yang dibangun langsung ke dalam Aera Router — tidak perlu instalasi.
Keduanya berjalan sebagai rute internal dan menggunakan perutean model Aera Router secara otomatis.
...
| ------------------------------- | ------------------------------ | -------------------------------------------------- |
| `Connection refused`            | Aera Router tidak berjalan     | `pm2 start aera-router`                            |
...
```bash
# Instal semua CLI dan konfigurasi untuk Aera Router (ganti dengan kunci dan URL server Anda)
AERA_ROUTER_URL="http://localhost:20128/v1"
AERA_ROUTER_KEY="sk-your-aera-router-key"
...
cat > ~/.claude/settings.json   <<< "{\"apiBaseUrl\":\"$AERA_ROUTER_URL\",\"apiKey\":\"$AERA_ROUTER_KEY\"}"
cat > ~/.codex/config.yaml      <<< "model: auto\napiKey: $AERA_ROUTER_KEY\napiBaseUrl: $AERA_ROUTER_URL"
cat >> ~/.bashrc << EOF
export OPENAI_BASE_URL="$AERA_ROUTER_URL"
export OPENAI_API_KEY="$AERA_ROUTER_KEY"
export ANTHROPIC_BASE_URL="$AERA_ROUTER_URL"
export ANTHROPIC_API_KEY="$AERA_ROUTER_KEY"
EOF

source ~/.bashrc
echo "✅ All CLIs installed and configured for Aera Router"
```
