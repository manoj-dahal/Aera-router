...
title: "Arquitetura do Aera Router"
version: 3.8.2
lastUpdated: 2026-05-13

---

# Arquitetura do Aera Router
...
Aera Router é um gateway de roteamento de IA local e um painel construído sobre Next.js.  
...
flowchart LR
    ...
    subgraph Router[Processo Local Aera Router]
...
- Construtor de metadados de resposta Aera Router: `src/domain/aeraRouterResponseMeta.ts`
...
- arquivo: `${DATA_DIR}/storage.sqlite` (ou `$XDG_CONFIG_HOME/aera-router/storage.sqlite` quando definido, caso contrário `~/.aera-router/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Runtime do Aera Router]
...
> Aera Router v3.8.0. Para a lista canônica e continuamente atualizada, consulte
...
- resposta final do cliente retornada pelo Aera Router; respostas transmitidas são armazenadas na mesma forma de resumo compacto
...
1. `usageDb` e `localDb` compartilham a mesma política de diretório base (`DATA_DIR` -> `XDG_CONFIG_HOME/aera-router` -> `~/.aera-router`) com migração de arquivos legados.
...
5. O diretório `open-sse/` é publicado como o pacote de **workspace npm** `@aera-router/open-sse`. O código-fonte o importa via `@aera-router/open-sse/...` (resolvido pelo Next.js `transpilePackages`). Os caminhos de arquivo neste documento ainda usam o nome do diretório `open-sse/` para consistência.
...
- Construir imagem Docker: `docker build -t aera-router .`
