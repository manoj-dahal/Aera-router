...
- before the active account is exhausted, Aera Router generates a compact structured summary
- after authentication selects a different account for the same session, Aera Router injects
...
If the active provider is enabled in `handoffProviders`, Aera Router generates a structured
...
Aera Router prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, Aera Router converts that payload into a `<context_handoff>` system
