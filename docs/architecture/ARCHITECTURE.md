---
title: "Aera Router Architecture"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Aera Router Architecture
...
Aera Router is a local AI routing gateway and dashboard built on Next.js.
...
Aera Router can install, supervise, and route to locally-running AI tool processes
...
- Aera Router response metadata builder: `src/domain/aeraRouterResponseMeta.ts`
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/aera-router/storage.sqlite` when set, else `~/.aera-router/storage.sqlite`)
...
> Aera Router v3.8.0. For the canonical and continuously-updated list, refer to
...
- final client response returned by Aera Router; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/aera-router` -> `~/.aera-router`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@aera-router/open-sse` **npm workspace package**. Source code imports it via `@aera-router/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t aera-router .`
