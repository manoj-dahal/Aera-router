...
# Aera Router — Design System & Visual Identity
...
> **Scope:** the Aera Router dashboard (`src/`) and the marketing site (`_mono_repo/aeraRouterSite/`) share **one visual identity** — same graph-paper grid background (32px), same color tokens, standardized components.
>
...
The exact recipe from the site (`_mono_repo/aeraRouterSite/css/base.css`): a **fixed, full-viewport pseudo-element** painting two 1px line gradients, sitting at `z-index:-1` behind all content.
...
| Site reference                    | `_mono_repo/aeraRouterSite/css/tokens.css`, `css/base.css`                                                           |
