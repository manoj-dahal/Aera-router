...
> **Status:** Accepted · **Context:** [#5670](https://github.com/manoj-dahal/Aera-router/issues/5670),
> [#5603](https://github.com/manoj-dahal/Aera-router/issues/5603) · **Contract:** `domain/routing/routerBackends.ts`
> (typed registry — code lands with [#5868](https://github.com/manoj-dahal/Aera-router/pull/5868))
...
1. **Lifecycle** (`RouterBackendLifecycle`) — _how the engine runs_:
   - `in-process` — runs inside the Aera Router Node process (the native TS pipeline).
   - `supervised` — a local child process Aera Router installs/starts/stops/health-checks
     via `ServiceSupervisor`, then consumes as a provider connection.
   - `external` — an HTTP endpoint Aera Router dispatches to but does **not** manage
...
[#5868](https://github.com/manoj-dahal/Aera-router/pull/5868)) declares every engine once, with its
...
[#5817](https://github.com/manoj-dahal/Aera-router/pull/5817); until it merges,
...
- **Selection** (`resolveRelayRoutingBackend`): a single global env toggle —
  `AERA_ROUTER_RELAY_BACKEND` / `RELAY_ROUTING_BACKEND` ∈ {`ts`, `bifrost`, `auto`}.
...
([#5869](https://github.com/manoj-dahal/Aera-router/pull/5869) manifest +
[#5870](https://github.com/manoj-dahal/Aera-router/pull/5870) `shouldTryBifrostForRequest`),
