...
This document is the authoritative reference for all CI quality gates in Aera Router.
...
| `nightly-schemathesis` | OpenAPI contract fuzzing (schemathesis) against a live Aera Router using `docs/openapi.yaml` — surfaces spec violations / unhandled 500s (Fase 8 B.4) | **Advisory** |
...
Beyond the CI gates, Aera Router ships an **opt-in** `agent-lsp` scaffold
