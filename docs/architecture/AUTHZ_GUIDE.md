...
Validated by `isValidApiKey()` / `extractApiKey()` in `src/sse/services/auth.ts` and re-exported through `src/shared/utils/apiAuth.ts`. The validator also accepts the `AERA_ROUTER_API_KEY` / `ROUTER_API_KEY` env vars as persistent passthrough keys (issue #1350).
...
Incoming request → src/proxy.ts
  → runAuthzPipeline() in src/server/authz/pipeline.ts
    1. Strip trusted internal headers (x-aera-router-auth-*, x-aera-router-route-class)
    ...
    8. Otherwise: POLICIES[routeClass].evaluate(ctx)
       - allow  → stamp x-aera-router-auth-{kind,id,label,scopes} → NextResponse.next()
       - reject → JSON error w/ correlation_id (dashboard pages → 302 /login)
```

Trusted internal headers (defined in `src/server/authz/headers.ts`) are **stripped from incoming requests** before classification — clients cannot pre-populate `x-aera-router-auth-*` to impersonate a subject.
...
request metadata, or `AERA_ROUTER_MCP_SCOPES`.
...
x-aera-router-route-class:    PUBLIC | CLIENT_API | MANAGEMENT
```

For authenticated requests the upstream (handler-side) request headers also include:

```
x-aera-router-auth-kind:      client_api_key | dashboard_session | management_key | anonymous
x-aera-router-auth-id:        key_<last-4> | "dashboard" | "anonymous"
x-aera-router-auth-label:     (optional)
x-aera-router-auth-scopes:    comma-separated list
```
