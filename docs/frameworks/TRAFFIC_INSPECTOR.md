...
Traffic Inspector is Aera Router's built-in HTTPS traffic debugger — a Charles Proxy / mitmweb / HTTP Toolkit-like tool that is **LLM-aware** and **agent-aware**. It lives at `/dashboard/tools/traffic-inspector` and receives live traffic from up to 5 simultaneous capture sources.
...
| Feature                                                             | mitmweb | Charles | Fiddler | **Aera Router Traffic Inspector** |
| ------------------------------------------------------------------- | :-----: | :-----: | :-----: | :-------------------------------: |
| Web-based                                                           |    ✓    |    ✗    |    ✗    |                 ✓                 |
| Open-source                                                         |    ✓    |    ✗    | partial |                 ✓                 |
...
| **Integrated with Aera Router** routing, fallback, cost             |    ✗    |    ✗    |    ✗    |                 ✓                 |
...
interface LlmMetadata {
  ...
  mappedTo: string | null; // x-aera-router-mapped header
  costEstimateUsd: number | null; // estimated cost based on Aera Router pricing
}
```
...
If Aera Router crashes while system-wide proxy mode is active:
...
| POST   | `/requests/{id}/replay`     | Re-execute the same request through Aera Router router                             |
