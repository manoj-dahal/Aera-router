...
Aera Router ships a WordPress-style plugin system. Plugins are self-contained
...
Plugins live under the Aera Router data directory:

```
~/.aera-router/plugins/<plugin-name>/
  ├─ plugin.json
  └─ index.js          # (or whatever manifest.main points to)
```

`getDefaultPluginDir()` (`src/lib/plugins/scanner.ts`) resolves this to
`<home>/.aera-router/plugins`, where `<home>` is taken from the `HOME` /
...
| `requires`         | object?   | `{ aera-router?, permissions[] }`                           |
...
install (POST /api/plugins, path)
  ...
  → atomic rename into ~/.aera-router/plugins/<name> → insert DB row
