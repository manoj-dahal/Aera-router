...
Aera Router can connect to a **Notion** workspace as a **context source** — a read/write
...
blocks — all proxied through Aera Router (with retry, timeout, and error classification)
...
> Aera Router to access with that integration (Notion's permission model is share-based,
> not workspace-wide).
...
Aera Router saves the token and immediately runs a 1-result test search; if Notion
...
`open-sse/mcp-server/server.ts` only when `AERA_ROUTER_MCP_ENFORCE_SCOPES=true`; the
caller's allowed scopes come from `AERA_ROUTER_MCP_SCOPES` (comma-separated) or the
