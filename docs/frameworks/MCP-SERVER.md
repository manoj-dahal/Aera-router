---
title: "Aera Router MCP Server Documentation"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Aera Router MCP Server Documentation
...
Aera Router MCP is built-in. Start it with:
...
aera-router --dev  # MCP auto-starts on /mcp endpoint
```
...
| :-------------------------------- | :-------------------- | :------------------------------------------------------------ |
| `aera_router_get_health`          | `read:health`         | Uptime, memory, circuit breakers, rate limits, cache stats    |
| `aera_router_list_combos`         | `read:combos`         | All configured combos with strategies (optional metrics)      |
| `aera_router_get_combo_metrics`   | `read:combos`         | Performance metrics for a specific combo                      |
| `aera_router_switch_combo`        | `write:combos`        | Activate or deactivate a combo                                |
| `aera_router_check_quota`         | `read:quota`          | Quota used/total, percent remaining, reset time, token health |
| `aera_router_route_request`       | `execute:completions` | Send a chat completion through Aera Router routing            |
| `aera_router_cost_report`         | `read:usage`          | Cost report by period (session/day/week/month)                |
| `aera_router_list_models_catalog` | `read:models`         | Full model catalog with capabilities, status, pricing         |

## Phase 1 — Search

| Tool                     | Scopes           | Description                                                                                                                          |
| :----------------------- | :--------------- | :----------------------------------------------------------------------------------------------------------------------------------- |
| `aera_router_web_search` | `execute:search` | Web search through Aera Router search gateway (Serper/Brave/Perplexity/Exa/Tavily/Google PSE/Linkup/SearchAPI/SearXNG) with failover |

## Advanced Tools (11) — Phase 2

| Tool                                 | Scopes                               | Description                                                                               |
| :----------------------------------- | :----------------------------------- | :---------------------------------------------------------------------------------------- |
| `aera_router_simulate_route`         | `read:health`, `read:combos`         | Dry-run routing simulation with fallback tree                                             |
| `aera_router_set_budget_guard`       | `write:budget`                       | Session budget with degrade/block/alert action                                            |
| `aera_router_set_routing_strategy`   | `write:combos`                       | Update combo strategy at runtime (priority/weighted/auto/etc.)                            |
| `aera_router_set_resilience_profile` | `write:resilience`                   | Apply `aggressive` / `balanced` / `conservative` resilience preset                        |
| `aera_router_test_combo`             | `execute:completions`, `read:combos` | Live test of every provider in a combo using a real upstream call                         |
| `aera_router_get_provider_metrics`   | `read:health`                        | Per-provider metrics with p50/p95/p99 latency and circuit breaker state                   |
| `aera_router_best_combo_for_task`    | `read:combos`, `read:health`         | Recommend combo by task type with budget/latency constraints                              |
| `aera_router_explain_route`          | `read:health`, `read:usage`          | Explain why a request was routed to a provider (scoring factors + fallbacks)              |
| `aera_router_get_session_snapshot`   | `read:usage`                         | Full session snapshot: cost, tokens, top models/providers, errors, budget guard           |
| `aera_router_db_health_check`        | `read:health`, `write:resilience`    | Diagnose (and optionally auto-repair) database drift like broken combo refs / orphan rows |
| `aera_router_sync_pricing`           | `pricing:write`                      | Sync pricing data from external sources (LiteLLM); supports `dryRun`                      |

## Cache Tools (2)

| Tool                      | Scopes        | Description                                         |
| :------------------------ | :------------ | :-------------------------------------------------- |
| `aera_router_cache_stats` | `read:cache`  | Semantic cache, prompt-cache, and idempotency stats |
| `aera_router_cache_flush` | `write:cache` | Flush cache globally or by signature/model          |

## Compression Tools (13)

| Tool                                  | Scopes              | Description                                                                                                              |
| :------------------------------------ | :------------------ | :----------------------------------------------------------------------------------------------------------------------- |
| `aera_router_compression_status`      | `read:compression`  | Compression settings, analytics summary, and cache-aware stats (includes `analytics.mcpDescriptionCompression` metadata) |
| `aera_router_compression_configure`   | `write:compression` | Configure compression mode, threshold, target ratio, system-prompt preservation, MCP description compression toggle      |
| `aera_router_set_compression_engine`  | `write:compression` | Pick the active engine (off/caveman/rtk/stacked) and Caveman/RTK intensity                                               |
| `aera_router_list_compression_combos` | `read:compression`  | List named compression combos and their engine pipelines                                                                 |
| `aera_router_compression_combo_stats` | `read:compression`  | Analytics grouped by compression combo and engine                                                                        |
| `aera_router_ccr_store`               | `write:compression` | Store caller-isolated content in the bounded in-memory CCR store and return a marker plus `ccr://` reference             |
| `aera_router_ccr_retrieve`            | `read:compression`  | Retrieve CCR content in full or with head, tail, lines, grep, and stats modes                                            |
| `aera_router_ccr_inspect`             | `read:compression`  | Inspect caller-owned CCR metadata without returning content                                                              |
| `aera_router_ccr_list`                | `read:compression`  | List paginated metadata for caller-owned CCR blocks                                                                      |
| `aera_router_ccr_delete`              | `write:compression` | Delete a caller-owned CCR block                                                                                          |
| `aera_router_ccr_stats`               | `read:compression`  | Report caller-scoped memory usage, lifecycle counters, and store limits                                                  |
| `aera_router_rtk_discover`            | `read:compression`  | Discover recurring noise in opt-in RTK output samples                                                                    |
| `aera_router_rtk_learn`               | `read:compression`  | Generate a reviewable RTK filter draft from opt-in samples                                                               |
...
`aera_router_compression_status` reports MCP description compression separately under
...
Separate from the compression tools above, Aera Router includes a post-execution filter that
...
| Tool                          | Scopes         | Description                                                                             |
| :---------------------------- | :------------- | :-------------------------------------------------------------------------------------- |
| `aera_router_oneproxy_fetch`  | `read:proxies` | Fetch free proxies from the 1proxy marketplace (protocol/country/quality/limit filters) |
| `aera_router_oneproxy_rotate` | `read:proxies` | Get the next available proxy by strategy (`random` / `quality` / `sequential`)          |
| `aera_router_oneproxy_stats`  | `read:proxies` | Pool stats, sync status, distribution by protocol and country                           |
...
| Tool                        | Scopes         | Description                                                                         |
| :-------------------------- | :------------- | :---------------------------------------------------------------------------------- |
| `aera_router_memory_search` | `read:memory`  | Search memories by query / type / API key with token-budget enforcement             |
| `aera_router_memory_add`    | `write:memory` | Add a new memory entry (`factual` / `episodic` / `procedural` / `semantic`)         |
| `aera_router_memory_clear`  | `write:memory` | Clear memories for an API key, optionally filtered by type or `olderThan` timestamp |
...
| Tool                            | Scopes           | Description                                                                       |
| :------------------------------ | :--------------- | :-------------------------------------------------------------------------------- |
| `aera_router_skills_list`       | `read:skills`    | List registered skills with optional filtering by API key, name, or enabled state |
| `aera_router_skills_enable`     | `write:skills`   | Enable or disable a specific skill by ID                                          |
| `aera_router_skills_execute`    | `execute:skills` | Execute a skill with provided input and return the execution record               |
| `aera_router_skills_executions` | `read:skills`    | List recent skill execution history                                               |
...
| Tool                                | Scopes         | Description                                                                                                      |
| :---------------------------------- | :------------- | :--------------------------------------------------------------------------------------------------------------- |
| `aera_router_agent_skills_list`     | `read:catalog` | List all 42 agent skills with optional `category` (api\|cli) and `area` filters; returns metadata + coverage     |
| `aera_router_agent_skills_get`      | `read:catalog` | Get full metadata + SKILL.md content for a single skill by canonical `id`                                        |
| `aera_router_agent_skills_coverage` | `read:catalog` | Coverage stats: how many of the 22 API and 20 CLI skills have SKILL.md files on the filesystem vs catalog totals |
...
Aera Router through the same connection model used for LLM providers. They are exposed via
...
| :---------------------------------------- | :--------------------------------- | :----------------------------------------------------------------------------------------------------------------------- |
| `AERA_ROUTER_BASE_URL`                    | `http://localhost:20128`           | Base URL the MCP server uses when calling Aera Router internal APIs                                                      |
| `AERA_ROUTER_API_KEY`                     | (empty)                            | API key forwarded as `Authorization: Bearer` to internal API calls                                                       |
| `AERA_ROUTER_MCP_ENFORCE_SCOPES`          | `false` (only `"true"` enables it) | When enabled, missing scopes deny tool calls and log `scope_denied:<reason>` in audit log                                |
| `AERA_ROUTER_MCP_SCOPES`                  | (empty)                            | Comma-separated allowlist of scopes considered "available" by default (used when caller does not provide its own scopes) |
| `AERA_ROUTER_MCP_COMPRESS_DESCRIPTIONS`   | (unset = on)                       | When set to `0/false/off/no`, disables MCP description compression at registration time                                  |
| `AERA_ROUTER_MCP_DESCRIPTION_COMPRESSION` | (unset = on)                       | Alternate alias for the same toggle as above                                                                             |
...
| `DATA_DIR`                                | `~/.aera-router`                   | Heartbeat file is written to `${DATA_DIR}/runtime/mcp-heartbeat.json`                                                    |
...
- Toggle process-wide via either `AERA_ROUTER_MCP_COMPRESS_DESCRIPTIONS=false` or `AERA_ROUTER_MCP_DESCRIPTION_COMPRESSION=false`.
- Realtime stats are surfaced via `aera_router_compression_status` under `analytics.mcpDescriptionCompression` and tagged `source: "mcp_metadata_estimate"` to disambiguate from real provider usage receipts.
...
MCP_TOOL_DENY="aera_router_get_health,aera_router_list_combos" aera-router --mcp

# Announce only the routing + quota tools (allow-list mode)
MCP_TOOL_ALLOW="aera_router_route_request,aera_router_check_quota" aera-router --mcp
```
