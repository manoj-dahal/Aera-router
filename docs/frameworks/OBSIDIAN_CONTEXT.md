...
Aera Router connects to an **Obsidian** vault as a **context source** — a local Markdown
...
vault sync (driven by `src/lib/obsidianSync.ts`). Enabling it points Aera Router at a
...
per call (per-API-key first, then global). Tools that hit the Aera Router **sync server**
(the four `obsidian_sync_*` tools) additionally require the sync auth token configured
in Aera Router settings.
...
| `obsidian_sync_status`       | Aera Router sync server status: running, vault name, port, uptime, last sync.        |
...
`open-sse/mcp-server/server.ts`, gated on `AERA_ROUTER_MCP_ENFORCE_SCOPES=true`, with
allowed scopes sourced from `AERA_ROUTER_MCP_SCOPES` or the API key's scope context. See
