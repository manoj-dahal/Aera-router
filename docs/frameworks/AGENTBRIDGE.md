...
AgentBridge is Aera Router's MITM (Man-in-the-Middle) proxy that intercepts HTTPS traffic from IDE AI agents and reroutes it through Aera Router's unified routing engine. It supports **9 IDE agents** — Antigravity, Kiro, GitHub Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code, and Trae (investigating) — making Aera Router the broadest-coverage MITM proxy for AI coding assistants on the market.
...
- **Reroute any agent to any provider**: Copilot talking to OpenAI? Redirect it to Anthropic Claude, Gemini, or any of Aera Router's 226+ providers.
...
- **Apply Aera Router resilience**: combo routing, circuit breakers, fallbacks, and cost tracking work for IDE agent traffic too.

### Positioning vs. the market

| Feature             | 9router | anti-api | llm-interceptor | **Aera Router AgentBridge** |
| ------------------- | :-----: | :------: | :-------------: | :-------------------------: |
| Antigravity         |    ✓    |    ✓     |        —        |              ✓              |
| GitHub Copilot      |    ✓    |    ✓     |        —        |              ✓              |
| Kiro (AWS)          |    ✓    |    ✓     |        —        |              ✓              |
...
src/mitm/server.cjs  (port 443, CJS child process)
    ...
    ├── Target match? → fetch → Aera Router router (port 20128)
...
> into the same `aera-router-mitm.crt` trust-store slot the old leaf used
...
```bash
certutil -A -d sql:$HOME/.pki/nssdb -n "Aera Router AgentBridge" -t CT,, -i ~/.aera-router/mitm/ca.crt
```
...
sudo security add-trusted-cert -d -r trustRoot \
  -k /Library/Keychains/System.keychain ~/.aera-router/mitm/ca.crt
```

**Windows (certmgr):**

```powershell
certutil -addstore -f Root $env:USERPROFILE\.aera-router\mitm\ca.crt
```
...
1. Point the runtime at the CA explicitly:
   ```bash
   export NODE_EXTRA_CA_CERTS=/path/to/aera-router-agentbridge-ca.crt
   ```
...
| Source model (agent native) | Target model (Aera Router) |
| --------------------------- | -------------------------- |
...
AgentBridge intercepts credentials (OAuth tokens, API keys) that the IDE uses to authenticate with upstream providers. These are **masked before logging** (see §2.7) but are visible to Aera Router's MITM layer. First activation of each agent shows a dismissible risk notice modal.
...
The bypass list ensures that financial institutions, OAuth/SSO providers, and other sensitive hosts are **never decrypted**. Their TLS traffic passes through as a transparent TCP tunnel — Aera Router never sees the plaintext.
...
1. Verify the cert was installed: `security find-certificate -c "Aera Router AgentBridge"` (macOS) or `certutil -L -d sql:$HOME/.pki/nssdb` (Linux/NSS)
...
```bash
grep "aera-router\|127.0.0.1.*github\|127.0.0.1.*cursor" /etc/hosts
```
...
2. Check Aera Router server logs: `APP_LOG_LEVEL=debug` in `.env`
3. Verify `AERA_ROUTER_BASE_URL` points to the correct router endpoint (default: `http://127.0.0.1:20128`)
