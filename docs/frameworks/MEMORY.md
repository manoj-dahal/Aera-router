...
Aera Router provides persistent conversational memory keyed by API key (and
...
> A client can opt a single request out with the `x-aera-router-no-memory`
...
- `searchSemanticMemory(query, topK, scope)` — embed the query, search the
  collection filtered by `kind = "aera_router_memory"` and optionally by
...
`aera_router_memory_search` tool.
...
`qdrantApiKey`, `qdrantCollection` default `"aera_router_memory"`,
...
- `aera_router_memory_search` — `{apiKeyId, query?, type?, maxTokens?, limit?}`
...
- `aera_router_memory_add` — `{apiKeyId, sessionId?, type, key, content,
metadata?}` → wraps `createMemory()`. Accepts only the 4 canonical types:
  `factual`, `episodic`, `procedural`, `semantic` (D17).
- `aera_router_memory_clear` — `{apiKeyId, type?, olderThan?}` → lists matching
...
- For hard deletion, use `DELETE /api/memory/[id]` or `aera_router_memory_clear`.
...
Aera Router's memory engine supports **four embedding sources** (`src/lib/memory/embedding/`). Each has different trade-offs in **latency, cost, model quality, and setup complexity**.
