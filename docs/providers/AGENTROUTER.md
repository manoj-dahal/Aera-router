...
For most users, **no special setup is required**. Aera Router ships a built-in
...
Aera Router also supports AgentRouter (and similar relays) through the **Claude Code
...
- Aera Router running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting Aera Router:
...
**`Invalid JSON response from provider (reset after Ns)` from the aera-router logs** —
