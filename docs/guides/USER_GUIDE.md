...
Complete guide for configuring providers, creating combos, integrating CLI tools, and deploying Aera Router.
...
models. Haiku models do not accept the `max` effort tier, so Aera Router downgrades that
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "aera-router/if/kimi-k2.7-code" }
    }
  },
  "models": {
    "providers": {
...
The CLI automatically loads `.env` from `~/.aera-router/.env` or `./.env`.
...
Void Linux users can package and install Aera Router natively using the `xbps-src` cross-compilation framework. This automates the Node.js standalone build along with the required `better-sqlite3` native bindings.
...
| --------------------------------------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `aera-router-default-secret-change-me` | JWT signing secret (**change in production**)                                                             |
| `INITIAL_PASSWORD`                      | `CHANGEME`                             | First login password                                                                                      |
| `DATA_DIR`                              | `~/.aera-router`                       | Data directory (db, usage, logs)                                                                          |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://aera-router.dev`              | Cloud sync endpoint base URL (replaces legacy `CLOUD_URL`)                                                |
...
| `AERA_ROUTER_MEMORY_MB`                 | `512`                                  | Node.js heap limit in MB                                                                                  |
...
### Chaining Aera Router Peers

Another Aera Router gateway can be added as a **Custom OpenAI-compatible** provider. Use the
...
AERA_ROUTER_PEER_URLS=http://gateway-b:20128/v1
AERA_ROUTER_PEER_MAX_HOPS=4
```

```bash
# gateway-b
AERA_ROUTER_INSTANCE_ID=gateway-b
AERA_ROUTER_PEER_URLS=http://gateway-a:20128/v1
AERA_ROUTER_PEER_MAX_HOPS=4
```

Only requests sent to an explicitly allowlisted peer URL receive the
`X-Aera-Router-Peer-Trace` header. A gateway rejects a repeated instance ID or exhausted hop
...
into multiple running Aera Router instances.
...
- **Semantic Cache** — Auto-caches non-streaming, temperature=0 responses (bypass with `X-Aera-Router-No-Cache: true`)
- **Request Idempotency** — Deduplicates requests within 5s via `Idempotency-Key` or `X-Request-Id` header
- **Progress Tracking** — Opt-in SSE `event: progress` events via `X-Aera-Router-Progress: true` header

---

### Translator Playground

Access via **Dashboard → Translator**. Debug and visualize how Aera Router translates API requests between providers.
...
Aera Router also accepts `x_session_id` and returns the effective session key in `X-Aera-Router-Session-Id`.
...
Aera Router implements provider-level resilience with five components:
...
4. **Wait For Cooldown** — If every candidate connection is already cooling down, Aera Router can wait for the earliest cooldown and retry the same client request automatically.
...
- Migrate Aera Router between machines
...
Aera Router supports audio transcription via the OpenAI-compatible endpoint:
...
Aera Router clamps it down to that cap before sending the upstream request.
...
Aera Router is both an **MCP server** (Model Context Protocol) and an **A2A server** (Agent-to-Agent JSON-RPC 2.0). Any MCP-compatible IDE or agent host can call Aera Router tools directly — no extra wrapper required.

### MCP transports

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `aera-router --mcp` (for IDE plugins that prefer stdio)
...
Aera Router exposes an extensible **skill framework** (`src/lib/skills/`) so agents and the A2A endpoint can run domain-specific routines (e.g. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
Aera Router persists **long-term conversational memory** with hybrid retrieval:
...
Subscribe to Aera Router events for real-time monitoring and automation.
...
- Every payload includes `X-Aera-Router-Signature` (HMAC-SHA256) for verification
...
Aera Router integrates with cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**) so you can dispatch long-running tasks from the same dashboard that handles your local routing.

- Create tasks in **Dashboard → Cloud Agents** or via `POST /api/v1/agents/tasks`
- Track status, logs, and artifacts per task
- Bring-your-own API key per provider — credentials never leave the Aera Router instance
...
You can manage every Aera Router resource (providers, combos, keys, settings) over HTTP using a **Bearer key with the `manage` scope**.
...
Aera Router ships an internal CLI (`aera-router …`) for setup, diagnostics, and runtime control. This is **separate from the "CLI Tools" page in the dashboard**, which configures third-party CLIs (Claude Code, Cursor, Codex, Cline, …) so they can talk to Aera Router.

```bash
aera-router setup                    # Interactive wizard (password, providers, combos)
aera-router setup --non-interactive  # CI-friendly
aera-router doctor                   # Health diagnostics (data dir, DB, providers, ports)
aera-router providers available      # List supported providers
aera-router providers list           # List configured connections
aera-router providers test <id>      # Live test a provider connection
aera-router combos list              # List combos
aera-router combos switch <name>     # Set default combo
aera-router models                   # List available models (--json, --search)
aera-router keys add | list | remove # Manage API keys from the terminal
aera-router backup                   # Snapshot config + DB
aera-router restore [<timestamp>]    # Restore from a snapshot
aera-router health                   # Detailed health (breakers, cache, memory)
aera-router quota                    # Provider quota usage
aera-router mcp status               # MCP server status
aera-router a2a status               # A2A server status
aera-router tunnel list|create|stop  # Cloudflare/Tailscale/ngrok tunnels
aera-router reset-password           # Reset the admin password
aera-router --mcp                    # Start MCP server over stdio
aera-router --port 3000              # Start the server on a custom port
```

Tip: pair `aera-router doctor --json` with your monitoring tool to alert on unhealthy provider connections.

---

## 🖥️ Desktop Application (Electron)

Aera Router is available as a native desktop application for Windows, macOS, and Linux.
