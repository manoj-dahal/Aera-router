---
title: "Codex CLI — Configuration with Aera Router"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Codex CLI — Configuration with Aera Router

Complete guide for using the Codex CLI pointed at Aera Router as an OpenAI-compatible backend.
...
export AERA_ROUTER_API_KEY="<YOUR_KEY>"
```
...
**Aera Router solves this transparently:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (Aera Router)
    → Aera Router Responses ↔ Chat Completions transformer
    → POST /chat/completions (DeepSeek / Mistral / GLM / Kimi / any provider)
```

You never need a separate translation proxy when using Aera Router. **All models use `wire_api = "responses"`** — Aera Router handles the rest.
...
| Model                                | Aera Router ID                       | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in Aera Router use the `cx/` prefix:

| Codex CLI name          | Aera Router model  |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the Aera Router provider alias.
...
## Generating profiles automatically with `aera-router setup-codex`

If you run Aera Router on a VPS, you can auto-generate profile files from the live model catalog:

```bash
# From a VPS (uses local Aera Router on port 20128)
aera-router setup-codex

# From any machine — point at your VPS
aera-router setup-codex --remote http://100.x.x.x:20128 --api-key sk-xxx

# Preview without writing files
aera-router setup-codex --remote http://100.x.x.x:20128 --dry-run

# Only generate GLM and Kimi profiles
aera-router setup-codex --only glm,kimi

# Write to a custom directory
aera-router setup-codex --codex-home /path/to/.codex
```
...
Aera Router can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `AERA_ROUTER_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `aera-router launch-codex`

Health-checks your Aera Router instance before launching Codex:

```bash
# Launch against local Aera Router (default port 20128)
aera-router launch-codex

# Launch with a specific profile
aera-router launch-codex --profile kimi-k27

# Launch against a remote VPS
aera-router launch-codex --remote http://100.x.x.x:20128/v1 --api-key sk-xxx

# Pass extra args to codex
aera-router launch-codex --profile glm52 -- --yolo "fix this bug"
```
...
[model_providers.aera-router.http_headers]
"X-Custom-Header" = "value"

# Headers read from env vars
[model_providers.aera-router.env_http_headers]
"X-Trace-Id" = "TRACE_ID"

# Extra URL query params (useful for Azure api-version)
[model_providers.aera-router.query_params]
...
```toml
[model_providers.aera-router-main]
base_url = "http://192.168.0.1:20128/v1"
env_key  = "AERA_ROUTER_API_KEY"

[model_providers.aera-router-tailscale]
base_url = "http://100.x.x.x:20128/v1"
env_key  = "AERA_ROUTER_API_KEY"
```
...
Verify the model exists in Aera Router with the correct prefix. Use `aera-router models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `AERA_ROUTER_API_KEY` is exported: `echo $AERA_ROUTER_API_KEY`.

**`Connection refused`**
Verify Aera Router is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
