...
Aera Router ships a cross-platform desktop app (Windows / macOS / Linux) built on
...
| App version        | `3.8.0`                      |
| App id             | `online.aera-router.desktop` |
| Product name       | `Aera Router`                |
...
- Windows: `%APPDATA%\aera-router`
- Linux: `$XDG_CONFIG_HOME/aera-router` or `~/.aera-router`
- macOS: `~/.aera-router`
...
- Close button minimizes to tray; the tray menu has **Open Aera Router**, **Open Dashboard** (external browser), **Server Port** submenu, **Check for Updates**, **Quit**.
...
- `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*.aera-router.online https://*.aera-router.dev`
- Dev mode adds `'unsafe-eval'` to `script-src` only

## Auto-update

Uses `electron-updater` with the GitHub provider (`manoj-dahal/Aera-router`).
...
- `Aera Router Setup X.Y.Z.exe`, `Aera-Router-X.Y.Z-portable.exe` (Windows)
- `Aera-Router-X.Y.Z-mac.dmg`, `Aera-Router-X.Y.Z-arm64-mac.dmg` (macOS)
- `Aera-Router-X.Y.Z.AppImage`, `aera-router-desktop_X.Y.Z_amd64.deb` (Linux)

Releases are published to GitHub Releases (`manoj-dahal/Aera-router`), which is also where `electron-updater` checks for new versions.
