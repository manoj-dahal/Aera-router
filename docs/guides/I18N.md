...
Aera Router supports **43 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
Aera Router uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through Aera Router
Cloud):
...
| Variable                              | Purpose                                 |
| ------------------------------------- | --------------------------------------- |
| `AERA_ROUTER_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `AERA_ROUTER_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `AERA_ROUTER_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
- **Chunked batching**: Joins multiple strings with `__AERA_ROUTER_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including Aera Router itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `aera-router` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | -------------------------- | ------------------------------------------- |
| 1        | `--lang` flag              | `aera-router --lang de status`              |
| 2        | `AERA_ROUTER_LANG` env var | `AERA_ROUTER_LANG=ja aera-router providers` |
...
```bash
# Set language and save to ~/.aera-router/.env (persists across sessions)
aera-router config lang set pt-BR

# View current language
aera-router config lang get

# List all 42 available languages
aera-router config lang list

# JSON output
aera-router config lang list --output json
```

The saved preference is written atomically to `~/.aera-router/.env` and is loaded by the
...
aera-router --lang de providers list
```
