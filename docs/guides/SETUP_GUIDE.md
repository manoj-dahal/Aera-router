---
title: "📖 Setup Guide — Aera Router"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 📖 Setup Guide — Aera Router

> Complete setup reference for Aera Router. For the quick version, see the [Quick Start in README](../README.md#-quick-start).
...
The [AUR package](https://aur.archlinux.org/packages/aera-router-bin) installs Aera Router and provides a systemd user service.
...
Aera Router ships a desktop wrapper built on Electron 41 + electron-builder 26.10. Available scripts (workspace root):
...
aera-router providers test-batch
```

Combined with env vars (`INITIAL_PASSWORD`, `AERA_ROUTER_WS_BRIDGE_SECRET`, etc.), this lets you spin up an Aera Router instance fully scriptable.
...
| `aera-router setup`       | Guided CLI onboarding for password and first provider          |
| `aera-router doctor`      | Run local health checks without starting the server            |
| `aera-router providers`   | Discover, list, validate, and test providers from CLI          |
| `aera-router config`      | CLI tool configuration — list, get, set, validate configs      |
| `aera-router status`      | Offline status dashboard — version, DB, tools, config          |
| `aera-router logs`        | Stream usage logs from the API (supports `--follow`)           |
| `aera-router update`      | Check for or apply Aera Router updates                         |
| `aera-router provider`    | Manage provider connections — add, list, remove, test, default |
...
```bash
aera-router setup --non-interactive --password "$AERA_ROUTER_PASSWORD"
aera-router setup --non-interactive --add-provider --provider openai --api-key "$OPENAI_API_KEY"
aera-router setup --non-interactive --add-provider --provider openai --api-key "$OPENAI_API_KEY" --test-provider
```
...
```bash
aera-router doctor
aera-router doctor --json
aera-router doctor --no-liveness
```
...
aera-router providers available --search openai
aera-router providers available --category api-key
aera-router providers list
aera-router providers test <id-or-name>
...
Instead of pasting the base URL and key by hand, let Aera Router write each tool's
own config from the live model catalog. One command per tool:

```bash
aera-router setup-codex        # ~/.codex/<name>.config.toml profiles
aera-router setup-claude       # ~/.claude/profiles/<name>/settings.json
aera-router setup-opencode     # ~/.config/opencode/opencode.json (openai-compatible)
aera-router setup-cline        # Cline CLI + VS Code extension settings
aera-router setup-kilo         # Kilo Code
aera-router setup-continue     # ~/.continue/config.yaml (Continue / cn)
aera-router setup-cursor       # prints Cursor's in-app steps
aera-router setup-roo          # Roo Code import + autoImport pointer
aera-router setup-crush        # ~/.config/crush/crush.json
aera-router setup-goose        # ~/.config/goose/config.yaml
aera-router setup-aider        # ~/.aider.conf.yml
aera-router setup-qwen         # ~/.qwen/settings.json + ~/.qwen/.env
```

Each accepts `--remote <url> --api-key <key>` to configure a local tool against a
**remote** Aera Router, plus `--dry-run` to preview. The launchers
...
aera_router_get_health        # Should return system health
aera_router_list_combos       # Should return active combos
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), Aera Router derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, Aera Router keeps the default `anthropic-beta` set conservative and, when `Client Cache Control` is left on `Auto`, only forwards client-provided `cache_control` markers. Enable the per-connection "Enable redact-thinking beta" toggle only when the upstream specifically requires redacted Claude thinking streams.
...
> **Note:** For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, Aera Router will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

### Reverse Proxy Compatibility

If you run Aera Router behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy timeouts are also higher than your Aera Router stream/fetch timeouts.
...
#   curl -L -o /tmp/aera-router.tar.gz "https://github.com/manoj-dahal/Aera-router/archive/refs/tags/v${version}.tar.gz" && sha256sum /tmp/aera-router.tar.gz
