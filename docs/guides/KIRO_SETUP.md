...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to Aera Router,
...
## How Aera Router Solves This (v3.8.0+)

Starting with v3.8.0, Aera Router calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each Aera Router connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; Aera Router reads credentials from the
...
4. Aera Router validates the key and saves the connection.
...
(`clientSecretExpiresAt`). Aera Router stores this timestamp in `providerSpecificData`
...
- Ensure Aera Router can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
...
- The key must be able to call `ListAvailableProfiles`; otherwise Aera Router cannot
