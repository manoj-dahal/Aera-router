...
How Aera Router estimates, records, and reports the cost of every request — and why the
...
Aera Router attributes a per-request USD cost to every completion by multiplying token
counts by a model's pricing rates. These numbers power the **Costs** dashboard, the
`aera-router cost` / `aera-router usage` CLI, CSV/JSON exports, and per-API-key budgets.

> **The dashboard "cost" is a savings tracker, not a bill.** Aera Router never charges you
...
- It depends on the pricing table Aera Router has for each model. A model with no pricing
...
3. **Hardcoded defaults** — shipped with Aera Router.
...
- API-key spend is buffered and flushed in batches by the
  ...
  | Env var                               | Default | Purpose                            |
  | ------------------------------------- | ------- | ---------------------------------- |
  | `AERA_ROUTER_SPEND_FLUSH_INTERVAL_MS` | `60000` | Flush interval in milliseconds.    |
  | `AERA_ROUTER_SPEND_MAX_BUFFER_SIZE`   | `1000`  | Max buffered entries before flush. |
...
Aera Router's CLI exposes cost, usage, and pricing commands (registered in
...
```bash
aera-router cost                          # last 30d, grouped by provider
aera-router cost --period 7d              # last 7 days
aera-router cost --group-by model         # group by provider | model | combo | api-key | day
aera-router cost --since 2026-06-01 --until 2026-06-13
aera-router cost --api-key <key> --limit 50
```
...
```bash
aera-router usage analytics --period 30d [--provider <id>]   # per-provider cost summary
aera-router usage logs [--limit 100] [--follow] [--api-key <k>] [--search <q>]
aera-router usage quota [--provider <id>] [--check]
aera-router usage utilization [--api-key <k>]
aera-router usage history [--limit 100]
aera-router usage proxy-logs [--limit 100]

# Budgets
aera-router usage budget list
aera-router usage budget get [scope]
aera-router usage budget set <amount> [--scope global] [--period monthly]
aera-router usage budget reset [scope]
```

### `aera-router pricing`

```bash
aera-router pricing list [--provider <p>] [--model <m>] [--limit 200]
aera-router pricing get <model>
aera-router pricing sync [--provider <p>] [--force]   # POST /api/pricing/sync
aera-router pricing diff [--model <m>]
aera-router pricing defaults show
aera-router pricing defaults set [--input <p>] [--output <p>] [--cache-read <p>] [--cache-write <p>]
```
...
- **All costs show $0 / "Legacy / Free".** The models in use have no pricing entry.
  Enable external sync (`PRICING_SYNC_ENABLED=true`) and run `aera-router pricing sync`, or
...
- **Spend lags behind real time.** Per-key spend is batched; lower
  `AERA_ROUTER_SPEND_FLUSH_INTERVAL_MS` if you need fresher numbers.
