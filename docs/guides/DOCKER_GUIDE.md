---
title: "🐳 Docker Guide — Aera Router"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Docker Guide — Aera Router
...
Aera Router ships four Compose profiles. Pick the one that matches your environment.

| Profile          | Service            | When to use                                                                                                                       | Command                                      |
| ---------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (default) | `aera-router-base` | Headless server / minimal runtime, no provider CLIs bundled                                                                       | `docker compose --profile base up -d`        |
| `cli`            | `aera-router-cli`  | Agentic workflows that call `aera-router providers/setup/doctor` and bundled CLIs (Codex, Claude Code, Droid, OpenClaw)           | `docker compose --profile cli up -d`         |
| `host`           | `aera-router-host` | Linux hosts that want `network_mode`-like access to host CLIs by mounting `~/.local/bin`, `~/.codex`, `~/.claude`, etc. read-only | `docker compose --profile host up -d`        |
...
Aera Router relies on Redis to back the distributed rate limiter and shared cache. The `redis` service is **always defined** in `docker-compose.yml` (it has no profile gate) and starts alongside any other profile.

| Detail               | Value                              |
| -------------------- | ---------------------------------- |
| Image                | `redis:7-alpine`                   |
| Container name       | `aera-router-redis`                |
| Internal port        | `6379`                             |
| Host port (override) | `REDIS_PORT` (defaults to `6379`)  |
| Volume               | `aera-router-redis-data` → `/data` |
...
| Image                  | `aera-router:prod` (built from `runner-cli` target)                                |
| Redis container        | `aera-router-redis-prod` (`redis:8.6.2`, dedicated `redis-prod-data` volume)       |
| Data volume            | `aera-router-prod-data` (named, persisted across rebuilds)                         |
...
```bash
docker build --target runner-base -t aera-router:base .
docker build --target runner-cli  -t aera-router:cli  .
```

Defaults exported by `runner-base`: `PORT=20128`, `HOSTNAME=0.0.0.0`, `NODE_OPTIONS=--max-old-space-size=512`, `DATA_DIR=/app/data`, `AERA_ROUTER_MIGRATIONS_DIR=/app/migrations`.

Memory behavior in Docker:

- `NODE_OPTIONS=--max-old-space-size=512` is baked into the image as a fallback.
- The actual server process is started by the standalone launcher, which reads `AERA_ROUTER_MEMORY_MB` and appends `--max-old-space-size=<AERA_ROUTER_MEMORY_MB>`.
- Node uses the last repeated `--max-old-space-size` value, so setting `AERA_ROUTER_MEMORY_MB` controls the effective Docker heap limit.
- If `AERA_ROUTER_MEMORY_MB` is unset, the launcher uses `512`.
...
| ------------------------------ | --------------------------------------------------------------------------------------------------- | ------------------------ |
| `AERA_ROUTER_WS_BRIDGE_SECRET` | Shared secret for the WebSocket bridge. **Required in production** — set to a strong random string. | unset (must be provided) |
...
| `AUTO_UPDATE_HOST_REPO_DIR`    | Host path mounted into `cli` profile at `/workspace/aera-router` for self-update workflows          | `.` (current directory)  |
| `AERA_ROUTER_MEMORY_MB`        | Runtime Node heap ceiling for the Docker standalone server; overrides the image fallback above      | `512`                    |
...
services:
  aera-router:
    ...
    environment:
      ...
      - BASE_URL=http://aera-router:20128
...
Caddy sets the standard forwarding headers for the upstream container. Aera Router uses
...
protection. Only enable `AERA_ROUTER_TRUST_PROXY` for advanced deployments where you intentionally
want Aera Router to derive the public origin from trusted forwarded headers instead of explicit
configuration.
...
| `diegosouzapw/aera-router` | `3.8.0`  | ~250MB | Current version       |
...
- **SQLite WAL Mode:** `docker stop` should be allowed to finish so Aera Router can checkpoint the latest changes back into `storage.sqlite`. The bundled Compose files already set a 40s stop grace period. If you run the image directly, keep `--stop-timeout 40`.
