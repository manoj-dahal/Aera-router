...
> **TL;DR**: Aera Router ranks its **free** providers by model quality using **Arena AI
...
Aera Router aggregates 160+ providers, many of which expose a **free tier** (no-auth,
...
Leaderboard categories map to Aera Router task categories:

| Arena leaderboard | Aera Router task categories                       |
| ----------------- | ------------------------------------------------- |
...
- It is **non-blocking and never fatal** — if the upstream fetch fails, Aera Router keeps
...
stripped and a small alias map expands canonical names into the variants Aera Router uses
