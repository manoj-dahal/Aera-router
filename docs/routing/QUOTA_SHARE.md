...
**Problem it solves:** Aera Router proxies many API keys against the same upstream
...
- All persistence is in the existing Aera Router SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("aera-router:quota-share:pools")` is non-empty.
