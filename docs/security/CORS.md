...
Aera Router controls which **browser origins** may read cross-origin responses
...
taking seriously, but Aera Router's exposure is **narrower than the generic case**,
...
> `Access-Control-Allow-Credentials: true`. Aera Router's shared CORS path never
> does.
...
- If Aera Router runs behind a reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
```bash
# Vite (5173) + Next.js (3000) dev servers calling a local Aera Router
...
## Example: reverse proxy in front of Aera Router

CORS is enforced by Aera Router itself, so the proxy generally should **not** add or
rewrite `Access-Control-*` headers (double headers break browsers). Terminate TLS
and forward — let Aera Router answer preflight:

```nginx
# nginx — forward to Aera Router; do NOT inject Access-Control-* here
location / {
...
Set the allowed browser origins in Aera Router (`CORS_ALLOWED_ORIGINS` or the
