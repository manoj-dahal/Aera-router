...
Aera Router CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-aera-router-cli-token`
request header.

This allows CLI subcommands (`aera-router status`, `aera-router providers`, etc.)
...
3. The CLI sends the token as `x-aera-router-cli-token` on every request to
...
Set `AERA_ROUTER_CLI_SALT` to rotate the derived token without code changes.
...
export AERA_ROUTER_CLI_SALT="my-secret-salt-2026"

# Verify new token is in use
aera-router status
```

Default salt: `aera-router-cli-auth-v1`
...
**Opt-out:** set `AERA_ROUTER_DISABLE_CLI_TOKEN=true` (env or `.env`) to disable the CLI
