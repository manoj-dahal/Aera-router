...
between Aera Router and upstream providers. Each guardrail can inspect (and
...
- Header `x-aera-router-disabled-guardrails` (or legacy
...
- **`garak` (advisory)** — runs garak `--probes promptinject,dan,leakreplay`
  against a local Aera Router instance (`http://localhost:20128/v1`). Gated on a
