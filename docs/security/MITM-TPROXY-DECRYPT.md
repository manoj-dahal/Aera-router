...
TPROXY transparent decrypt is Aera Router's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA Aera Router
...
> and installs under the pre-existing `aera-router-mitm.crt` trust-store slot
> (superseding the old single leaf there — no dual-trust cleanup needed),
> kept fully separate from TPROXY's own `aera-router-tproxy-ca.crt` slot below.
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"Aera Router MITM CA"`,
...
`aera-router-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`aera-router-mitm.crt`) so the two never clobber each
other.
...
| **Dedicated CA slot**            | The dynamic CA installs to `aera-router-tproxy-ca.crt`, never clobbering the static MITM cert.                                                                                                                                                                                                                                                                     |
...
host means anything Aera Router intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `aera-router-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if Aera Router was killed mid-session, use the AgentBridge
