...
1. `npm stage list aera-router` — find the stage id (also printed in the workflow summary).
...
`aera-router` in stage-only mode so a leaked long-lived token cannot `npm publish`
directly from anywhere — CI can only stage; only the owner's 2FA releases.

**Broken-artifact playbook (unchanged):** `npm deprecate aera-router@<bad> "<reason> — use <fixed>"`
...
- [ ] If source English docs changed, run `npm run i18n:run` (requires `AERA_ROUTER_TRANSLATION_API_KEY` in `.env`) before tagging
...
- [ ] Test on fresh install: delete `~/.aera-router/aera-router.db` and run `npm run dev`
...
> **Operator note:** the remote VPS image directory remains `/usr/lib/node_modules/aera-router/app/`.
...
- [ ] `aera-router --tray` boots on macOS (systray2 installed into `~/.aera-router/runtime/`)
- [ ] `aera-router --tray` boots on Linux (requires DISPLAY; graceful error if not set)
- [ ] `aera-router --tray` boots on Windows (PowerShell NotifyIcon, no extra binaries)
- [ ] `aera-router config tray enable` creates autostart entry; disable removes it
- [ ] `npm install -g aera-router@<this-version>` runs postinstall without fatal exit
- [ ] Update path keeps optional deps: `aera-router update --apply` and the auto-updater
...
- [ ] `aera-router status` works with no `.env` (CLI token path, loopback only)
...
- [ ] All 10 `skills/aera-router*/SKILL.md` files are publicly fetchable via raw GitHub URL
