---
title: "Aera Router — Deployment Guide on VM with Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Aera Router — Deployment Guide on VM with Cloudflare
...
AERA_ROUTER_WS_BRIDGE_SECRET=REPLACE-WITH-WS-BRIDGE-SECRET  # REQUIRED em produção: usado pelo Codex Responses WS bridge
...
Aera Router uses `NEXT_PUBLIC_BASE_URL` as the canonical browser-facing origin for OAuth
...
`AERA_ROUTER_TRUST_PROXY` if Aera Router is not directly reachable by clients and your proxy
...
See also [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) for the in-repo Cloudflare Tunnel walkthrough. The standalone `aeraRouterCloud/` worker lives in a separate companion repo.
