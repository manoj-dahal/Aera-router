...
Aera Router resolves its SQLite driver at startup through a 5-step fallback chain:
...
2. **Runtime-installed `better-sqlite3`** (in `~/.aera-router/runtime/`)
...
- **Windows EBUSY**: `npm install -g aera-router@latest` can fail if the previous
  version's `better_sqlite3.node` is locked by a running process. The runtime
  install in `~/.aera-router/runtime/` sidesteps the global npm cache.
- **No build tools**: Some environments (corporate Windows without VS Build
  ...
  drivers ensure Aera Router still boots even if that fails.
- **Air-gapped systems**: If the npm registry is unreachable, `node:sqlite`
  or `sql.js` guarantee baseline functionality.

## Magic-byte validation

Before loading a runtime-installed `.node` file, Aera Router reads the first 8
...
AERA_ROUTER_SKIP_POSTINSTALL=1 npm install -g aera-router

# Force-reinstall runtime better-sqlite3
rm -rf ~/.aera-router/runtime
aera-router  # will reinstall on next start

# Check what driver is active
aera-router config db-info  # (if CLI command exists)
```
