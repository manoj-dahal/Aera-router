---
title: "🌐 Aera Router Proxy Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🌐 Aera Router Proxy Guide

> **Bypass geographic blocks, protect your identity, and route AI traffic through any proxy — with zero configuration complexity.**

Aera Router includes a full-featured proxy management system that lets you route upstream AI provider traffic through HTTP, HTTPS, or SOCKS5 proxies. Whether you're in a blocked region, need IP rotation, or want stealth fingerprinting — this guide covers everything.
...
Aera Router supports proxy configuration at **four independent scopes**, resolved in priority order:
...
When Aera Router sends a request to an upstream provider, it calls `resolveProxyForConnectionFromRegistry()` which checks each level in order:
...
Proxies are included in the **Backup/Restore** system. When you export your Aera Router configuration:
...
If you configured proxies in an older version (pre-registry), Aera Router automatically migrates them:
...
> 🆕 **Contributed by [@oyi77](https://github.com/oyi77)** — PR [#1847](https://github.com/manoj-dahal/Aera-router/pull/1847) (Issue [#1788](https://github.com/manoj-dahal/Aera-router/issues/1788))

Aera Router integrates with the **[1proxy](https://1proxy-api.aitradepulse.com)** community platform to provide access to **hundreds of free, validated proxies** from around the world. This is perfect for users who don't have their own proxy infrastructure.
...
1. **Sync** — Aera Router fetches validated proxies from the 1proxy API
...
Aera Router doesn't just route traffic through a proxy — it makes the traffic look legitimate:
...
For providers that use the CLIProxyAPI pattern, Aera Router supports three upstream proxy modes:

| Mode          | Description                                          |
| ------------- | ---------------------------------------------------- |
| `native`      | Aera Router handles proxy routing directly (default) |
...
This is normal with cheap proxies that drop idle connections. Aera Router already handles this by:
...
Make sure the proxy is configured **before** starting the OAuth flow. Aera Router routes OAuth token exchange through the configured proxy. Set a global or provider-level proxy first, then connect.
...
Aera Router's **proxy fast-fail** mechanism (`src/lib/proxyHealth.ts`) detects dead proxies in <2s via a quick TCP connection check, then **caches the result** to avoid per-request overhead.
...
```ts
import { getAllProxyHealthStatuses, invalidateProxyHealth } from "aera-router/proxyHealth";
...
Aera Router tracks per-proxy usage to help operators diagnose routing patterns, latency spikes, and recurring failures.

### What's Tracked

For every request through a configured proxy, Aera Router records:
...
```bash
# Recent proxy events
curl -H "Authorization: Bearer $AERA_ROUTER_KEY" \
...
When multiple proxies are assigned to a scope, Aera Router uses a **rotation strategy** to pick which one to use for each request. The strategy is configured at the scope level (global, per-provider, per-account, per-combo).
...
```ts
import { rotateOneproxyProxy } from "aera-router/oneproxyRotator";
...
```ts
import { resetSequentialIndex } from "aera-router/oneproxyRotator";
...
```ts
import { failOneproxyProxy } from "aera-router/oneproxyRotator";
