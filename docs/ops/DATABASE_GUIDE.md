...
> **TL;DR**: Aera Router uses **SQLite with WAL journaling** as its primary store, with **AES-256-GCM** encryption at rest for sensitive fields. This guide covers the schema, migrations, backup/recovery, and operational runbooks.
...
Aera Router chose SQLite over PostgreSQL/MySQL for several reasons:
...
For **single-user, single-instance** deployments (the primary Aera Router use case), SQLite is simpler and faster.
...
| OS      | Path                                                     |
| ------- | -------------------------------------------------------- |
| Linux   | `~/.aera-router/storage.sqlite`                          |
| macOS   | `~/.aera-router/storage.sqlite`                          |
| Windows | `%USERPROFILE%\.aera-router\storage.sqlite`              |
...
```bash
DATA_DIR=/custom/path aera-router
```
...
Aera Router's database has **94 domain modules** in `src/lib/db/`. Each module:
...
Aera Router has **94 module files** in `src/lib/db/`. Below is a sampling of core modules; see the directory listing for the complete list:
...
Aera Router uses **versioned, idempotent migrations** in `src/lib/db/migrations/`. Each migration is a single SQL file named `NNN_description.sql`.
...
1. Creates `_aera_router_migrations` table if not exists
...
> **Backwards-incompatible changes** (e.g., dropping columns) are tricky. Aera Router does NOT support downgrade — once a migration is applied, the schema change is permanent. Plan accordingly.
...
Aera Router uses **`migrateLegacyEncryptedString()`** to handle two encryption schemes transparently:
...
aera-router backup create --name pre-migration
...
```bash
# Via CLI
aera-router restore pre-migration
...
aera-router backup auto enable --cron "0 2 * * *" --retention 7
```
...
```bash
sqlite3 ~/.aera-router/storage.sqlite ".backup /backups/aera-router-hot.db"
```

This uses SQLite's online backup API — safe to run while Aera Router is running.
...
Long-running Aera Router instances benefit from occasional `VACUUM`:

```bash
sqlite3 ~/.aera-router/storage.sqlite "VACUUM;"
```
...
```bash
sqlite3 ~/.aera-router/storage.sqlite "PRAGMA integrity_check;"
# Should print: ok
```
...
```bash
sqlite3 ~/.aera-router/storage.sqlite ".recover" > recovered.sql
sqlite3 recovered.db < recovered.sql
mv recovered.db ~/.aera-router/storage.sqlite
```
...
```bash
aera-router sync pull --merge   # or: aera-router backup restore <backup-id>
```
...
sqlite3 ~/.aera-router/storage.sqlite "PRAGMA wal_checkpoint(TRUNCATE);"
```
...
```bash
sqlite3 ~/.aera-router/storage.sqlite "SELECT * FROM api_keys LIMIT 5;"
```

### Count Rows in All Tables

```bash
sqlite3 ~/.aera-router/storage.sqlite <<EOF
...
```bash
# Stop Aera Router first
aera-router stop

# Delete the DB file
rm ~/.aera-router/storage.sqlite*
...
- If persistent, restart Aera Router
...
1. **Stop Aera Router** (prevent further attempts)
2. **Check the DB state** with `sqlite3`
3. **Manually fix** the partial migration
4. **Re-run** Aera Router (the migration will be retried)
