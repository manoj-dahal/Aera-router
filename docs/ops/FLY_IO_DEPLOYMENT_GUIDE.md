---
title: "Aera Router Fly.io Deployment Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Aera Router Fly.io Deployment Guide

This document describes the actual deployment process for Aera Router on Fly.io, covering two scenarios:
...
This guide is based on a verified working configuration for the current project. The application name is `aera-router`.
...
- Access URL: `https://aera-router.fly.dev/`
...
- `app = 'aera-router'` determines which Fly application the deployment targets
...
```toml
app = 'aera-router-yourname'
```

Note:

- Make sure the application you see in the console matches the `app` value in `fly.toml`
- If you previously used a different name, such as `oroute`, do not confuse it with `aera-router`
...
```powershell
flyctl apps create aera-router
```

If you changed the application name, replace `aera-router` with your chosen name.
...
These parameters have been used in actual deployments on the current `aera-router` application:
...
- `AERA_ROUTER_WS_BRIDGE_SECRET` (required in production — used for WebSocket bridge authentication)
...
| `AERA_ROUTER_WS_BRIDGE_SECRET` | Required in production | WebSocket bridge authentication secret                |
...
| ---------------------- | ----------------------------- |
| `DATA_DIR`             | `/data`                       |
| `NEXT_PUBLIC_BASE_URL` | `https://aera-router.fly.dev` |
...
1. **Set `NEXT_PUBLIC_BASE_URL` to your public HTTPS domain**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://aera-router.fly.dev -a aera-router
   ```

   If you are using a custom domain, replace it with the corresponding domain (e.g. `https://aera-router.yourdomain.com`).
...
2. **Configure the callback URL on the provider console**
   ...
   - `https://aera-router.fly.dev/callback`
...
- Intended for the current project `aera-router`
...
flyctl secrets set `
  ...
  AERA_ROUTER_WS_BRIDGE_SECRET=$wsBridgeSecret `
  DATA_DIR=/data `
  NEXT_PUBLIC_BASE_URL=https://aera-router.fly.dev `
  -a aera-router
```
...
```bash
flyctl secrets set AERA_ROUTER_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a aera-router
```

Notes:

- `AERA_ROUTER_WS_BRIDGE_SECRET` is required in production; missing it will break the WebSocket bridge handshake

If you also want to set an initial password:

```powershell
flyctl secrets set INITIAL_PASSWORD=your-strong-password -a aera-router
```
...
```powershell
flyctl secrets list -a aera-router
```

If the `Secrets` page in the console does not show the expected variables, check:

- That you are viewing the `aera-router` application
...
```powershell
flyctl secrets set KEY=value -a aera-router
```
...
If the current repository is a fork and you want to sync updates from the upstream `https://github.com/manoj-dahal/Aera-router`, follow the workflow below.
...
```powershell
git remote add upstream https://github.com/manoj-dahal/Aera-router.git
```
...
5. `flyctl deploy`
6. `flyctl status -a aera-router`
7. `flyctl logs --no-tail -a aera-router`
...
```powershell
flyctl status -a aera-router
```

### 10.2 View Startup Logs

```powershell
flyctl logs --no-tail -a aera-router
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://aera-router.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- You are viewing a different application (e.g. `oroute` instead of `aera-router`)
...
1. `flyctl auth login`
2. `flyctl apps create aera-router`
3. `flyctl secrets set ... -a aera-router`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a aera-router`
