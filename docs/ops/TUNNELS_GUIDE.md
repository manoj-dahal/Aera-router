...
Aera Router can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — Aera Router starts/stops the underlying
...
- **Auto-install.** On first use, Aera Router downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  Aera Router. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                                 |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, Aera Router uses it instead of downloading. |
...
must be configured in the ngrok dashboard — Aera Router itself only forwards the
...
1. Install Tailscale (or let Aera Router do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via Aera Router's `login` endpoint).
...
Aera Router's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose Aera Router through a tunnel, the dashboard and OAuth flows must
...
and restart Aera Router before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; Aera Router will track each
independently.
...
Aera Router attempts to auto-install on first use. If the install is blocked
