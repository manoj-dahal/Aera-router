---
title: "🗜️ Prompt Compression Guide — Aera Router"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🗜️ Prompt Compression Guide — Aera Router
...
Aera Router implements a modular prompt compression pipeline that runs **proactively** before requests hit upstream providers. This means your token savings happen transparently — no changes needed to your workflow.
...
Aera Router documents compression savings from two sources: upstream project benchmarks and
Aera Router's own engine composition.
...
For overlapping tool/context payloads, the default Aera Router combo stacks the engines:
...
Send the `x-aera-router-compression` request header to override the compression plan for a single
...
The applied plan is echoed back in the `X-Aera-Router-Compression: <mode>; source=<source>` response
...
Beyond the 7 standard modes, Aera Router includes several advanced compression
...
} from "@aera-router/open-sse/services/compression/cachingAware";
...
```ts
import { applyAging } from "@aera-router/open-sse/services/compression/progressiveAging";
