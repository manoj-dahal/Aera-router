...
> **TL;DR**: Aera Router's compression engine is **pluggable** — you can register custom engines, ship language packs for new languages, and compose stacked pipelines. This guide shows how.
...
````ts
import type { CompressionEngine } from "aera-router/compression/engines/types";
import { registerCompressionEngine } from "aera-router/compression/engines/registry";
...
```
~/.aera-router/compression/engines/my-engine.ts    # User-level
...
} from "@aera-router/open-sse/services/compression/engines/registry";
...
Caveman-style compression uses **language-specific rule packs** to handle fillers, hedging, and verbose patterns in each natural language. Aera Router ships with **6 language packs**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
```ts
import { loadRulePack } from "aera-router/compression/ruleLoader";
...
```
~/.aera-router/compression/rules/hi/filler.json  # User-level
...
Aera Router selects **ONE mode per request** based on configuration, auto-trigger thresholds, and combo overrides.
...
`@aera-router/open-sse/services/compression/strategySelector`):

```ts
import { applyStackedCompression } from "@aera-router/open-sse/services/compression/strategySelector";
...
Aera Router's compression engines credit several upstream projects in the README
...
pack, how does that reach Aera Router?** This section is the authoritative answer.
...
rule/filter in Aera Router's own format** (see
...
Aera Router's own `smartcrusher` / `toon` / `tabular` layers. There is no live
...
1. **Don't vendor** — re-express the upstream rule/filter in Aera Router's format.
