...
RTK compression is Aera Router's command-aware compression engine for terminal and tool output. It is
...
`~118,000` standard tokens to `~23,900` RTK tokens, which is `79.7%` saved (`~80%`). Aera Router uses
...
Within the same scope, RTK TOML schema v1 filters take precedence over Aera Router JSON filters. TOML
...
- `AERA_ROUTER_RTK_TRUST_PROJECT_FILTERS=1` is set.
...
Aera Router can parse, validate, test, and install declarative filter files using RTK TOML schema v1.
...
Aera Router receives tool output after the client has already captured it, so `filter_stderr = true`
...
`rtkEngine` (`@aera-router/open-sse/services/compression/engines/rtk`) is a
`CompressionEngine` and has no `updateConfig` method. Update an engine's config
through the registry helper instead:

```ts
import { updateEngineConfig } from "@aera-router/open-sse/services/compression/engines/registry";
...
```ts
import { runRtkFilterTests } from "aera-router/compression/engines/rtk/verify";
...
```
~/.aera-router/rtk/filters/my-filter.json     # User-level
...
- User directory: `~/.aera-router/rtk/filters/`
- Project directory: `<project>/.rtk/filters/`

To load filters programmatically:

```ts
import { loadRtkFilters } from "@aera-router/open-sse/services/compression/engines/rtk/filterLoader";
...
```ts
import { readRtkRawOutput } from "aera-router/compression/engines/rtk/rawOutput";
