...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/aera-router.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.aera-router/.env to survive across upgrades. This prevents credential loss
  // when upgrading Aera Router (issue #1622).
  MACHINE_ID_SALT: () => `aera-router-${randomBytes(8).toString("hex")}`,
};
 ...
 * It's generated at server startup in bin/aera-router.mjs and persisted to
 * ~/.aera-router/.env to survive across upgrades.
 * @see https://github.com/manoj-dahal/Aera-router/issues/1622
 */
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "aera-router");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "aera-router");

  return join(homedir(), ".aera-router");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.aera-router/server.env, ~/.aera-router/.env, ` +
