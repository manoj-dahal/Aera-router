#!/usr/bin/env node
/**
 * system-info.mjs — Aera Router System Information Reporter (#280)
 *
...
lines.push("Aera Router System Information Report");
...
lines.push(section("Aera Router"));
try {
...
const installedGlobal = run("npm list -g aera-router --depth=0 2>/dev/null | grep aera-router");
lines.push(`Global npm: ${installedGlobal || "not installed globally"}`);

const pm2Status = run("pm2 list 2>/dev/null | grep aera-router | awk '{print $4, $10, $12}'");
...
console.log(
  `📎 Attach this file when reporting issues at: https://github.com/manoj-dahal/Aera-router/issues`
);
