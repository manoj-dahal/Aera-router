/**
 * WebDAV file server handler for Aera Router.
 *
...
// Key derivation: scryptSync(secret, "aera-router-field-encryption-v1", 32)
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Decrypt a value that may be encrypted with the Aera Router enc:v1: format.
...
function getDefaultDataDir() {
  ...
  const legacyDir = path.join(homeDir, ".aera-router"); // getLegacyDotDataDir()

  // 2) Preserve the legacy ~/.aera-router path if it already exists (avoid data loss).
  try {
  ...
  if (typeof xdgConfigHome === "string" && xdgConfigHome.trim().length > 0) {
    return path.join(path.resolve(xdgConfigHome.trim()), "aera-router");
  }

  // 5) Default → legacy ~/.aera-router (even if it does not exist yet).
  return legacyDir;
}
...
function sendUnauthorized(res) {
  sendError(res, 401, "Unauthorized", {
    "WWW-Authenticate": 'Basic realm="Aera Router WebDAV"',
  });
}
...
async function handlePut(req, res, absPath) {
  ...
  const tmpPath = absPath + ".aera-router-webdav-tmp-" + Date.now();
  let writeStream;
  try {
