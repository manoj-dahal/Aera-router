...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://aera-router.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://aera-router.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-aera-router-ws-session-id": this.sessionId,
      "x-aera-router-ws-request-id": requestId,
    };
...
export function createAeraRouterWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createAeraRouterWsBridge requires a baseUrl");
  }
