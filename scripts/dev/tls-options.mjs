...
//
// Pure, dependency-light helpers for Aera Router's opt-in native HTTPS/TLS serving
 ...
 *
 * Reads `AERA_ROUTER_TLS_CERT` and `AERA_ROUTER_TLS_KEY` (filesystem paths). Returns
...
export function resolveTlsOptions(
  ...
  const certPath =
    typeof env?.AERA_ROUTER_TLS_CERT === "string" ? env.AERA_ROUTER_TLS_CERT.trim() : "";
  const keyPath =
    typeof env?.AERA_ROUTER_TLS_KEY === "string" ? env.AERA_ROUTER_TLS_KEY.trim() : "";
  ...
  if (!certPath || !keyPath) {
    warn(
      `[aera-router][tls] HTTPS not enabled: both AERA_ROUTER_TLS_CERT and AERA_ROUTER_TLS_KEY ` +
  ...
  try {
    ...
    warn(
      `[aera-router][tls] HTTPS not enabled: could not read TLS cert/key ` +
