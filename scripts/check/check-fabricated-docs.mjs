...
//   3. CLI commands `aera-router ...`     → must exist in bin/cli/commands/ or bin/
...
const ENV_VAR_ALLOWLIST = new Set([
  ...
  "VERBOSE",
  "LOG_LEVEL",
  "PORT", // generic, not Aera-Router-specific
  "DATA_DIR",
  "REQUIRE_API_KEY",
  "AERA_ROUTER_BUILD_PROFILE", // build-time only
  "AERA_ROUTER_BUILD_SHA",
  "AERA_ROUTER_URL", // used by ad-hoc tooling, validated elsewhere
  "AERA_ROUTER_KEY", // ditto
  ...
  // subprocess — never read via `process.env.X` in Aera Router's own source, so the
  ...
  "OPENAI_BASE_URL", // env var Aera Router passes to downstream CLIs (AGENT_PROTOCOLS_GUIDE.md)
...
const ENDPOINT_ALLOWLIST = new Set([
  ...
  // paths on the UPSTREAM service (Claude.ai web, Blackbox), not Aera Router routes.
...
// Env var helper wrappers used across Aera Router — envInt(NAME, 5), envBool(NAME),
...
export function buildCodebaseIndex(root = ROOT) {
  ...
  // documented Aera Router env var (including external-CLI / docker / electron vars
  ...
  // Set of `aera-router <subcommand>` strings that exist in bin/
...
const COARSE_PATTERNS = {
  ...
  // aera-router <verb> <sub> ... — only on the same line, captures first 2 tokens
  cliCmd: /\baera_router\s+([a-z][a-z0-9-]+)(?:\s+([a-z][a-z0-9-]+))?/g,
...
export function scanDocFile(absPath, index, root = ROOT) {
  ...
  // 3) CLI commands: `aera-router foo bar` — only flag when the line is in
  //    a code-like context (inside backticks or a shell block). Bare prose
  //    like "we use aera-router and..." is not a command claim.
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.cliCmd)) {
    ...
    const isShellLike = /^[ \t]*\$\s|^```sh|^```bash|^```shell|`aera-router/.test(lineText);
    if (!isShellLike) continue;
    if (/example|placeholder|tbd/i.test(lineText)) continue;
    findings.push({
      kind: "cli-cmd",
      value: `aera-router ${sub}`,
      line: ln,
      msg: `aera-router subcommand '${sub}' not registered in bin/`,
    });
  }
  ...
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.fileRef)) {
    ...
    // (`@aera-router/opencode-provider/src/index.ts`). Neither resolves from repo root.
...
export function formatHumanReport(result) {
  ...
  const KIND_LABELS = {
    ...
    "cli-cmd": "aera-router subcommands not registered",
