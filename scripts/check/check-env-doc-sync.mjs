 ...
 *
 * Enforces that every env var referenced in Aera Router source code appears in
...
const IGNORE_FROM_CODE = new Set([
  ...
  "USERPROFILE",
  "PREFIX",
  // X11 display server — set by the OS/session manager, not Aera Router config.
  "DISPLAY",
  ...
  // bundler is active — framework-internal. The Aera-Router-facing knob is
  // AERA_ROUTER_USE_TURBOPACK (scripts/dev/run-next.mjs), which IS documented.
  "TURBOPACK",
  "NODE_TEST_CONTEXT",
  "VITEST",
  // Instruction snippet shown to users (Traffic Inspector HttpProxySnippetCard) — not Aera Router config.
  ...
  // points at Aera Router). A downstream client-tool var, not an Aera Router server
  ...
  // (scripts/check/check-openapi-breaking.mjs) — a build/check signal, not Aera Router runtime config.
  "BASE_REF",
  // PR body injected by GitHub Actions into the pr-evidence gate (github.event.pull_request.body);
  // a CI-only signal, never an Aera Router runtime config (Phase 7.10).
  "PR_BODY",
  // CLI machine-id token opt-out (server-side flag; not user-configurable via .env).
  "AERA_ROUTER_DISABLE_CLI_TOKEN",
  ...
  // ("http://192.168.0.15:20128" / null), never Aera Router runtime config (#5151).
  "COMBO_LIVE_BASE_URL",
  "COMBO_LIVE_API_KEY",
  ...
  "AERA_ROUTER_NO_UPDATE_NOTIFIER",
  // Headless CLI execution flag for Electron.
  "AERA_ROUTER_HEADLESS",
  // Platform / OS detection vars read by CLI environment helper (bin/cli/utils/environment.mjs).
  // These are external signals set by the host OS or cloud provider — not Aera Router config.
  "CODESPACES",
  ...
  // internals, never Aera Router runtime config.
  "VERIFY_DEADLINE_S",
  "VERIFY_PORT",
  "VERIFY_VERSION",
  // Doctor diagnostic flags (no runtime behavior yet — placeholders).
  "AERA_ROUTER_DOCTOR_HOST",
  "AERA_ROUTER_DOCTOR_LIVENESS_URL",
  "AERA_ROUTER_PROVIDER_CATALOG_PATH",
  "AERA_ROUTER_PROVIDER_TEST_MODEL",
  // Test-only opt-out: instructs bin/aera-router.mjs to skip auto-loading the
  // repository .env so isolation tests get a deterministic environment.
  "AERA_ROUTER_CLI_SKIP_REPO_ENV",
  ...
  // A dev/ops measurement tool, never Aera Router runtime config.
  "AERA_ROUTER_EVAL_CREDENTIALS",
  ...
  "AERA_ROUTER_INTERNAL_SCHEME",
  // Source typo / placeholder.
  "OMNIROUT",
  // Static config alias path (the canonical var is AERA_ROUTER_PAYLOAD_RULES_PATH).
  "PAYLOAD_RULES_PATH",
  // Node.js module resolution path — OS/Node internal, not an Aera Router config var.
  ...
  "NVIDIA_BASE_URL",
  "NVIDIA_MODEL",
  // XDG standard data directory — set by OS/desktop session, not Aera Router config.
  ...
  "AERA_ROUTER_OPENCODE_PLUGIN_DIR",
]);
...
const DOC_ONLY_ALLOWLIST = new Set([
  ...
  "AERA_ROUTER_CRYPT_KEY",
  "AERA_ROUTER_API_KEY_BASE64",
