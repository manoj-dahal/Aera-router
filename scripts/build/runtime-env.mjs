 ...
 * `AERA_ROUTER_MEMORY_MB`, mirroring `aera-router serve`. Clamped to [64, 16384];
 * invalid/unset → fallback (512). The standalone launcher uses this so
 * AERA_ROUTER_MEMORY_MB can override the Docker image's NODE_OPTIONS fallback
 ...
 * when `AERA_ROUTER_MEMORY_MB` is unset. A fixed 512MB default crashed boxes with
 ...
 * explicit AERA_ROUTER_MEMORY_MB override always wins.
 ...
 * (`--max-old-space-size=…`). Used to decide whether `aera-router serve` may
 ...
 * the user already exported. #5238: `aera-router serve` used to UNCONDITIONALLY
...
export function withRuntimePortEnv(env, runtimePorts) {
  ...
  return {
    ...env,
    AERA_ROUTER_PORT: String(basePort),
