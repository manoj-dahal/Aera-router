#!/usr/bin/env node
/**
 * Aera Router — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.aera-router/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by Aera Router bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/aera-router.mjs`.
  //
  ...
  if (needsPersist) {
    merged.AERA_ROUTER_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "aera-router-field-encryption-v1", 32)
              const legacySalt = "aera-router-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.aera-router/server.env or ~/.aera-router/.env,"
                );
                log(
                  "   or run: aera-router reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
