...
/**
 * Aera Router — Prepublish Build Script
 *
...
console.log("🔨 Aera Router — Building for npm publish...\n");
...
const cliSrcFile = join(ROOT, "bin", "aera-router.ts");
const cliDestFile = join(ROOT, "bin", "aera-router.mjs");
  ...
  try {
    execFileSync(
      ...
      [
        ...
        "--packages=external",
        "--format=esm",
        "--outfile=bin/aera-router.mjs",
      ],
      { cwd: ROOT, stdio: "inherit" }
    );
    chmodSync(cliDestFile, 0o755);
    console.log("  ✅ CLI Entrypoint bundled to bin/aera-router.mjs");
  } catch (err: any) {
    console.warn("  ⚠️  CLI bundle error:", err.message);
  }
}

// ── Step 8.8: Build @aera-router/opencode-plugin ──────────────
// The plugin ships bundled inside the aera-router npm package (see root
// package.json "files": ["@aera-router/", ...]). Its built `dist/` MUST be
// present in the publish tarball so `aera-router setup opencode` can copy it
...
const opencodePluginSrc = join(ROOT, "@aera-router", "opencode-plugin");
...
if (existsSync(opencodePluginSrc) && existsSync(join(opencodePluginSrc, "package.json"))) {
  ...
  if (!pluginAlreadyBuilt) {
    console.log("\n  🔨 Building @aera-router/opencode-plugin (tsup)...");
    try {
      ...
      console.log(
        "  ✅ @aera-router/opencode-plugin bundled to @aera-router/opencode-plugin/dist/"
      );
    } catch (err: any) {
      console.error("  ❌ Failed to build @aera-router/opencode-plugin:", err.message);
      console.error("     The published package would be missing the plugin dist.");
      console.error(
        "     Run `cd @aera-router/opencode-plugin && npm install && npm run build` to debug."
      );
      process.exit(1);
    }
  } else {
    console.log("  ✅ @aera-router/opencode-plugin dist/ already present (skipping rebuild)");
  }
  ...
  if (existsSync(pluginNodeModules)) {
    ...
    console.log("  🧹 Removed @aera-router/opencode-plugin/node_modules (hard link guard)");
  }
} else {
  console.log("  ⏭️  @aera-router/opencode-plugin not found in workspace (skipping build)");
}
