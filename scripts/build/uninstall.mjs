...
const uninstallAlreadyInProgress =
  process.env.AERA_ROUTER_SKIP_UNINSTALL_HOOK === "1" ||
  process.env.npm_lifecycle_event === "uninstall";

console.log("🛑 Aera Router Uninstaller");
console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

// 1. Stop PM2 process if it exists
try {
  console.log("Stopping and removing background PM2 processes...");
  execSync("pm2 delete aera-router 2>/dev/null", { stdio: "ignore" });
} catch {
...
if (uninstallAlreadyInProgress) {
  ...
  try {
    execSync("npm uninstall -g aera-router", {
      stdio: "inherit",
      env: {
        ...process.env,
        AERA_ROUTER_SKIP_UNINSTALL_HOOK: "1",
      },
    });
    console.log("\n✅ Aera Router has been successfully uninstalled from your system.");
    if (!fullUninstall) {
