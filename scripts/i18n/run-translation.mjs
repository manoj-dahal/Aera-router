#!/usr/bin/env node
/**
 * Aera Router — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   AERA_ROUTER_TRANSLATION_API_URL     e.g. https://cloud.aera-router.dev/v1
 *   AERA_ROUTER_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   AERA_ROUTER_TRANSLATION_MODEL       e.g. cx/gpt-5.6-sol
 *   AERA_ROUTER_TRANSLATION_TIMEOUT_MS  optional, default 60000
 *   AERA_ROUTER_TRANSLATION_CONCURRENCY optional, default 4
 */
...
function backendConfig() {
  const apiUrl = requireEnv("AERA_ROUTER_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("AERA_ROUTER_TRANSLATION_API_KEY");
  const model = requireEnv("AERA_ROUTER_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.AERA_ROUTER_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.AERA_ROUTER_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.AERA_ROUTER_TRANSLATION_API_URL || "").replace(/\/$/, "");
