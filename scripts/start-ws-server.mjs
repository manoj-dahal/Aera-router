 ...
 *   AERA_ROUTER_ENABLE_LIVE_WS — Set to "0" or "false" to disable
 */
...
const BOOTSTRAPPED_ENV = "AERA_ROUTER_LIVE_WS_BOOTSTRAPPED";
...
async function main() {
  ...
  // child is re-spawned with AERA_ROUTER_ENABLE_LIVE_WS="0" purely to stop liveServer.ts
  ...
  if (
    ...
    (process.env.AERA_ROUTER_ENABLE_LIVE_WS === "0" ||
      process.env.AERA_ROUTER_ENABLE_LIVE_WS?.toLowerCase() === "false")
  ) {
    console.log("[LiveWS] Disabled via AERA_ROUTER_ENABLE_LIVE_WS");
    process.exit(0);
  }
