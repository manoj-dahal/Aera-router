#!/usr/bin/env bash
# =============================================================================
# codex-ws — roda a OpenAI Codex CLI contra um Aera Router LOCAL usando o
#            transporte Responses-over-WebSocket (em vez do HTTP da Cloud).
#
# POR QUE ESTE WRAPPER EXISTE
# ---------------------------
# O Aera Router expõe um proxy WebSocket para a API de Responses do Codex em
...
#     account"). É preciso mandar o id "puro" -> "gpt-5.5". (O Aera Router, no
...
#   AERA_ROUTER_WS_BASE   base URL do Aera Router local   (default abaixo)
#   AERA_ROUTER_WS_MODEL  modelo codex (id puro)        (default "gpt-5.5")
#   AERA_ROUTER_LOCAL_KEY API key (qualquer valor se REQUIRE_API_KEY=false)
...
AERA_ROUTER_WS_BASE="${AERA_ROUTER_WS_BASE:-http://127.0.0.1:20128/v1}"  # base do Aera Router local
AERA_ROUTER_WS_MODEL="${AERA_ROUTER_WS_MODEL:-gpt-5.5}"                  # id PURO (sem "codex/")
...
export AERA_ROUTER_LOCAL_KEY="${AERA_ROUTER_LOCAL_KEY:-local}"
...
# Gerado por codex-ws.sh — config isolada para o WS local do Aera Router.
model = "$AERA_ROUTER_WS_MODEL"
model_provider = "aera-router-local"

[model_providers.aera-router-local]
name = "Aera Router Local (WS)"
base_url = "$AERA_ROUTER_WS_BASE"   # a URL WebSocket é derivada desta base pela CLI
...
env_key = "AERA_ROUTER_LOCAL_KEY"   # a CLI lê a key Bearer desta env var
...
overrides=(-c model="$AERA_ROUTER_WS_MODEL" -c model_provider="aera-router-local")
