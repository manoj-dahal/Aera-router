...
export function getAeraRouterTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatAeraRouterCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

/**
 * Build the `X-Aera Router-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-Aera Router-Provider` header already applies)
 ...
 */
export function buildAeraRouterDecisionHeaderValue({
  strategy = null,
  provider = null,
  latencyMs = 0,
}: {
...
export function buildAeraRouterResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
   ...
   * Emitted as `X-Aera Router-Cost-Saved` only when provided (omitted on normal
   ...
   * request) surfaced via `X-Aera Router-Decision`. See #6022.
   */
  strategy?: string | null;
  usage?: UsageLike;
}): Record<string, string> {
  const tokens = getAeraRouterTokenCounts(usage);
  const headers: Record<string, string> = {
    [AERA_ROUTER_RESPONSE_HEADERS.cacheHit]: toHeaderValue(String(cacheHit)),
    [AERA_ROUTER_RESPONSE_HEADERS.latencyMs]: toHeaderValue(
      String(toNonNegativeInteger(latencyMs))
    ),
    [AERA_ROUTER_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatAeraRouterCost(costUsd)),
    [AERA_ROUTER_RESPONSE_HEADERS.tokensIn]: toHeaderValue(String(tokens.input)),
    [AERA_ROUTER_RESPONSE_HEADERS.tokensOut]: toHeaderValue(String(tokens.output)),
    [AERA_ROUTER_RESPONSE_HEADERS.version]: toHeaderValue(APP_CONFIG.version),
  };

  if (typeof model === "string" && model.trim().length > 0) {
    headers[AERA_ROUTER_RESPONSE_HEADERS.model] = toHeaderValue(model);
  }

  if (typeof requestId === "string" && requestId.trim().length > 0) {
    headers[AERA_ROUTER_RESPONSE_HEADERS.requestId] = toHeaderValue(requestId);
  }

  if (typeof provider === "string" && provider.trim().length > 0) {
    headers[AERA_ROUTER_RESPONSE_HEADERS.provider] = toHeaderValue(getProviderAlias(provider));
  }
  ...
  if (costSavedUsd != null) {
    headers[AERA_ROUTER_RESPONSE_HEADERS.costSaved] = toHeaderValue(
      formatAeraRouterCost(costSavedUsd)
    );
  }

  const attempts = toNonNegativeInteger(fallbackAttempts);
  if (attempts > 0) {
    headers[AERA_ROUTER_RESPONSE_HEADERS.fallbackAttempts] = toHeaderValue(String(attempts));
  }

  const decisionValue = buildAeraRouterDecisionHeaderValue({ strategy, provider, latencyMs });
  if (decisionValue !== null) {
    headers[AERA_ROUTER_RESPONSE_HEADERS.decision] = decisionValue;
  }

  return headers;
}

export function buildAeraRouterSseMetadataComment(
  options: Parameters<typeof buildAeraRouterResponseMetaHeaders>[0]
): string {
  const headers = buildAeraRouterResponseMetaHeaders(options);
...
/**
 * Single choke-point for attaching the X-Aera Router-* response meta headers.
 ...
 */
export function attachAeraRouterMetaHeaders(
  headers: Headers | Record<string, string>,
  meta: Parameters<typeof buildAeraRouterResponseMetaHeaders>[0]
): void {
  const built = buildAeraRouterResponseMetaHeaders(meta);
...
/**
 * Attach the X-Aera Router-* meta headers onto an already-built Response, ADDING
 ...
 */
export function attachAeraRouterMetaToResponse(
  response: Response,
  meta: Parameters<typeof buildAeraRouterResponseMetaHeaders>[0]
): Response {
  ...
  try {
    attachAeraRouterMetaHeaders(response.headers, meta);
    return response;
  } catch {
    ...
    attachAeraRouterMetaHeaders(cloned.headers, meta);
    return cloned;
  }
}
