 ...
 *
 * Re-issues the captured request through the local Aera Router instance and
...
const AERA_ROUTER_BASE = process.env.AERA_ROUTER_BASE_URL ?? "http://127.0.0.1:20128";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${AERA_ROUTER_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-aera-router-source": "inspector-replay",
  };
