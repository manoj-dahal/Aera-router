...
import { setSystemPromptConfig } from "@aera-router/open-sse/services/systemPrompt.ts";
 ...
 * via `AERA_ROUTER_DB_IMPORT_MAX_MB`. Invalid / out-of-range values fall back to the 100 MB
...
export function resolveMaxUploadSizeBytes(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.AERA_ROUTER_DB_IMPORT_MAX_MB;
...
// Required tables that must exist in a valid Aera Router database
...
export async function POST(request: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          error:
            ...
            `Set AERA_ROUTER_DB_IMPORT_MAX_MB to raise it, or VACUUM the database before exporting.`,
        },
        { status: 400 }
      );
    }
    ...
    tmpPath = path.join(os.tmpdir(), `aera-router-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid Aera Router database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
