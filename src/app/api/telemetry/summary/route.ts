...
export async function GET(request) {
  try {
    ...
    const { getQuotaMonitorSummary } =
      await import("@aera-router/open-sse/services/quotaMonitor.ts");
    const { getActiveSessions } = await import("@aera-router/open-sse/services/sessionManager.ts");
