...
/**
 * Render the Aera Router config block in CodeWhale TOML format.
...
function renderCodewhaleConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# CodeWhale config — managed by Aera Router (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
...
// POST — write Aera Router settings to CodeWhale's config.toml (primary), and
...
// DELETE — remove Aera Router CodeWhale config (primary + legacy, if present)
