  ...
  try {
    switch (toolId) {
      ...
      case "opencode":
        ...
        // OpenCode reads ~/.config/opencode/opencode.json — write the Aera Router settings there.
...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Build the Aera Router model entry
  ...
  const routerModel = {
    ...
    model: model,
    provider: "openai",
    apiKey: apiKey || "sk_aera_router",
    aeraRouterManaged: true,
  };
  ...
  // Check if Aera Router entry already exists and update it, or add new
  const existingIdx = models.findIndex(
    (m) =>
      m &&
      (m.aeraRouterManaged === true ||
        normalizeApiBase(m.apiBase) === normalizedBaseUrl.toLowerCase() ||
        normalizeApiBase(m.apiBase).includes("aera-router") ||
 ...
 * keys, but make sure the Aera Router provider entry is present and selected.
 */
async function saveHermesConfig({ baseUrl, apiKey, model }) {
  ...
  const nextConfig = {
    ...
    providers: {
      ...
      "aera-router": {
        ...((existingConfig.providers && existingConfig.providers["aera-router"]) || {}),
        base_url: providerBaseUrl,
        api_key:
          apiKey ||
          (existingConfig.providers &&
            existingConfig.providers["aera-router"] &&
            existingConfig.providers["aera-router"].api_key) ||
          "",
      },
    },
  };
