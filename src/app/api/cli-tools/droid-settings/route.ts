...
// Check if settings has Aera Router customModels.
// Multi-model entries are stored as `custom:Aera-Router-0`, `custom:Aera-Router-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasAeraRouterConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isAeraRouterCustomModel);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      settings,
      hasAeraRouter: hasAeraRouterConfig(settings),
...
// POST - Update Aera Router customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing Aera Router config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isAeraRouterCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend Aera Router entries (one per requested model)
...
// DELETE - Remove Aera Router customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Aera Router customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isAeraRouterCustomModel(m));
