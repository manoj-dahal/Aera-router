...
/**
 * Render the Aera Router provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by Aera Router (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-Aera-Router marker comment.
 */
...
// POST — write Aera Router settings to Forge config.toml
...
// DELETE — remove Forge Aera Router config
