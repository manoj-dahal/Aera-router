...
// POST — write Aera Router settings to Pi config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge Aera Router settings (pi uses OpenAI-compatible config)
...
// DELETE — remove Aera Router settings from Pi config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Pi Aera Router settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
