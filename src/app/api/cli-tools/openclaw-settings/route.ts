...
// Check if settings has Aera Router config
const hasAeraRouterConfig = (settings: any) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["aera-router"];
};
...
// POST - Update Aera Router settings (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    settings.agents.defaults.model.primary = `aera-router/${model}`;

    // Update models.providers.aera-router
    settings.models.providers["aera-router"] = {
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove Aera Router from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["aera-router"];
    ...
    // Reset agents.defaults.model.primary if it uses aera-router
    if (settings.agents?.defaults?.model?.primary?.startsWith("aera-router/")) {
