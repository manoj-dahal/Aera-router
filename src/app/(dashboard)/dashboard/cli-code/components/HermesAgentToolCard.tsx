...
export default function HermesAgentToolCard({
  ...
  const setRoleSelection = (roleId: string, model: string, provider = "Aera Router") => {
    setSelections((prev) => ({ ...prev, [roleId]: { model, provider } }));
  };

  const applyToAll = (model: string) => {
    const newSel: Record<string, RoleSelection> = {};
    HERMES_ROLES.forEach((r) => (newSel[r.id] = { model, provider: "Aera Router" }));
  ...
  const effectiveRoles = React.useMemo(() => {
    // If user has pending changes, treat selected roles as Aera Router
    if (Object.keys(selections).length > 0) {
      const map: Record<string, any> = {};
      HERMES_ROLES.forEach((r) => {
        if (selections[r.id]) {
          map[r.id] = { usingAeraRouter: true };
  ...
  // Count of roles that are (or will be) routed via Aera Router
  const configuredRolesCount = HERMES_ROLES.filter((role) => {
    // Pending selection always counts as Aera Router intent
    if (selections[role.id]) return true;

    const info = effectiveRoles[role.id];
    if (!info) return false;

    // Support both shapes: detector shape (usingAeraRouter) and settings shape (provider + base_url)
    if (typeof info.usingAeraRouter === "boolean") {
      return info.usingAeraRouter;
    }
    return (
      info?.provider === "aera-router" ||
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {HERMES_ROLES.map((role) => {
              ...
              // - If user has selected something in this session (pending): show as via Aera Router
              // - Else if current from disk: show real provider name + "(not Aera Router)" or "Aera Router"
              let badge: { label: string; pending: boolean; outsideAeraRouter: boolean } | null =
                null;

              if (sel) {
                // pending change made via the Select modal / quick apply → will be routed via Aera Router
                const prov = sel.provider || "Aera Router";
                badge = {
                  label: t("hermesViaAeraRouter", { provider: prov }),
                  pending: true,
                  outsideAeraRouter: false,
                };
              } else if (current) {
                const isOmni =
                  current?.provider === "aera-router" ||
                  (current?.base_url || "").includes("20128") ||
                  (current?.base_url || "").includes("localhost");

                if (isOmni) {
                  badge = { label: "Aera Router", pending: false, outsideAeraRouter: false };
                } else {
                  const realProvider = current.provider || t("other");
                  badge = {
                    label: t("hermesNotAeraRouter", { provider: realProvider }),
                    pending: false,
                    outsideAeraRouter: true,
                  };
                }
              }
        ...
        onSelect={(model: any) => {
          if (modalRole) {
            ...
            if (modelValue) {
              ...
              const prov =
                (model && (model.provider || model.providerId || model.group)) || "Aera Router";
