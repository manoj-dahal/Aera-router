 ...
 *
 * Feature request: https://github.com/manoj-dahal/Aera-router/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("aera-router-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("aera-router-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "aera-router-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "Aera Router",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.aera-router}`,
      models,
    };

    const responsesConfig = {
      name: "Aera-Router-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.aera-router}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("aera-router-cli-key-copilot", value);
  };
