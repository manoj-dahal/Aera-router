 ...
 * `selectedApiKey` was empty — but the default `sk_aera_router` key is a valid
 * local-mode default and should not block Apply. The Reset button was disabled
 * whenever `codexStatus.hasAeraRouter` was false — but a user should always be
 * able to reset, even when Codex was never configured against Aera Router.
 */
 ...
 * `sk_aera_router` default kicks in, so an empty `selectedApiKey` must NOT
 * disable Apply.
 */
