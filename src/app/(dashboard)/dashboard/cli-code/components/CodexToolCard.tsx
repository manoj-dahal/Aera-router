...
export default function CodexToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_aera_router for localhost if no key, otherwise use selected key
      const keyToUse =
  ...
  const getManualConfigs = () => {
    const keyToUse = !cloudEnabled ? "sk_aera_router" : "<YOUR_AERA_ROUTER_API_KEY>";

    let configContent = `# Aera Router Configuration for Codex CLI
model = "${selectedModel || CODEX_DEFAULT_MODELS[0]}"`;
...
# Utilize the built-in OpenAI provider pointed to Aera Router
