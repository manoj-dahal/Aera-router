 ...
 *      Aera Router itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      Aera Router's own `/v1` so the vision model can be any provider
 *      registered in Aera Router (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. Aera Router self-loop (`http://localhost:20128/v1`) — auto-detected when
 *      the model uses a known Aera-Router-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require Aera Router self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through Aera Router's own router,
  // not through a direct OpenAI/Anthropic endpoint. Use the operator-configured
  // port via AERA_ROUTER_PORT / PORT env vars, falling back to the default 20128.
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // Aera Router itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the Aera Router self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so Aera Router can resolve the
      ...
      // When routing through Aera Router's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_aera_router as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_aera_router";
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${selfLoopApiKey}`,
      };
      if (useFullModelId) {
        headers["x-aera-router-disabled-guardrails"] = "vision-bridge";
      }
