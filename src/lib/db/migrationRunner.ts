...
function resolveMigrationsDir(): string {
  const configuredDir = process.env.AERA_ROUTER_MIGRATIONS_DIR;
  ...
  throw new Error(
    "[Migration] Could not resolve migrations directory. Set AERA_ROUTER_MIGRATIONS_DIR."
  );
}
 ...
 *
 * Set the threshold to 0 (via `AERA_ROUTER_MAX_PENDING_MIGRATIONS`) to disable
 * this safety check.
 */
 ...
 * the default via the `AERA_ROUTER_MAX_PENDING_MIGRATIONS` env var (#3416). This
 ...
 * `AERA_ROUTER_MIGRATIONS_DIR` convention used in resolveMigrationsDir(). Falls
 * back to the default on missing or invalid (non-numeric / negative) input.
 */
function resolveMaxPendingMigrations(): number {
  const raw = process.env.AERA_ROUTER_MAX_PENDING_MIGRATIONS;
...
function supportsFts5(db: SqliteAdapter): boolean {
  ...
  try {
    const probeTable = `__aera_router_fts5_probe_${crypto.randomUUID().replace(/-/g, "_")}`;
    db.transaction(() => {
...
function getMigrationFiles(): Array<{ version: string; name: string; path: string }> {
  ...
  // would record version=NNN in _aera_router_migrations; the rest would never run).
...
function getAppliedVersions(db: SqliteAdapter): Set<string> {
  const rows = db.prepare("SELECT version FROM _aera_router_migrations").all() as Array<{
    version: string;
  }>;
...
function getAppliedRecords(db: SqliteAdapter): Array<{ version: string; name: string }> {
  return db
    .prepare("SELECT version, name FROM _aera_router_migrations ORDER BY version")
    .all() as Array<{
    version: string;
    name: string;
  }>;
...
function reconcileRenumberedMigrations(
  ...
  for (const compatibility of RENAMED_MIGRATION_COMPATIBILITY) {
    ...
    const legacyRow = db
      .prepare("SELECT version, name FROM _aera_router_migrations WHERE version = ? AND name = ?")
    ...
    const targetRow = db
      .prepare("SELECT version FROM _aera_router_migrations WHERE version = ?")
      .get(compatibility.toVersion) as { version: string } | undefined;

    const applyRepair = db.transaction(() => {
      if (targetRow) {
        db.prepare("DELETE FROM _aera_router_migrations WHERE version = ? AND name = ?").run(
          compatibility.fromVersion,
          compatibility.fromName
        );
      } else {
        db.prepare(
          "UPDATE _aera_router_migrations SET version = ?, name = ? WHERE version = ? AND name = ?"
        ).run(
    ...
    if (residualRow) {
      ...
      db.prepare("DELETE FROM _aera_router_migrations WHERE version = ?").run(
        compatibility.fromVersion
      );
    }
  }
...
function rehomeLegacyVersionSlotMigrations(
  ...
  for (const legacy of LEGACY_VERSION_SLOT_MIGRATIONS) {
    ...
    const applyRepair = db.transaction(() => {
      ...
      db.prepare(
        "UPDATE _aera_router_migrations SET version = ? WHERE version = ? AND name = ?"
...
export function runMigrations(db: SqliteAdapter, options?: { isNewDb?: boolean }): number {
  ...
  if (mismatches.length > 0) {
    ...
    console.error(
      `[Migration] The version-only tracking will skip these (version already applied), ` +
        `but please report this to the Aera Router maintainers.`
    );
  }
  ...
  // from the _aera_router_migrations table.
  ...
  // #3416: resolve the threshold at call time so AERA_ROUTER_MAX_PENDING_MIGRATIONS
  ...
  if (
    ...
    if (plausiblePendingCount !== null && actionablePending.length <= plausiblePendingCount) {
      ...
      const bypassHint =
        ...
        `tracking table was wiped), set AERA_ROUTER_MAX_PENDING_MIGRATIONS=0 in your ` +
      ...
      if (memoizedSafetyAbort && memoizedSafetyAbort.message === msg) {
        console.error(
          `[Migration] 🛑 ABORT (repeat — see earlier detail): ` +
            ...
            `Set AERA_ROUTER_MAX_PENDING_MIGRATIONS=0 to bypass.`
        );
    ...
    try {
      ...
      if (message.includes("duplicate column name")) {
        const applyMarkerOnly = db.transaction(() => {
          db.prepare(
            "INSERT OR IGNORE INTO _aera_router_migrations (version, name) VALUES (?, ?)"
...
export function getMigrationStatus(db: SqliteAdapter): {
  ...
  const appliedRows = db
    .prepare("SELECT version, name, applied_at FROM _aera_router_migrations ORDER BY version")
