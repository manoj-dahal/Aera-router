...
import { resolveModelAlias } from "@aera-router/open-sse/services/modelDeprecation.ts";
...
function isRedisAuthCacheEnabled(): boolean {
  return (
    process.env.AERA_ROUTER_DISABLE_REDIS_AUTH_CACHE !== "1" &&
...
/**
 * Select an API key for internal Aera Router operations (combo health checks,
...
export async function getApiKeyMetadata(
  ...
  if (isConfiguredEnvApiKey(key)) {
    ...
    // The deployment-time env key (`AERA_ROUTER_API_KEY` / `ROUTER_API_KEY`)
