/**
 * Aera Router Copilot — Chat Engine
 *
...
function getKnowledgeResponse(query: string): string | null {
  ...
  if (
    ...
    return `## Aera Router Architecture
...
Use \`createCombo\` tool or \`runAeraRouterCli\` to create them.`;
  }
...
Aera Router supports 212+ providers across categories:
...
I can use CodeGraph to explore the Aera Router codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## Aera Router Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`aera-router health\`
- "CLI list-combos" → ejecuta \`aera-router list-combos\`
- "CLI set-budget 10" → ejecuta \`aera-router set-budget 10\`

### Conocimiento
- "Cómo funciona Aera Router?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with Aera Router.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runAeraRouterCli") {
    ...
    return {
      message: result,
      toolCalls: [{ name: "runAeraRouterCli", args: intent.args, result }],
    };
  }
