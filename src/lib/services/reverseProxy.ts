 ...
 * can be iframed inside the Aera Router dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between Aera Router and the embedded service.
 ...
 * Prevents Aera Router session cookies and Authorization from leaking upstream.
 */
 ...
 *   Aera Router origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the Aera Router iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (Aera Router controls framing via its own CSP).
