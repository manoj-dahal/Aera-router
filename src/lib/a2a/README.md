# Aera Router A2A Server

> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use Aera Router as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes Aera Router as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through Aera Router intelligent pipeline",
...
Routes prompts through Aera Router's intelligent pipeline with full observability.
...
Discovers Aera Router agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use Aera Router A2A as a custom LLM.
"""
...
An orchestrator agent delegates code generation to Aera Router, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via Aera Router A2A
    ...
    # Step 2: Review the code via Aera Router A2A (different model)
...
Multiple agents share quota through Aera Router, using the quota skill to coordinate.
...
```
Authorization: Bearer YOUR_AERA_ROUTER_API_KEY
```

If no API key is configured on the server (`AERA_ROUTER_API_KEY` is empty), authentication is bypassed.
