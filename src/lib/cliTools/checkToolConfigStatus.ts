...
import { hasAeraRouterQwenCodeConfig } from "@/shared/services/qwenCodeConfig";
import { getRuntimePorts } from "@/lib/runtime/ports";

const { apiPort } = getRuntimePorts();

/**
 * Check if a tool has Aera Router configured by reading its config file directly.
...
export async function checkToolConfigStatus(
  ...
  try {
    ...
    if (toolId === "codex") {
      ...
      const hasAeraRouter =
        lower.includes("aera-router") ||
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      if (!hasAeraRouter) return "not_configured";
    ...
    if (toolId === "hermes") {
      ...
      const hasAeraRouter =
        ...
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      return hasAeraRouter ? "configured" : "not_configured";
    }

    const config = JSON.parse(content) as Record<string, unknown>;

    // Each tool stores Aera Router config differently
    switch (toolId) {
      case "claude":
        return (config?.env as Record<string, unknown>)?.ANTHROPIC_BASE_URL
          ...
          : "not_configured";
      case "qwen":
        return hasAeraRouterQwenCodeConfig(config) ? "configured" : "not_configured";
      case "droid":
      case "openclaw":
      case "cline":
      case "kilo": {
        // Generic check: look for Aera-Router-specific markers in the config
        const configStr = JSON.stringify(config).toLowerCase();
        if (
          configStr.includes("aera-router") ||
          configStr.includes("sk_aera_router") ||
