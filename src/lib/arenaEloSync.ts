...
/**
 * Maps Arena leaderboard categories to Aera Router task-type categories.
 *
...
/**
 * Aera Router model aliases: canonical name → known aliases.
 * Creates additional DB entries for each alias so that models
 * are findable under any name Aera Router uses internally.
 */
 ...
 *
 * Leaderboard categories are mapped to Aera Router task types:
 * - "text" → default, review, documentation, debugging
 * - "code" → coding
 *
 * Known Aera Router model aliases are also expanded into additional entries.
 *
