...
import {
  ...
  refreshCopilotToken,
} from "@aera-router/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("AERA_ROUTER_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
 ...
 * (e.g. "codex,openai"). Targeted alternative to AERA_ROUTER_DISABLE_TOKEN_HEALTHCHECK:
...
function getHealthCheckSkipProviders(): Set<string> {
  const raw = process.env.AERA_ROUTER_HEALTHCHECK_SKIP_PROVIDERS || "";
  return new Set(
    raw
      .split(",")
...
async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("AERA_ROUTER_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
declare global {
  var __aeraRouterTokenHC:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null; sweeping: boolean }
    | undefined;
}
function getHCState() {
  if (!globalThis.__aeraRouterTokenHC) {
    globalThis.__aeraRouterTokenHC = { initialized: false, interval: null, sweeping: false };
  }
  return globalThis.__aeraRouterTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in Aera Router
