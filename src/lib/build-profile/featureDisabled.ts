/**
 * Helper used by the `AERA_ROUTER_BUILD_PROFILE=minimal` stubs to surface a
...
export class FeatureDisabledError extends Error {
  ...
  constructor(featureName: string) {
    super(
      `Feature "${featureName}" is disabled in this build (AERA_ROUTER_BUILD_PROFILE=minimal). ` +
        `Install the full aera-router artifact instead of aera-router-secure if you need this feature.`
    );
