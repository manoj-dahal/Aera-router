/**
 * Cloudflare Worker source emitter for the Aera Router proxy relay.
 *
 ...
 *
 * The Aera Router variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerUploadRequest(
  ...
  const boundary = `----AeraRouterCFWorker${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
  const parts: Buffer[] = [
    Buffer.from(
      `--${boundary}${CRLF}` +
...
export function buildCloudflareWorkerScript(relayAuth: string): string {
  ...
  return `// Aera Router Cloudflare Worker proxy relay — generated at deploy time.
