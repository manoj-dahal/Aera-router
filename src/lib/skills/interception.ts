...
import { AERA_ROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME } from "@aera-router/open-sse/services/webSearchFallback.ts";
import { AERA_ROUTER_WEB_FETCH_FALLBACK_TOOL_NAME } from "@aera-router/open-sse/services/webFetchInterception.ts";
...
const BUILTIN_TOOL_ALIASES: Record<string, string> = {
  [AERA_ROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME]: "web_search",
  [AERA_ROUTER_WEB_FETCH_FALLBACK_TOOL_NAME]: "web_fetch",
};
...
export async function handleToolCallExecution(
  ...
  switch (provider) {
    ...
    case "anthropic": {
      ...
      // blocks untouched, remove the Aera-Router-handled tool_use blocks, and expose
