...
// ─── Provider mapping: LiteLLM provider → Aera Router aliases ─────
...
/**
 * Transform LiteLLM raw data → Aera Router PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (Aera Router format).
 ...
 */
export function transformToAeraRouter(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToAeraRouter(raw);
