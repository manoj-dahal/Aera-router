 ...
 * with the running Aera Router server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.AERA_ROUTER_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.AERA_ROUTER_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.AERA_ROUTER_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
