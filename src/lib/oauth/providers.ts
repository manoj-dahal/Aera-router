 ...
 * navigate back to Aera Router instead of stalling on localhost.
 */
export function resolveBrowserOAuthRedirectUri(
  ...
  const publicBaseUrl =
    normalizeBaseUrl(env.NEXT_PUBLIC_BASE_URL) || normalizeBaseUrl(env.AERA_ROUTER_PUBLIC_BASE_URL);
