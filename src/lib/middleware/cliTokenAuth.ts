...
const HEADER_NAME = "x-aera-router-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local aera-router CLI.
 * Only accepted from loopback IPs. Disabled via AERA_ROUTER_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.AERA_ROUTER_DISABLE_CLI_TOKEN === "true") return false;
