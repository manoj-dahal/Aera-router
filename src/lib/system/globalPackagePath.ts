...
/**
 * Resolve the real install directory of the globally-installed `aera-router` package — the directory
 * that owns `node_modules/better-sqlite3` and so is the correct cwd for `npm rebuild`.
 *
 * Replaces the hardcoded `${globalRoot}/aera-router/app` assumption (Bug 3, security-report v3.8.15):
 * the global package root is `${npm root -g}/aera-router`, not `/aera-router/app`. We probe the real
 ...
 * global install.
 */
export async function resolveGlobalAeraRouterPath(
  ...
  const packageRoot = path.join(globalRoot, "aera-router");
