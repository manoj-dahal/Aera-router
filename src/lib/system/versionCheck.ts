 ...
 * ONLY from `npm info aera-router version --json` (the `npm` CLI binary). When that binary
...
const NPM_REGISTRY_LATEST_URL = "https://registry.npmjs.org/aera-router/latest";
...
const GITHUB_RELEASES_LATEST_URL =
  "https://api.github.com/repos/manoj-dahal/Aera-router/releases/latest";
  ...
  try {
    const res = await fetchImpl(GITHUB_RELEASES_LATEST_URL, {
      ...
      headers: {
        ...
        "User-Agent": "aera-router-version-check",
