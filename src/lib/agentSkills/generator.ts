...
function buildApiBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  if (ops.length === 0) {
    ...
    for (const op of ops) {
      ...
      lines.push('  -H "Authorization: Bearer $AERA_ROUTER_TOKEN"');
...
function buildCliBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  lines.push("```bash");
  lines.push("npm install -g aera-router   # or: npx aera-router");
  lines.push("aera-router --version");
  lines.push("```");
  lines.push("");
  ...
  if (cmds.length === 0) {
    ...
    for (const cmd of cmds) {
      ...
      lines.push("**Example:**\n");
      lines.push("```bash");
      lines.push(`aera-router ${cmd.name}`);
      lines.push("```");
      lines.push("");
    }
  }
