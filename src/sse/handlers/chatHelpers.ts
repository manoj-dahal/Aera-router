...
import { createBuiltinAutoCombo } from "@aera-router/open-sse/services/autoCombo/builtinCatalog.ts";
import * as log from "../utils/logger";
import { updateProviderCredentials } from "../services/tokenRefresh";
import {
  detectFormatFromEndpoint,
  getTargetFormat,
} from "@aera-router/open-sse/services/provider.ts";
import {
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@aera-router/open-sse/config/providerModels.ts";
import { handleChatCore } from "@aera-router/open-sse/handlers/chatCore.ts";
import {
  errorResponse,
  modelCooldownResponse,
  providerCircuitOpenResponse,
  unavailableResponse,
} from "@aera-router/open-sse/utils/error.ts";
...
} from "@aera-router/open-sse/utils/proxyFetch.ts";
...
import { getRuntimeProviderProfile } from "@aera-router/open-sse/services/accountFallback.ts";
...
export async function safeLogEvents({
  ...
  try {
    ...
    try {
      ...
      const { proxyConfigToUrl } = await import("@aera-router/open-sse/utils/proxyDispatcher.ts");
...
export function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-Aera-Router-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-Aera-Router-Session-Id", sessionId);
    return cloned;
  }
}
...
export function withSelectedConnectionHeader(
  ...
  try {
    response.headers.set("X-Aera-Router-Selected-Connection-Id", connectionId);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
    ...
    cloned.headers.set("X-Aera-Router-Selected-Connection-Id", connectionId);
    return cloned;
  }
}
