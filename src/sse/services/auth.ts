...
import { getQuotaScopeLabelForProvider } from "@aera-router/open-sse/services/antigravityQuotaFamily.ts";
import {
...
import { isLocalProvider } from "@aera-router/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "@aera-router/open-sse/config/constants.ts";
import {
  preflightQuota,
  isQuotaPreflightEnabled,
} from "@aera-router/open-sse/services/quotaPreflight.ts";
import { resolveResilienceSettings } from "@/lib/resilience/settings";
import { resolveModelLockoutSettings } from "@/lib/resilience/modelLockoutSettings";
import { syncHealthFromDB, type KeyHealth } from "@aera-router/open-sse/services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@aera-router/open-sse/services/errorClassifier.ts";
...
} from "@aera-router/open-sse/config/codexQuotaScopes.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-aera-router-session"),
    "header"
  );
 ...
 * - `Authorization: Bearer <key>` (OpenAI / Aera Router / Codex CLI / Bearer clients)
 ...
 * rejected by per-route gates that compare against Aera Router keys.
 *
 ...
 * Feature #1350: Supports AERA_ROUTER_API_KEY / ROUTER_API_KEY env vars as
...
export async function isValidApiKey(apiKey: string) {
  ...
  const envKey = process.env.AERA_ROUTER_API_KEY || process.env.ROUTER_API_KEY;
