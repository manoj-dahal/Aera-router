 ...
 * trust store under a DEDICATED slot (`aera-router-tproxy-ca.crt`), separate from
 * the static MITM cert (`aera-router-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "aera-router-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.AERA_ROUTER_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] AERA_ROUTER_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
