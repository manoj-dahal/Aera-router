...
const LINUX_CERT_NAME = "aera-router-mitm.crt";
  ...
  return new Promise((resolve) => {
    exec(
      script,
      {
        ...
        env: {
          ...process.env,
          CERT_NAME: "Aera Router MITM Root CA",
          CERT_PATH: certPath || "",
          ACTION: action,
        },
      },
...
export async function installCert(sudoPassword: string, certPath: string): Promise<void> {
  ...
  if (process.env.AERA_ROUTER_SKIP_SYSTEM_TRUST === "1") {
    console.log("[cert] AERA_ROUTER_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
 ...
 * cert-path-agnostic and keep writing to the same `aera-router-mitm.crt`
 ...
 * cleanup needed. Distinct from TPROXY's own `aera-router-tproxy-ca.crt` slot
...
// They install / remove the Aera Router MITM root CA from the OS trust store and
