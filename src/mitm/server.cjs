...
const ROUTER_BASE_URL = (
  process.env.AERA_ROUTER_BASE_URL ||
...
// Inspector. The token is injected by manager.ts (same value the Aera Router
 ...
 * shape. The route-only namespace is reserved for client-facing Aera Router model ids;
...
async function passthrough(req, res, bodyBuffer) {
  ...
  // Defense-in-depth loop guard (Gap 14). The x-aera-router-source header is the
...
async function intercept(req, res, bodyBuffer, override, sourceModel) {
  ...
  // The Aera Router router uses these to distinguish AgentBridge traffic from
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Aera Router ${response.status}: ${errText}`);
    }
...
async function startMitmServer() {
  ...
  const server = https.createServer(sslOptions, async (req, res) => {
    ...
    if (req.headers["x-aera-router-source"] === "aera-router") {
      vlog(1, `[MITM] → PASSTHROUGH (Aera Router source loop)`);
