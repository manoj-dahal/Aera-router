 ...
 *
 * Slim Docker images (e.g. `node:24-trixie-slim` used by Aera Router's runtime
...
/**
 * Truthy-env check for `AERA_ROUTER_NO_SUDO`. Inlined (not imported from
...
export function isNoSudoEnv(): boolean {
  const value = process.env.AERA_ROUTER_NO_SUDO;
 ...
 * the operator opts into root-less mode via `AERA_ROUTER_NO_SUDO` (#6122). No
...
export function execFileWithPassword(
  ...
  // `AERA_ROUTER_NO_SUDO` (root-less / user-namespace deployments — #6122), skip
...
export async function runElevatedPowerShell(script: string): Promise<string> {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aera-router-elevate-"));
  const scriptName = `aera-router-elevate-${crypto.randomUUID()}.ps1`;
