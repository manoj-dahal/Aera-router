 ...
 *
 * When a request carries `x-aera-router-compression`, docs promise the response echoes
 * `X-Aera-Router-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-aera-router-compression";
const COMPRESSION_RESPONSE_HEADER = AERA_ROUTER_RESPONSE_HEADERS.compression;
...
/**
 * Wrap a Response so it carries `X-Aera-Router-Compression: <mode>; source=request-header`
 * when the request supplied `x-aera-router-compression` and the inner pipeline did not
