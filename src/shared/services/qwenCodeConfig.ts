...
export const QWEN_CODE_ENV_KEY = "AERA_ROUTER_API_KEY";
...
/**
 * Identifies only entries owned by Aera Router. Generic custom endpoints are not
...
export const isManagedQwenCodeModel = (value: unknown): boolean => {
  ...
  if (value.envKey === QWEN_CODE_ENV_KEY || value.id === "aera-router") return true;

  return (
    typeof value.name === "string" &&
    value.name.endsWith(" (Aera Router)") &&
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (Aera Router)`,
...
/**
 * Merge one Aera Router model into Qwen Code's current V4 settings contract.
...
export const mergeQwenCodeSettings = (
  ...
  if (next.selectedProvider === "aera-router") delete next.selectedProvider;

  return next;
};

export const findAeraRouterQwenCodeModel = (settings: unknown): JsonRecord | undefined => {
...
export const hasAeraRouterQwenCodeConfig = (settings: unknown): boolean =>
  findAeraRouterQwenCodeModel(settings) !== undefined;

/** Remove only Aera-Router-owned Qwen Code entries and selection state. */
...
const OWNED_ENV_LINE = /^\s*(?:export\s+)?AERA_ROUTER_API_KEY\s*=/;
...
export const mergeQwenCodeEnv = (existing: unknown, apiKey: unknown): string => {
  ...
  lines.push(`${QWEN_CODE_ENV_KEY}=${JSON.stringify(String(apiKey || "sk_aera_router"))}`);
  return `${lines.join("\n")}\n`;
};
