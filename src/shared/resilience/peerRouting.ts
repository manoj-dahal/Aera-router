...
export const AERA_ROUTER_PEER_TRACE_HEADER = "X-Aera-Router-Peer-Trace";

type HeaderSource = Headers | Record<string, unknown> | null | undefined;
type PeerEnvironment = {
  AERA_ROUTER_INSTANCE_ID?: string;
  AERA_ROUTER_PEER_URLS?: string;
  AERA_ROUTER_PEER_MAX_HOPS?: string;
};
...
function getInstanceId(env: PeerEnvironment): string | null {
  const value = env.AERA_ROUTER_INSTANCE_ID?.trim() ?? "";
  return INSTANCE_ID_PATTERN.test(value) ? value : null;
}

function getMaxPeerHops(env: PeerEnvironment): number {
  const parsed = Number.parseInt(env.AERA_ROUTER_PEER_MAX_HOPS ?? "", 10);
...
export function isConfiguredAeraRouterPeer(
  targetUrl: string,
  ...
  return (env.AERA_ROUTER_PEER_URLS ?? "")
    .split(",")
...
export function inspectPeerRequest(
  ...
  const trace = parsePeerTrace(readHeader(headers, AERA_ROUTER_PEER_TRACE_HEADER));
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted Aera Router URL.
...
export function applyPeerTraceHeader(
  ...
  const instanceId = getInstanceId(env);
  if (!instanceId || !isConfiguredAeraRouterPeer(targetUrl, env)) return false;

  const trace = parsePeerTrace(readHeader(clientHeaders, AERA_ROUTER_PEER_TRACE_HEADER));
  if (!trace.includes(instanceId)) trace.push(instanceId);
  const traceHeaderLower = AERA_ROUTER_PEER_TRACE_HEADER.toLowerCase();
  for (const key of Object.keys(outgoingHeaders)) {
    if (key.toLowerCase() === traceHeaderLower) delete outgoingHeaders[key];
  }
  outgoingHeaders[AERA_ROUTER_PEER_TRACE_HEADER] = trace.join(",");
  return true;
}
