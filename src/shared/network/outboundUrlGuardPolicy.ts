...
// of `./outboundUrlGuard.ts` so the CLI (`aera-router setup-opencode`, loaded via tsx with no
...
export const PRIVATE_PROVIDER_URLS_ENV = "AERA_ROUTER_ALLOW_PRIVATE_PROVIDER_URLS";
...
// cloud-metadata endpoints stay blocked. Defaults ON (Aera Router is local-first); operators
// who only use public providers can disable it to restore strict SSRF blocking.
export const LOCAL_PROVIDER_URLS_ENV = "AERA_ROUTER_ALLOW_LOCAL_PROVIDER_URLS";
 ...
 * (Aera Router is local-first — local OpenAI-compatible providers should validate out of the
 * box). Disable via the `AERA_ROUTER_ALLOW_LOCAL_PROVIDER_URLS` flag (DB toggle or env) to
 ...
 * provider URLs (`AERA_ROUTER_ALLOW_PRIVATE_PROVIDER_URLS`, default OFF). Protocol and
